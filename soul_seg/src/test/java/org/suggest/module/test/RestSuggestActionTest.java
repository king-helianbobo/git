...
public class RestSuggestActionTest extends AbstractSuggestTest {
	...
	public List<String> getSuggestions(SuggestionQuery suggestionQuery)
		...
		String url = "http://localhost:" + port + "/" + suggestionQuery.index
				+ "/" + suggestionQuery.type + "/__suggest";
	...
	private void assertThatResponseHasNoShardFailures(Response r)
			throws IOException {
		XContentParser parser = JsonXContent.jsonXContent.createParser(r
				.getResponseBody());
		Map<String, Object> jsonResponse = parser.mapAndClose();
		assertThat(jsonResponse, hasKey("_shards"));
		Map<String, Object> shardResponse = (Map<String, Object>) jsonResponse
				.get("_shards");
		...
		Response r = httpClient
				.preparePost(
						"http://localhost:" + port + "/" + index
								+ "/product/__suggestRefresh").execute().get();
		assertThat(r.getStatusCode(), is(200));
	}
	...
	public void refreshFieldSuggesters(String index, String field)
		...
		Response r = httpClient
				.preparePost(
						"http://localhost:" + port + "/" + index
								+ "/product/__suggestRefresh")
				.setBody(jsonBody).execute().get();
		assertThat(r.getStatusCode(), is(200));
	}
	...
	public FstStats getStatistics() throws Exception {
		...
		Response r = httpClient
				.prepareGet("http://localhost:" + port + "/__suggestStatistics")
				.execute().get();
		assertThat(r.getStatusCode(), is(200));
	...
	private String createJSONQuery(SuggestionQuery suggestionQuery) {
		...
		query.append(String
				.format("\"field\": \"%s\", ", suggestionQuery.field));
		query.append(String.format("\"term\": \"%s\"", suggestionQuery.term));
		if (suggestionQuery.size != null) {
			query.append(String.format(", \"size\": \"%s\"",
					suggestionQuery.size));
		}
		if (suggestionQuery.suggestType != null) {
			query.append(String.format(", \"type\": \"%s\"",
					suggestionQuery.suggestType));
		}
		if (suggestionQuery.similarity != null
				&& suggestionQuery.similarity > 0.0
				&& suggestionQuery.similarity < 1.0) {
			query.append(String.format(", \"similarity\": \"%s\"",
					suggestionQuery.similarity));
		}
		if (Strings.hasLength(suggestionQuery.indexAnalyzer)) {
			query.append(String.format(", \"indexAnalyzer\": \"%s\"",
					suggestionQuery.indexAnalyzer));
		}
		if (Strings.hasLength(suggestionQuery.queryAnalyzer)) {
			query.append(String.format(", \"queryAnalyzer\": \"%s\"",
					suggestionQuery.queryAnalyzer));
		}
		if (Strings.hasLength(suggestionQuery.analyzer)) {
			query.append(String.format(", \"analyzer\": \"%s\"",
					suggestionQuery.analyzer));
		}
		query.append(String.format(
				", \"preserve_position_increments\": \"%s\"",
				suggestionQuery.preservePositionIncrements));
		query.append("}");
