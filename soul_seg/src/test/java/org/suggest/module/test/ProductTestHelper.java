...
public class ProductTestHelper {
	...
	static void indexProducts(List<Map<String, Object>> products, Node node)
			throws Exception {
	...
	public static void refreshIndex(String index, Node node)
			throws ExecutionException, InterruptedException {
		node.client().admin().indices().refresh(new RefreshRequest(index))
				.get();
	}

	public static void assertDocumentCountAfterIndexing(String index,
			long expectedDocumentCount, Node node) throws Exception {
		assertThat(getCurrentDocumentCount(index, node),
				is(expectedDocumentCount));
	}
