...
import static org.suggest.module.test.NodeTestHelper.createIndexWithMapping;
import static org.suggest.module.test.NodeTestHelper.createNode;
import static org.suggest.module.test.NodeTestHelper.stopNode;
...
import static org.suggest.module.test.ProductTestHelper.refreshIndex;
...
public abstract class AbstractSuggestTest {
	...
	public void stopNodes() throws Exception {
		node.client().admin().indices().prepareDelete("products").execute()
				.actionGet();
	...
	abstract public void refreshFieldSuggesters(String index, String field)
			throws Exception;
	...
	@Test
	public void testThatSimpleSuggestionShouldSupportLimitWithConcreteWord()
			throws Exception {
		List<Map<String, Object>> products = createProducts("ProductName",
				"foo", "fooba", "foobar");
		indexProducts(products, node);
	...
	@Test
	public void testThatSuggestionShouldWorkOnDifferentFields()
			throws Exception {
		List<Map<String, Object>> products = createProducts("ProductName",
				"Kochjacke Pute", "Kochjacke Henne", "Kochjacke Hahn");
		...
		List<String> suggestions = getSuggestions("ProductName.suggest",
				"kochjacke", 10);
		assertSuggestions(suggestions, "kochjacke", "kochjacke hahn",
				"kochjacke henne", "kochjacke pute");

		suggestions = getSuggestions("Description", "Kochjacke", 10);
		assertSuggestions(suggestions, "Kochjacke Hahn", "Kochjacke Henne",
				"Kochjacke Pute");
	}
	...
	public void testThatSuggestionShouldWorkWithWhitespaces() throws Exception {
		List<Map<String, Object>> products = createProducts("ProductName",
				"Kochjacke Paul", "Kochjacke Pauline", "Kochjacke Paulinea");
		indexProducts(products, node);

		List<String> suggestions = getSuggestions("ProductName.suggest",
				"kochja", 10);
		assertSuggestions(suggestions, "kochjacke", "kochjacke paul",
				"kochjacke pauline", "kochjacke paulinea");

		suggestions = getSuggestions("ProductName.suggest", "kochjacke ", 10);
		assertSuggestions(suggestions, "kochjacke paul", "kochjacke pauline",
				"kochjacke paulinea");

		suggestions = getSuggestions("ProductName.suggest",
				"kochjacke pauline", 10);
		assertSuggestions(suggestions, "kochjacke pauline",
				"kochjacke paulinea");
	}

	@Test
	public void testThatSuggestionWithShingleWorksAfterUpdate()
			throws Exception {
		List<Map<String, Object>> products = createProducts("ProductName",
				"Kochjacke Paul", "Kochjacke Pauline", "Kochjacke Paulinator");
		indexProducts(products, node);

		List<String> suggestions = getSuggestions("ProductName.suggest",
				"kochjacke", 10);
		assertSuggestions(suggestions, "kochjacke", "kochjacke paul",
				"kochjacke paulinator", "kochjacke pauline");
		...
		suggestions = getSuggestions("ProductName.suggest", "kochjacke paulin",
				10);
		assertSuggestions(suggestions, "kochjacke paulinator",
				"kochjacke pauline", "kochjacke paulinpanzer");
	...
	public void testThatSuggestionWorksWithSimilarity() throws Exception {
		List<Map<String, Object>> products = createProducts("ProductName",
				"kochjacke bla", "kochjacke blubb", "kochjacke blibb",
				"kochjacke paul");
		indexProducts(products, node);
	...
	public void testThatRefreshingPerIndexWorks() throws Exception {
		...
		// this ensures that when adding the second product it is added to the
		// same shard
		// if it is not added to the same shard, refreshing might not work as
		// expected
		// in case the data is added to a shard where there was no data in
		// before, it is added immediately to the
		// suggestions, this means more results than expected might be returned
		// in the last line of this test
		...
		SuggestionQuery productsQuery = new SuggestionQuery(DEFAULT_INDEX,
				DEFAULT_TYPE, "ProductName.suggest", "auto");
		SuggestionQuery secondProductsIndexQuery = new SuggestionQuery(
				"secondproductsindex", DEFAULT_TYPE, "ProductName.suggest",
				"auto");
		getSuggestions(productsQuery);
		getSuggestions(secondProductsIndexQuery);

		// index another product
		List<Map<String, Object>> newProducts = createProducts("ProductName",
				"automatik");
	...
	public void testThatRefreshingPerIndexFieldWorks() throws Exception {
		...
		SuggestionQuery suggestionQuery = new SuggestionQuery(DEFAULT_INDEX,
				DEFAULT_TYPE, "ProductName.suggest", "auto");
		SuggestionQuery lowerCaseQuery = new SuggestionQuery(DEFAULT_INDEX,
				DEFAULT_TYPE, "ProductName.lowercase", "auto");
	...
	public void testThatAnalyzingSuggesterWorks() throws Exception {
		...
		SuggestionQuery query = new SuggestionQuery(DEFAULT_INDEX,
				DEFAULT_TYPE, "ProductName.keyword", "b").suggestType("full")
				.analyzer("simple").size(10);
	...
	public void testThatAnalyzingSuggesterSupportsStopWords() throws Exception {
		...
		SuggestionQuery query = new SuggestionQuery(DEFAULT_INDEX,
				DEFAULT_TYPE, "ProductName.keyword", "b").suggestType("full")
				.indexAnalyzer("stop").queryAnalyzer("stop")
	...
	public void testThatFuzzySuggesterWorks() throws Exception {
		...
		SuggestionQuery query = new SuggestionQuery(DEFAULT_INDEX,
				DEFAULT_TYPE, "ProductName.keyword", "bwm")
	...
	@Test
	public void testThatFlushForcesReloadingOfAllFieldsWithoutErrors()
			throws Exception {
		List<Map<String, Object>> products = createProducts("ProductName",
				"BMW 318");
		indexProducts(products, node);
		...
		SuggestionQuery query = new SuggestionQuery(DEFAULT_INDEX,
				DEFAULT_TYPE, "ProductName.keyword", "bwm").suggestType("full");
		getSuggestions(query);

		// add data to index and flush
		// indexProducts(createProducts("ProductName", "BMW 320"), node);
		node.client().prepareIndex(DEFAULT_INDEX, DEFAULT_TYPE, "foo")
				.setSource(createProducts(1).get(0)).execute().actionGet();
		node.client().admin().indices().prepareFlush(DEFAULT_INDEX).execute()
				.actionGet();
		getSuggestions(query);
	}
	...
	public void gettingStatisticsShouldWork() throws Exception {
		...
		SuggestionQuery query = new SuggestionQuery(DEFAULT_INDEX,
				DEFAULT_TYPE, "ProductName.keyword", "b").suggestType("full")
				.analyzer("stop").size(10);
		getSuggestions(query);

		FstStats filledFstStats = getStatistics();
		assertThat(filledFstStats.getStats(), hasSize(greaterThanOrEqualTo(1)));

		List<FstStats.FstIndexShardStats> allStats = Lists
				.newArrayList(filledFstStats.getStats());
	...
	private List<String> getSuggestions(String field, String term, Integer size)
			throws Exception {
		return getSuggestions(new SuggestionQuery(DEFAULT_INDEX, DEFAULT_TYPE,
				field, term).size(size));
	}

	private List<String> getSuggestions(String field, String term,
			Integer size, Float similarity) throws Exception {
		return getSuggestions(new SuggestionQuery(DEFAULT_INDEX, DEFAULT_TYPE,
				field, term).size(size).similarity(similarity));
	}

	private void assertSuggestions(List<String> suggestions, String... terms) {
		String msg = String.format("%s should have size %s, content %s",
				suggestions, terms.length, Arrays.asList(terms));
		assertThat(msg, suggestions, hasSize(terms.length));
		assertThat("Suggestions are: " + suggestions, suggestions,
				contains(terms));
	}

	private void assertSuggestions(SuggestionQuery query, String... terms)
			throws Exception {
		List<String> suggestions = getSuggestions(query);
		String assertionError = String.format("%s for query %s should be %s",
				suggestions, query, Arrays.asList(terms));
	...
	private void cleanIndex() {
		node.client()
				.deleteByQuery(
						new DeleteByQueryRequest("products").types("product")
