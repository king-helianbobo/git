...
public class SynonymTokenFilterFactory
	...
	private SynonymMap synonymMap; // from lucene
	private final boolean ignoreCase;
	private final boolean expand; // whether expand or not
	...
	@Inject
	public SynonymTokenFilterFactory(Index index,
		...
		this.analyzer = new Analyzer() {
			...
			protected TokenStreamComponents createComponents(String fieldName,
				...
				Tokenizer tokenizer = tokenizerFactory == null
						? new WhitespaceTokenizer(Lucene.ANALYZER_VERSION,
	...
	public TokenStream create(TokenStream tokenStream) {
		...
		log.info("creatTokenStream is called!");
		if (synonymMap.fst == null) {
			log.info("synonymMap is null!");
			return tokenStream;
		} else {
			log.info("synonymMap is called: " + synonymMap.toString());
			return new SynonymFilter(tokenStream, synonymMap, ignoreCase);
		}
	}
			...
			try {
				...
				if (synonymFile.exists()
					...
					if ("wordnet".equalsIgnoreCase(format)) {
						...
						parser = new SolrSynonymParser(true, expand, analyzer);
						((SolrSynonymParser) parser).parse(rulesReader);
						log.info("Use solr formats, content is changed!");
					}

					synonymMap = parser.build();
					log.info("lastModified = "
							+ (synonymFile.lastModified() - lastModified));
					lastModified = synonymFile.lastModified();
