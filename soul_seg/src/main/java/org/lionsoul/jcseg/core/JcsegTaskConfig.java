...
public class JcsegTaskConfig {
	 ...
	 * Chinese and English mix word, like 'B超,AA制...' . soul_seg cann't deal
	 * with this situation
	 */
	...
	/** the maximum length for the text between the pair punctution. */
	...
	/**
	 * reset the value of its options from a properties file . <br />
	 * 
	 * @param proFile
	 *            path of library.properties file. when null is given, soul will
	 *            look up the default library.properties file. <br />
	 * 
	 * @throws IOException
	 */
	public void resetFromPropertyFile(String proFile) throws IOException {
		...
		if (proFile == null) {
			/*
			 * 1.load the the library.properties located with the jar file.
			 * 2.load the Library.propertiess from the class-path. 3.load the
			...
			/*
			 * jcseg properties file loading status report, show the correct
			 * properties file location information . <br />
			 * 
			 * @date 2013-07-06
			 */
			if (!jcseg_properties) {
				String _report = "jcseg properties[library.properties] file loading error: \n";
		...
		} else { /* load the mapping from the specified property file. */
		...
		if (lexDirs == null)
			throw new IOException(
					"lexicon.path property not find in library.properties file!!!");
		...
		for (String fpath : lexPath) {
			...
			if (!f.exists())
				throw new IOException("Invalid sub lexicon path " + fpath
						+ " for lexicon.path in library.properties");
