...
import org.elasticsearch.hadoop.serailize.ValueReader;
...
/**
 * ElasticSearch {@link InputFormat} for streaming data (typically based on a
 * query) from ElasticSearch. Returns the document ID as key and its content as
 * value.
 * 
 * <p/>
 * This class implements both the "old" (<tt>org.apache.hadoop.mapred</tt>) and
 * the "new" (<tt>org.apache.hadoop.mapreduce</tt>) API.
 */
public class ESInputFormat<K, V> extends InputFormat<K, V> implements
		org.apache.hadoop.mapred.InputFormat<K, V>, ConfigurationOptions {

	private static Log log = LogFactory.getLog(ESInputFormat.class);

	protected static class ShardInputSplit extends InputSplit implements
		...
		public ShardInputSplit(String nodeIp, int httpPort, String nodeId,
				String nodeName, Integer shard, String mapping) {
			this.nodeIp = nodeIp;
		...
		public String toString() {
			...
			builder.append("ShardInputSplit [node=[").append(nodeId)
					.append("/").append(nodeName).append("|").append(nodeIp)
					.append(":").append(httpPort).append("],shard=")
	...
	protected static abstract class ShardRecordReader<K, V> extends
			RecordReader<K, V> implements
		...
		public ShardRecordReader(org.apache.hadoop.mapred.InputSplit split,
				Configuration job, Reporter reporter) {
		...
		void init(ShardInputSplit esSplit, Configuration cfg) {
			...
			// target node
			settings.cleanUri().setHost(esSplit.nodeIp)
					.setPort(esSplit.httpPort);

			this.esSplit = esSplit;

			// initialize mapping/ scroll reader
			SerializationUtils.setValueReaderIfNotSet(settings,
					WritableValueReader.class, log);
			ValueReader reader = ObjectUtils.instantiate(
					settings.getSerializerValueReaderClassName(), settings);
			...
			if (StringUtils.hasText(mappingData)) {
				...
				log.warn(String
						.format("No mapping found for [%s] - either no index exists or the split configuration has been corrupted",
								esSplit));
			}
			...
			queryBuilder = QueryBuilder.query(settings).shard(esSplit.shardId)
					.onlyNode(esSplit.nodeId);

			if (log.isDebugEnabled()) {
				log.debug(String.format("Initializing RecordReader for [%s]",
						esSplit));
			}
		}
		...
		public void close() throws IOException {
			if (log.isDebugEnabled()) {
				log.debug(String.format("Closing RecordReader for [%s]",
						esSplit));
			}
			...
			if (result == null) {
				...
				if (log.isTraceEnabled()) {
					log.trace(String.format(
							"Received scroll [%s],  size [%d] for query [%s]",
							result, size, queryBuilder));
				}
			}
		...
		protected abstract K setCurrentKey(K oldApiKey, K newApiKey,
				Object object);

		protected abstract V setCurrentValue(V oldApiValue, V newApiKey,
				Object object);
	...
	protected static class WritableShardRecordReader extends
			ShardRecordReader<Text, MapWritable> {
		public WritableShardRecordReader() {
			super();
		}

		public WritableShardRecordReader(
				org.apache.hadoop.mapred.InputSplit split, Configuration job,
				Reporter reporter) {
		...
		@Override
		protected Text setCurrentKey(Text oldApiKey, Text newApiKey,
				Object object) {
		...
		@Override
		protected MapWritable setCurrentValue(MapWritable oldApiValue,
				MapWritable newApiKey, Object object) {
	...
	public List<InputSplit> getSplits(JobContext context) throws IOException {
		...
		// NOTE: this method expects a ShardInputSplit to be returned (which
		// implements both the old and the new API).
		return Arrays.asList((InputSplit[]) getSplits(conf,
				conf.getNumMapTasks()));
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	@Override
	public ShardRecordReader<K, V> createRecordReader(InputSplit split,
			TaskAttemptContext context) {
		return (ShardRecordReader<K, V>) new WritableShardRecordReader();
	}

	//
	// Old API - if this method is replaced, make sure to return a new/old-API
	// compatible InputSplit
	//
	...
	public org.apache.hadoop.mapred.InputSplit[] getSplits(JobConf job,
		...
		if (!targetShards.isEmpty()) {
			...
			log.info(String.format("Discovered mapping {%s} for [%s]", mapping,
		...
		if (settings.getIndexReadMissingAsEmpty() && targetShards.isEmpty()) {
			log.info(String.format("Index [%s] missing - treating it as empty",
					settings.getIndexType()));
		}
		...
		for (Entry<Shard, Node> entry : targetShards.entrySet()) {
			...
			splits[index++] = new ShardInputSplit(node.getIpAddress(),
					node.getHttpPort(), node.getId(), node.getName(),
					shard.getName(), savedMapping);
		}
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@Override
	public ShardRecordReader<K, V> getRecordReader(
			org.apache.hadoop.mapred.InputSplit split, JobConf job,
			Reporter reporter) {
		return (ShardRecordReader<K, V>) new WritableShardRecordReader(split,
				job, reporter);
	}
}