...
import org.elasticsearch.hadoop.serailize.ContentBuilder;
...
public abstract class InitializationUtils {
	...
	public static boolean setIdExtractorIfNotSet(Settings settings,
			Class<? extends IdExtractor> clazz, Log log) {
		if (!StringUtils.hasText(settings.getMappingIdExtractorClassName())) {
			settings.setProperty(
					ConfigurationOptions.ES_MAPPING_ID_EXTRACTOR_CLASS,
					clazz.getName());
			Log logger = (log != null ? log : LogFactory.getLog(clazz));
			logger.debug(String.format(
					"Using pre-defined id extractor [%s] as default",
	...
	public static <T> void saveSchemaIfNeeded(Object conf,
			ValueWriter<T> schemaWriter, T schema, Log log) {
				...
				if (schemaWriter == null) {
					...
					if (log.isDebugEnabled()) {
						log.debug(String.format(
								"Creating ES mapping [%s] from schema [%s]",
								content.toString(), schema));
					}
				}
			}
			client.close();
