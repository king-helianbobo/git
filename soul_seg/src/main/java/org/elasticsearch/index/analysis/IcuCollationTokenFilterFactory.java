...
/**
 * An ICU based collation token filter. There are two ways to configure
 * collation:
 * <p/>
 * <p>
 * The first is simply specifying the locale (defaults to the default locale).
 * The <tt>language</tt> parameter is the lowercase two-letter ISO-639 code. An
 * additional <tt>country</tt> and <tt>variant</tt> can be provided.
 * <p/>
 * <p>
 * The second option is to specify collation rules as defined in the <a
 * href="http://www.icu-project.org/userguide/Collate_Customization.html">
 * Collation customization</a> chapter in icu docs. The <tt>rules</tt> parameter
 * can either embed the rules definition in the settings or refer to an external
 * location (preferable located under the <tt>config</tt> location, relative to
 * it).
 */
public class IcuCollationTokenFilterFactory extends AbstractTokenFilterFactory {
	...
	@Inject
	public IcuCollationTokenFilterFactory(Index index,
			@IndexSettings Settings indexSettings, Environment environment,
		...
		if (rules != null) {
			...
			try {
				...
				throw new ElasticSearchIllegalArgumentException(
						"Failed to load collation rules", e);
			}
			try {
				...
				if (failureToResolve != null) {
					throw new ElasticSearchIllegalArgumentException(
							"Failed to resolve collation rules location",
							failureToResolve);
				} else {
					throw new ElasticSearchIllegalArgumentException(
							"Failed to parse collation rules", e);
				}
			}
		} else {
		...
		if (decomposition != null) {
			if (decomposition.equalsIgnoreCase("no")) {
				...
				throw new ElasticSearchIllegalArgumentException(
						"Invalid decomposition: " + decomposition);
			}
		}
		...
		Boolean hiraganaQuaternaryMode = settings.getAsBoolean(
				"hiraganaQuaternaryMode", null);
