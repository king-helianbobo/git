...
public abstract class ADictionary {
	...
	/**
	 * load all words form all the files under specified lexicon directory . <br />
	 * 
	 * @param lexDir
	 * 
	 * @throws IOException
	...
	public void startAutoload() {
		...
		autoloadThread = new Thread(new Runnable() {
			@Override
			public void run() {
				File todo = new File(config.getLexiconPath() + "/"
						+ AL_TODO_FILE);
				...
				while (true) {
					...
					try {
						...
						while ((line = reader.readLine()) != null) {
							...
							if ("".equals(line))
								continue;
							loadFromLexiconFile(config.getLexiconPath() + "/"
									+ line);
						}
						reader.close();
		...
		try {// read dictionary file
			...
			while ((line = br.readLine()) != null) {
				...
				if (line.charAt(0) == '#' && line.length() >= 1)
					continue;
				// the first line of the lexicon file.
				if (isFirstLine == true) {
					type = ADictionary.getIndex(line);
					isFirstLine = false;
					if (type >= 0)
						continue;
				}
				...
				if (type == ILexicon.STOP_WORD) {
					// this dictionary is stop word dictionary
				...
				// special lexicon , / doesn't exist
				if (line.indexOf('/') == -1) {
					/*
					 * 1. English and Chinese mixed words, 2. Chinese and
					 * English mixed words, 3. English punctuation words, don't
					 * have to limit its length.
					 */
					boolean olen = (type == ILexicon.EC_MIXED_WORD);
					olen = olen || (type == ILexicon.CE_MIXED_WORD);
					olen = olen || (type == ILexicon.EN_PUN_WORD);
					if (olen || line.length() <= config.MAX_LENGTH) {
						dic.add(type, line, IWord.T_CJK_WORD);
					}
				} else { // normal words lexicon file
					String[] wd = line.split("/");
					if (wd.length < 4) { // format check , at least >=4
					...
					if ((type == ILexicon.CJK_WORD)
							&& wd[0].length() > config.MAX_LENGTH)
						continue;

					if (dic.get(type, wd[0]) == null) {
						if (wd.length > 4)
							dic.add(type, wd[0], Integer.parseInt(wd[4]),
									IWord.T_CJK_WORD);
						else
							dic.add(type, wd[0], IWord.T_CJK_WORD);
					}

					IWord iword = dic.get(type, wd[0]);// each type have one map
					// System.out.println(wd.length);
					// set the pinying of the word.
					if (config.LOAD_CJK_PINYIN && !"null".equals(wd[2])) {
						iword.setPinyin(wd[2]);
					}

					boolean li = (type == ILexicon.CJK_WORD);
					String[] arr = iword.getSyn(); // 获得同义词词组
					if (config.LOAD_CJK_SYN && !"null".equals(wd[3])) {
						...
						for (int j = 0; j < syns.length; j++) {
							...
							if (li && syns[j].length() > config.MAX_LENGTH)
								continue;
							/*
							 * check the syn word is not exists, make sure the
							 * same syn word won't duplicated. (dictionary
							 * reload)
							 * 
							 * @date 2013-09-02
							 */
					...
					if (config.LOAD_CJK_POS && !"null".equals(wd[1])) {
						...
						for (int j = 0; j < pos.length; j++) {
							...
							iword.addPartSpeech(pos[j].trim());
						}
					}
				}
			}
