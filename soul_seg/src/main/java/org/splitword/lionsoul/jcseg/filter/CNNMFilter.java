package org.splitword.lionsoul.jcseg.filter;
...
public class CNNMFilter {
	...
	public static int convertCNToInteger(char c) {
	 ...
	 * @return int
	 */
	public static int cnNumberToArabic(String cnn, boolean flag) {
		cnn = cnn.trim();
		if (cnn.length() == 1)
			return convertCNToInteger(cnn.charAt(0));
		if (flag)
		...
		if (yi > -1) {
			val += cnNumberToArabic(cnn.substring(0, yi), false) * 100000000;
			if (yi < cnn.length() - 1)
			...
			if (cnn.length() == 1) {
				int arbic = convertCNToInteger(cnn.charAt(0));
				if (arbic <= 10)
					val += arbic * 10000000;
				cnn = "";
			}
		...
		if (wan > -1) {
			val += cnNumberToArabic(cnn.substring(0, wan), false) * 10000;
			if (wan < cnn.length() - 1)
		...
		if (qian > -1) {
			val += cnNumberToArabic(cnn.substring(0, qian), false) * 1000;
		...
		if (bai > -1) {
			val += cnNumberToArabic(cnn.substring(0, bai), false) * 100;
			if (bai < cnn.length() - 1)
		...
		if (shi > -1) {
			if (shi == 0)
				...
				val += cnNumberToArabic(cnn.substring(0, shi), false) * 10;
			if (shi < cnn.length() - 1)
		...
		for (int j = 0; j < cnn.length(); j++) {
			int a = convertCNToInteger(cnn.charAt(j));
			if (a > 0)
				val += a * Math.pow(10, cnn.length() - j - 1);
		}
		return val;
	}
	...
	public static int qCNNumericToArabic(String cnn) {
		...
		cnn = cnn.trim();
		for (int j = 0; j < cnn.length(); j++)
			val += convertCNToInteger(cnn.charAt(j))
