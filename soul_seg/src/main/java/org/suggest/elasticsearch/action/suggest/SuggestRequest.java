...
public class SuggestRequest extends BroadcastOperationRequest {
	...
	public ActionRequestValidationException validate() {
		...
		if (field == null || field.length() == 0) {
			validationException = ValidateActions.addValidationError(
					"No suggest field specified", validationException);
		}
		if (term == null || term.length() == 0) {
			validationException = ValidateActions.addValidationError(
					"No query term specified", validationException);
		}
	...
	public SuggestRequest preservePositionIncrements(
	...
	public String toString() {
		return String
				.format("[%s] %s, field[%s], term[%s], size[%s], similarity[%s], suggestType[%s], indexAnalyzer[%s], queryAnalyzer[%s]",
						Arrays.toString(indices), Arrays.toString(types),
						field, term, size, similarity, suggestType,
