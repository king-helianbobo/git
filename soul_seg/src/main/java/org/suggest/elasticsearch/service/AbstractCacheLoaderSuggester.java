...
public abstract class AbstractCacheLoaderSuggester<T> extends
	...
	public AbstractCacheLoaderSuggester(MapperService mapperService,
	...
	public T load(ShardSuggestService.FieldType fieldType) throws Exception {
		FieldMapper fieldMapper = mapperService.smartName(fieldType.field(),
				fieldType.types()).mapper();

		Analyzer queryAnalyzer = fieldMapper.searchAnalyzer();
		if (Strings.hasLength(fieldType.indexAnalyzer())) {
			NamedAnalyzer namedAnalyzer = analysisService.analyzer(fieldType
					.queryAnalyzer());
			if (namedAnalyzer == null) {
				throw new ElasticSearchException("Query analyzer["
						+ fieldType.queryAnalyzer() + "] does not exist.");
			}
		...
		if (Strings.hasLength(fieldType.indexAnalyzer())) {
			NamedAnalyzer namedAnalyzer = analysisService.analyzer(fieldType
					.indexAnalyzer());
			if (namedAnalyzer == null) {
				throw new ElasticSearchException("Index analyzer["
						+ fieldType.indexAnalyzer() + "] does not exist.");
			}
	...
	public static class CacheLoaderAnalyzingSuggester extends
		...
		@Override
		public AnalyzingSuggester getSuggester(Analyzer indexAnalyzer,
				Analyzer queryAnalyzer, ShardSuggestService.FieldType fieldType)
				throws Exception {
	...
	public static class CacheLoaderFuzzySuggester extends
		...
		@Override
		public FuzzySuggester getSuggester(Analyzer indexAnalyzer,
				Analyzer queryAnalyzer, ShardSuggestService.FieldType fieldType)
				throws Exception {
