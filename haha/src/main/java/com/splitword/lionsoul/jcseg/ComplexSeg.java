...
public class ComplexSeg extends ASegment {
	...
	public IChunk getBestCJKChunk(char chars[], int index) {
		...
		for (int x = 0; x < mword1.length; x++) {// the second layer
			idx_2 = index + mword1[x].getLength();
			if (idx_2 < chars.length) {
				...
				if (mword2.length == 1
					...
					return new Chunk(new IWord[] { mword1[mword1.length - 1] });
				}
				for (int y = 0; y < mword2.length; y++) {// the third layer
					...
					if (idx_3 < chars.length) {
						...
						chunkArr.add(new Chunk(new IWord[] { mword1[x],
								mword2[y] }));
					}
				}
			} else {
		...
		IChunk[] chunks = new IChunk[chunkArr.size()]; // 获得多个chunk，按规则选择最优的那几个
	...
	private IChunk filterChunks(IChunk[] chunks) {
		...
		if (afterChunks.length >= 2) {// 如果有两个以上的chunk
			...
			if (afterChunks.length >= 2) {
				...
				if (afterChunks.length >= 2) {
					...
					if (afterChunks.length >= 2) {
						/*
						 * Attention: there is chance for length of the chunks
						 * over 2 even after the four rules. we use the LASTRule
						 * to clear the Ambiguity.
						 */
						// return
						// LASTRule.createRule().call(afterChunks).getWords()[0];
						afterChunks = new IChunk[] { LASTRule.createRule()
								.call(afterChunks) };
					}
				}
			}
