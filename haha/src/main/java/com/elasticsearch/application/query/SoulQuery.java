...
public class SoulQuery {
	...
	protected Map<Integer, Float> fScores = new HashMap<Integer, Float>();
	...
	protected Map<Integer, List<String>> fillList() {
		fillBaseMap();
		fillNounWordMap();
		...
		for (int i = 0; i < seqMap.size(); i++) {
			...
			// result.add(keywords);
			result.put(i, keywords);
			log.info(keywords);
		}
		return result;
	}

	protected List<List<String>> fillList1() {
		fillBaseMap();
		fillNounWordMap();
	...
	private void fillNounWordMap() {
		...
		for (Integer pos : terms.keySet()) {
			...
			if (bNature && term.length() > 1)
				nounWords.add(pos); // string's length must larger than 1
			...
			seqMap.put(i, result);
			freqMap.put(i, TokenFreqMap.termTotalFreq(result.get(0)));
			i++;
		}
		int max = -1;
		for (Integer pos : freqMap.keySet()) {
			max = Math.max(freqMap.get(pos), max);
		}
		if (max < 0)
			max = 10;
		for (Integer pos : freqMap.keySet()) {
			int freq = freqMap.get(pos);
			if (freq == -1)
				freq = 10;
			float a = (float) Math.sqrt(1.0f * max / freq);
			fScores.put(pos, a);
		}
	}
