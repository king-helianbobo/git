...
public class QueuePlacementPolicy {
	...
	public static QueuePlacementPolicy fromXml(Element el,
		...
		for (int i = 0; i < elements.getLength(); i++) {
			...
			if (node instanceof Element) {
				...
				Class<? extends QueuePlacementRule> clazz = ruleClasses
						.get(ruleName);
				if (clazz == null) {
					throw new AllocationConfigurationException(
							"No rule class found for " + ruleName);
				}
				QueuePlacementRule rule = ReflectionUtils.newInstance(clazz,
						null);
	...
	public static QueuePlacementPolicy fromConfiguration(Configuration conf,
		...
		try {
			...
			throw new RuntimeException(
					"Should never hit exception when loading"
	 ...
	 *         should be rejected.
	 * @throws IOException
	 *             If an exception is encountered while getting the user's
	 *             groups
	 */
