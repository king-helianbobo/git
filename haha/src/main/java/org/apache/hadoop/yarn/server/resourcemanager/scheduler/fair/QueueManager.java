...
public class QueueManager {
	public static final Log LOG = LogFactory.getLog(QueueManager.class
			.getName());
	...
	/**
	 * Creates a leaf queue and places it in the tree. Creates any parents that
	 * don't already exist.
	 * 
	...
	private FSLeafQueue createLeafQueue(String name) {
		...
		AllocationConfiguration queueConf = scheduler
		...
		for (int i = newQueueNames.size() - 1; i >= 0; i--) {
			...
			if (i == 0) {
				...
				FSParentQueue newParent = new FSParentQueue(queueName,
						scheduler, parent);
				try {
	 ...
	 * queues with no apps in them. Incompatibility could be due to (1)
	 * leafToCreate being currently being a parent, or (2) an existing leaf
	 * queue in the ancestry of leafToCreate.
	 * 
	...
	private boolean removeEmptyIncompatibleQueues(String leafToCreate) {
		...
		// Ensure leafToCreate is not root and doesn't have the default queue in
		// its
		// ancestry.
		if (leafToCreate.equals(ROOT_QUEUE)
				|| leafToCreate.startsWith(ROOT_QUEUE + "."
	...
	public void updateAllocationConfiguration(AllocationConfiguration queueConf) {
		...
		for (FSQueue queue : queues.values()) {
			...
			try {
				SchedulingPolicy policy = queueConf.getSchedulingPolicy(queue
						.getName());
