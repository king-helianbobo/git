...
public class RMAppManager implements EventHandler<RMAppManagerEvent>,
	...
	public RMAppManager(RMContext context, YarnScheduler scheduler,
		...
		this.maxCompletedAppsInStateStore = conf
				.getInt(YarnConfiguration.RM_STATE_STORE_MAX_COMPLETED_APPLICATIONS,
	...
	static class ApplicationSummary {
		...
		static class SummaryBuilder {
			...
			<T> SummaryBuilder add(String key, T value) {
				String escapedString = StringUtils
						.escapeString(String.valueOf(value),
		 ...
		 * 
		 * @param app
		 *            {@link RMApp} whose summary is to be created, cannot be
		 *            <code>null</code>.
		 */
	...
	protected synchronized void finishApplication(ApplicationId applicationId) {
		if (applicationId == null) {
			...
			if (UserGroupInformation.isSecurityEnabled()) {
				rmContext.getDelegationTokenRenewer().applicationFinished(
						applicationId);
			}
	...
	protected void writeAuditLog(ApplicationId appId) {
		...
		if (success) {
			...
			RMAuditLogger.logFailure(app.getUser(), operation, msg,
					"RMAppManager", "App failed with state: " + app.getState(),
					appId);
		}
	}
	...
	protected synchronized void checkAppNumCompletedLimit() {
		...
		while (completedAppsInStateStore > this.maxCompletedAppsInStateStore) {
			ApplicationId removeId = completedApps.get(completedApps.size()
					- completedAppsInStateStore);
			RMApp removeApp = rmContext.getRMApps().get(removeId);
			LOG.info("Max number of completed apps kept in state store met:"
					+ " maxCompletedAppsInStateStore = "
					+ maxCompletedAppsInStateStore + ", removing app "
					+ removeApp.getApplicationId() + " from state store.");
	...
	protected void recoverApplication(ApplicationState appState, RMState rmState)
		...
		RMAppImpl application = createAndPopulateNewRMApp(appContext,
				appState.getSubmitTime(), appState.getUser());
		application.recover(rmState);
		if (isApplicationInFinalState(appState.getState())) {
			// We are synchronously moving the application into final state so
			// that
			// momentarily client will not see this application in NEW state.
			// Also
		...
		if (UserGroupInformation.isSecurityEnabled()) {
			...
			try {
				...
				application
						.handle(new RMAppEvent(appId, RMAppEventType.RECOVER));
			} catch (Exception e) {
	...
	private void validateResourceRequest(
		...
		// checked here as they have to be validated whether they are part of
		// new
	...
	private boolean isApplicationInFinalState(RMAppState rmAppState) {
		if (rmAppState == RMAppState.FINISHED
				|| rmAppState == RMAppState.FAILED
	...
	public void recover(RMState state) throws Exception {
		...
		Map<ApplicationId, ApplicationState> appStates = state
				.getApplicationState();
	...
	public void handle(RMAppManagerEvent event) {
		...
		LOG.debug("RMAppManager processing event for " + applicationId
