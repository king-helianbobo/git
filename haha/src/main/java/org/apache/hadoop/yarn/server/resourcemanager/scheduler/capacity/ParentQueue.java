...
public class ParentQueue implements CSQueue {
	...
	public ParentQueue(CapacitySchedulerContext cs, String queueName,
		...
		if (rootQueue
			...
			throw new IllegalArgumentException("Illegal " + "capacity of "
					+ rawCapacity + " for queue " + queueName + ". Must be "
					+ CapacitySchedulerConfiguration.MAXIMUM_CAPACITY_VALUE);
		}

		float capacity = (float) rawCapacity / 100;
		float parentAbsoluteCapacity = (rootQueue) ? 1.0f : parent
				.getAbsoluteCapacity();
		...
		LOG.info("Initialized parent-queue " + queueName + " name=" + queueName
	...
	private synchronized void setupQueueConfigs(Resource clusterResource,
		...
		LOG.info(queueName + ", capacity=" + capacity + ", asboluteCapacity="
				+ absoluteCapacity + ", maxCapacity=" + maximumCapacity
				+ ", asboluteMaxCapacity=" + absoluteMaxCapacity + ", state="
				+ state + ", acls=" + aclsString);
	}
	...
	public String toString() {
		return queueName + ": " + "numChildQueue= " + childQueues.size() + ", "
				+ "capacity=" + capacity + ", " + "absoluteCapacity="
				+ absoluteCapacity + ", " + "usedResources=" + usedResources
				+ "usedCapacity=" + getUsedCapacity() + ", " + "numApps="
	...
	public synchronized void reinitialize(CSQueue newlyParsedQueue,
		...
		for (Map.Entry<String, CSQueue> e : newChildQueues.entrySet()) {
			...
			if (childQueue != null) {
				...
				LOG.info(getQueueName() + ": re-configured queue: "
						+ childQueue);
			} else {
				...
				LOG.info(getQueueName() + ": added new child queue: "
						+ newChildQueue);
			}
		}
	...
	private synchronized void addApplication(ApplicationId applicationId,
		...
		LOG.info("Application added -" + " appId: " + applicationId + " user: "
	...
	public synchronized void removeApplication(ApplicationId applicationId,
		...
		LOG.info("Application removed -" + " appId: " + applicationId
				+ " user: " + user + " leaf-queue of parent: " + getQueueName()
				+ " #applications: " + getNumApplications());
	}
		...
		while (canAssign(clusterResource, node)) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("ParentQ=" + getQueueName()
						+ " assignedSoFarInThisIteration="
						+ assignment.getResource() + " usedCapacity="
						+ getUsedCapacity() + " absoluteUsedCapacity="
						+ getAbsoluteUsedCapacity());
			}

			// Do not assign more than one container if this isn't the root
			// queue
	...
	private synchronized boolean assignToQueue(Resource clusterResource) {
		...
		// using...
		float currentCapacity = Resources.divide(resourceCalculator,
				clusterResource, usedResources, clusterResource);

		if (currentCapacity >= absoluteMaxCapacity) {
			LOG.info(getQueueName() + " used=" + usedResources
	...
	private boolean canAssign(Resource clusterResource, FiCaSchedulerNode node) {
		return (node.getReservedContainer() == null)
				&& Resources.greaterThanOrEqual(resourceCalculator,
						clusterResource, node.getAvailableResource(),
						minimumAllocation);
	}
	...
	synchronized CSAssignment assignContainersToChildQueues(Resource cluster,
		...
		for (Iterator<CSQueue> iter = childQueues.iterator(); iter.hasNext();) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Trying to assign to queue: "
						+ childQueue.getQueuePath() + " stats: " + childQueue);
			}
			...
			// If we do assign, remove the queue and re-insert in-order to
			// re-sort
			if (Resources.greaterThan(resourceCalculator, cluster,
					assignment.getResource(), Resources.none())) {
				// Remove and re-insert to sort
				iter.remove();
				LOG.info("Re-sorting assigned queue: "
						+ childQueue.getQueuePath() + " stats: " + childQueue);
	...
	public void completedContainer(Resource clusterResource,
		...
		if (application != null) {
			...
			for (Iterator<CSQueue> iter = childQueues.iterator(); iter
					.hasNext();) {
				CSQueue csqueue = iter.next();
				if (csqueue.equals(completedChildQueue)) {
					iter.remove();
					LOG.info("Re-sorting completed queue: "
							+ csqueue.getQueuePath() + " stats: " + csqueue);
