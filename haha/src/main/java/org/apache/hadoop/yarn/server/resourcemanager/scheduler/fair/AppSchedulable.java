...
public class AppSchedulable extends Schedulable {
	 ...
	 * bookeeping. This dispatches ro relevant handlers in the
	 * {@link FSSchedulerNode} and {@link SchedulerApp} classes.
	 */
	private void reserve(Priority priority, FSSchedulerNode node,
			Container container, boolean alreadyReserved) {
		LOG.info("Making reservation: node=" + node.getNodeName() + " app_id="
				+ app.getApplicationId());
		if (!alreadyReserved) {
			getMetrics()
					.reserveResource(app.getUser(), container.getResource());
	 ...
	 * This dispatches to the SchedulerApp and SchedulerNode handlers for an
	 * unreservation.
	 */
	...
	private Resource assignContainer(FSSchedulerNode node, Priority priority,
		...
		if (Resources.fitsIn(capability, available)) {
			...
			RMContainer allocatedContainer = app.allocate(type, node, priority,
					request, container);
	...
	private Resource assignContainer(FSSchedulerNode node, boolean reserved) {
		...
		Collection<Priority> prioritiesToTry = (reserved) ? Arrays.asList(node
				.getReservedContainer().getReservedPriority()) : app
				.getPriorities();
		...
		synchronized (app) {
			for (Priority priority : prioritiesToTry) {
				...
				ResourceRequest rackLocalRequest = app.getResourceRequest(
				...
				if (scheduler.isContinuousSchedulingEnabled()) {
					allowedLocality = app.getAllowedLocalityLevelByTime(
							priority, scheduler.getNodeLocalityDelayMs(),
							scheduler.getRackLocalityDelayMs(), scheduler
									.getClock().getTime());
				} else {
				...
				if (rackLocalRequest != null
						&& !rackLocalRequest.getRelaxLocality()) {
					continue;
				}
				...
				ResourceRequest offSwitchRequest = app.getResourceRequest(
						priority, ResourceRequest.ANY);
				if (offSwitchRequest != null
						&& !offSwitchRequest.getRelaxLocality()) {
					continue;
				}
	...
	public boolean hasContainerForNode(Priority prio, FSSchedulerNode node) {
		...
		anyRequest != null
				...
				&&
				// If locality relaxation is turned off at *-level, there must
				// be a
				// non-zero request for the node's rack:
				(anyRequest.getRelaxLocality() || (rackRequest != null && rackRequest
						.getNumContainers() > 0))
				&&
		...
		// non-zero request for the node's rack:
				...
				(rackRequest == null || rackRequest.getRelaxLocality() || (nodeRequest != null && nodeRequest
						.getNumContainers() > 0)) &&
				// The requested container must be able to fit on the node:
				Resources
						.lessThanOrEqual(RESOURCE_CALCULATOR, null, anyRequest
								.getCapability(), node.getRMNode()
								.getTotalCapability());
	}
}
