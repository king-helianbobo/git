...
public class EmbeddedElectorService extends AbstractService implements
		ActiveStandbyElector.ActiveStandbyElectorCallback {
	private static final Log LOG = LogFactory
			.getLog(EmbeddedElectorService.class.getName());
	...
	protected synchronized void serviceInit(Configuration conf)
			throws Exception {
		conf = conf instanceof YarnConfiguration ? conf
	...
	public synchronized void becomeActive() throws ServiceFailedException {
		try {
			...
			throw new ServiceFailedException(
					"RM could not transition to Active", e);
		}
	}
	...
	public void enterNeutralMode() {
		/**
		 * Possibly due to transient connection issues. Do nothing. TODO: Might
		 * want to keep track of how long in this state and transition to
		 * standby.
		 */
	}
	...
	public synchronized void notifyFatalError(String errorMessage) {
		rmContext
				.getDispatcher()
				.getEventHandler()
				.handle(new RMFatalEvent(
						RMFatalEventType.EMBEDDED_ELECTOR_FAILED, errorMessage));
	}
	...
	private static byte[] createActiveNodeInfo(String clusterId, String rmId)
		...
		return YarnServerResourceManagerServiceProtos.ActiveRMInfoProto
				.newBuilder().setClusterId(clusterId).setRmId(rmId).build()
				.toByteArray();
	}
