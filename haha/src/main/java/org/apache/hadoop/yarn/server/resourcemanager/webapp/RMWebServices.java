...
public class RMWebServices {
	...
	public NodeInfo getNode(@PathParam("nodeId") String nodeId) {
		...
		if (nodeId == null || nodeId.isEmpty()) {
			throw new NotFoundException("nodeId, " + nodeId
					+ ", is empty or null");
		}
		ResourceScheduler sched = this.rm.getResourceScheduler();
		if (sched == null) {
	...
	public AppsInfo getApps(@Context HttpServletRequest hsr,
		...
		if (finishBegin != null && !finishBegin.isEmpty()) {
			...
			if (fBegin < 0) {
				throw new BadRequestException(
						"finishTimeBegin must be greater than 0");
			}
		}
		if (finishEnd != null && !finishEnd.isEmpty()) {
			...
			if (fEnd < 0) {
				throw new BadRequestException(
						"finishTimeEnd must be greater than 0");
			}
		}
		if (fBegin > fEnd) {
			throw new BadRequestException(
	...
	public ApplicationStatisticsInfo getAppStatistics(
		...
		if (types.size() == 0) {
			...
			throw new BadRequestException("# of applicationTypes = "
					+ types.size()
		...
		Map<YarnApplicationState, Map<String, Long>> scoreboard = buildScoreboard(
				states, types);

		// go through the apps in RM to count the numbers, ignoring the case of
		// the state/type name
		ConcurrentMap<ApplicationId, RMApp> apps = rm.getRMContext()
				.getRMApps();
		...
		for (Map.Entry<YarnApplicationState, Map<String, Long>> partScoreboard : scoreboard
			...
			for (Map.Entry<String, Long> statEntry : partScoreboard.getValue()
				...
				StatisticsItemInfo statItem = new StatisticsItemInfo(
						partScoreboard.getKey(), statEntry.getKey(),
						statEntry.getValue());
						...
						if (paramStr != null && !paramStr.trim().isEmpty()) {
							if (isState) {
								try {
									...
									YarnApplicationState.valueOf(paramStr
											.trim().toUpperCase());
								} catch (RuntimeException e) {
									YarnApplicationState[] stateArray = YarnApplicationState
											.values();
									String allAppStates = Arrays
											.toString(stateArray);
									throw new BadRequestException(
	...
	public AppInfo getApp(@Context HttpServletRequest hsr,
		...
		if (appId == null || appId.isEmpty()) {
			throw new NotFoundException("appId, " + appId
					+ ", is empty or null");
		}
		ApplicationId id;
