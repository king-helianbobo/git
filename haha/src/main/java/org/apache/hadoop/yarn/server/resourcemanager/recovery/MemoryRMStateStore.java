...
public class MemoryRMStateStore extends RMStateStore {
	...
	public synchronized RMState loadState() throws Exception {
		// return a copy of the state to allow for modification of the real
		// state
		RMState returnState = new RMState();
		returnState.appState.putAll(state.appState);
		returnState.rmSecretManagerState.getMasterKeyState().addAll(
	...
	public void updateApplicationStateInternal(ApplicationId appId,
		...
		LOG.info("Updating final state " + appStateData.getState()
				+ " for app: " + appId);
	...
	public synchronized void storeApplicationAttemptStateInternal(
		...
		ApplicationAttemptState attemptState = new ApplicationAttemptState(
				appAttemptId, attemptStateData.getMasterContainer(),
	...
	public synchronized void updateApplicationAttemptStateInternal(
		...
		ApplicationAttemptState updatedAttemptState = new ApplicationAttemptState(
				appAttemptId, attemptStateData.getMasterContainer(),
				credentials, attemptStateData.getStartTime(),
	...
	public synchronized void storeRMDelegationTokenAndSequenceNumberState(
		...
		Map<RMDelegationTokenIdentifier, Long> rmDTState = state.rmSecretManagerState
				.getTokenState();
		if (rmDTState.containsKey(rmDTIdentifier)) {
			IOException e = new IOException("RMDelegationToken: "
					+ rmDTIdentifier + "is already stored.");
			LOG.info("Error storing info for RMDelegationToken: "
					+ rmDTIdentifier, e);
			throw e;
		}
	...
	public synchronized void storeRMDTMasterKeyState(DelegationKey delegationKey)
		...
		state.getRMDTSecretManagerState().getMasterKeyState()
				.add(delegationKey);
		LOG.info("rmDTMasterKeyState SIZE: " + rmDTMasterKeyState.size());
	}

	@Override
	public synchronized void removeRMDTMasterKeyState(
