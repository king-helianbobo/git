...
public class ResourceManager extends CompositeService implements Recoverable {
	...
	protected void serviceInit(Configuration conf) throws Exception {
		...
		this.configurationProvider = ConfigurationProviderFactory
				.getConfigurationProvider(conf);
	...
	protected RMAppManager createRMAppManager() {
		return new RMAppManager(this.rmContext, this.scheduler,
				this.masterService, this.applicationACLsManager, this.conf);
	}
	...
	class RMActiveServices extends CompositeService {
		...
		protected void serviceInit(Configuration configuration)
			...
			containerAllocationExpirer = new ContainerAllocationExpirer(
					rmDispatcher);
			...
			try {
				...
				throw new RuntimeException("Failed to initialize scheduler",
						ioe);
			}
		...
		protected void serviceStart() throws Exception {
			...
			// The state store needs to start irrespective of recoveryEnabled as
			// apps
			...
			if (scheduler instanceof PreemptableResourceScheduler
				...
				if (policies.size() > 0) {
					...
					for (SchedulingEditPolicy policy : policies) {
						LOG.info("LOADING SchedulingEditPolicy:"
								+ policy.getPolicyName());
						policy.init(conf, rmContext.getDispatcher()
								.getEventHandler(),
								(PreemptableResourceScheduler) scheduler);
						// periodically check whether we need to take action to
						// guarantee
						// constraints
	...
	public static class SchedulerEventDispatcher extends AbstractService
		...
		@Override
		protected void serviceInit(Configuration conf) throws Exception {
			this.shouldExitOnError = conf.getBoolean(
			...
			public void run() {
				...
				while (!stopped && !Thread.currentThread().isInterrupted()) {
					...
					try {
						...
						// shutdown could have caused it and is probably
						// harmless.
						if (stopped) {
						...
						LOG.fatal(
								"Error in handling event type "
										+ event.getType() + " to the scheduler",
								t);
	...
	@Private
	public static final class RMContainerPreemptEventDispatcher implements
	...
	protected void startWepApp() {
		...
		if (WebAppUtils.getResolvedRMWebAppURLWithoutScheme(conf).equals(
				proxyHostAndPort)) {
	...
	protected boolean areActiveServicesRunning() {
		return activeServices != null
				&& activeServices.isInState(STATE.STARTED);
	}
	...
	private Dispatcher setupDispatcher() {
		...
		dispatcher
				.register(RMFatalEventType.class,
