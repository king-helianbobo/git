 ...
 * 
 * <pre>
 * WebApp wa = WebApps.$for(myApp).at(address, port).with(configuration)
 * 		.start(new WebApp() {
 * 			&#064;Override
 * 			public void setup() {
		...
		public WebApp start(WebApp webapp) {
			...
			try {
				...
				if (devMode) {
					if (port > 0) {
						try {
							new URL("http://localhost:" + port + "/__stop")
									.getContent();
							LOG.info("stopping existing webapp instance");
							Thread.sleep(100);
						} catch (ConnectException e) {
							LOG.info("no existing webapp instance found: {}",
									e.toString());
						} catch (Exception e) {// should not be fatal
							LOG.warn("error stopping existing instance: {}",
									e.toString());
						}
					} else {
				...
				if (this.httpPolicy == null) {
					...
					httpScheme = (httpPolicy == Policy.HTTPS_ONLY) ? WebAppUtils.HTTPS_PREFIX
				...
				HttpServer2.defineFilter(server.getWebAppContext(), "guice",
						GuiceFilter.class.getName(), null,
						new String[] { "/*" });
			...
			Injector injector = Guice.createInjector(webapp,
					new AbstractModule() {
						@Override
			...
			// save a guice filter instance for webapp stop (mostly for unit
			// tests)
