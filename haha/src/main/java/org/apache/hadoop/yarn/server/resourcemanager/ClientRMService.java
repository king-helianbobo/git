...
public class ClientRMService extends AbstractService implements
	...
	private boolean checkAccess(UserGroupInformation callerUGI, String owner,
		...
		return applicationsACLsManager.checkAccess(callerUGI,
				operationPerformed, owner, application.getApplicationId())
				|| queueACLsManager.checkAccess(callerUGI,
						QueueACL.ADMINISTER_QUEUE, application.getQueue());
	}
	...
	public GetApplicationAttemptReportResponse getApplicationAttemptReport(
		...
		if (allowAccess) {
			RMAppAttempt appAttempt = application.getAppAttempts().get(
					appAttemptId);
			if (appAttempt == null) {
				throw new ApplicationAttemptNotFoundException(
						"ApplicationAttempt " + appAttemptId
								+ " Not Found in RM");
			}
			ApplicationAttemptReport attemptReport = appAttempt
					.createApplicationAttemptReport();
			response = GetApplicationAttemptReportResponse
					.newInstance(attemptReport);
		} else {
			throw new YarnException("User " + callerUGI.getShortUserName()
					+ " does not have privilage to see this attempt "
					+ appAttemptId);
		}
		return response;
	}
	...
	public GetApplicationAttemptsResponse getApplicationAttempts(
		...
		if (allowAccess) {
			...
			throw new YarnException("User " + callerUGI.getShortUserName()
					+ " does not have privilage to see this aplication "
					+ appId);
		}
		return response;
	}
	...
	public GetContainerReportResponse getContainerReport(
		...
		if (allowAccess) {
			...
			RMContainer rmConatiner = this.rmContext.getScheduler()
	...
	public GetContainersResponse getContainers(GetContainersRequest request)
		...
		if (allowAccess) {
			...
			SchedulerAppReport schedulerAppReport = this.rmContext
					.getScheduler().getSchedulerAppInfo(appAttemptId);
	...
	public SubmitApplicationResponse submitApplication(
		...
		if (rmContext.getRMApps().get(applicationId) != null) {
			LOG.info("This is an earlier submitted application: "
					+ applicationId);
	...
	public KillApplicationResponse forceKillApplication(
		...
		return KillApplicationResponse.newInstance(application
	...
	public GetApplicationsResponse getApplications(
		...
		EnumSet<YarnApplicationState> applicationStates = request
		...
		if (queues != null && !queues.isEmpty()) {
			...
			appsIter = new Iterator<RMApp>() {
				...
				public boolean hasNext() {
					// Because queueAppLists has no empty lists, hasNext is
					// whether the
					// current list hasNext or whether there are any remaining
					// lists
		...
		while (appsIter.hasNext() && reports.size() < limit) {
			...
			if (scope == ApplicationsRequestScope.OWN
				...
				continue;
			} else if (scope == ApplicationsRequestScope.VIEWABLE
					&& !allowAccess) {
				continue;
			}

			if (applicationTypes != null && !applicationTypes.isEmpty()) {
				String appTypeToMatch = caseSensitive ? application
						.getApplicationType() : application
						.getApplicationType().toLowerCase();
			...
			if (start != null
					&& !start.containsLong(application.getStartTime())) {
				continue;
			}

			if (finish != null
					&& !finish.containsLong(application.getFinishTime())) {
				continue;
			}
	...
	public MoveApplicationAcrossQueuesResponse moveApplicationAcrossQueues(
		...
		try {
			...
			LOG.info("Error getting UGI ", ie);
			RMAuditLogger.logFailure("UNKNOWN",
					AuditConstants.MOVE_APP_REQUEST, "UNKNOWN",
					"ClientRMService", "Error getting UGI", applicationId);
			throw RPCUtil.getRemoteException(ie);
		}
		...
		if (application == null) {
			RMAuditLogger.logFailure(callerUGI.getUserName(),
					AuditConstants.MOVE_APP_REQUEST, "UNKNOWN",
					"ClientRMService", "Trying to move an absent application",
					applicationId);
		...
		if (EnumSet.of(RMAppState.NEW, RMAppState.NEW_SAVING,
				RMAppState.FAILED, RMAppState.FINAL_SAVING,
				RMAppState.FINISHING, RMAppState.FINISHED, RMAppState.KILLED,
				RMAppState.KILLING, RMAppState.FAILED).contains(
		...
		this.rmContext
				.getDispatcher()
				.getEventHandler()
				.handle(new RMAppMoveEvent(applicationId, request
						.getTargetQueue(), future));
		...
		RMAuditLogger.logSuccess(callerUGI.getShortUserName(),
				AuditConstants.MOVE_APP_REQUEST, "ClientRMService",
				applicationId);
