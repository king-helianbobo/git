...
public class ApplicationMasterService extends AbstractService implements
	...
	private final AllocateResponse resync = recordFactory
	...
	private ApplicationAttemptId authorizeRequest() throws YarnException {
		...
		try {
			...
			if (appTokenIdentifier == null) {
				tokenFound = false;
				message = "No AMRMToken found for user "
	...
	public RegisterApplicationMasterResponse registerApplicationMaster(
		...
		if (lock == null) {
			RMAuditLogger.logFailure(this.rmContext.getRMApps().get(appID)
					.getUser(), AuditConstants.REGISTER_AM,
					"Application doesn't exist in cache "
		...
		synchronized (lock) {
			...
			this.rmContext
					.getDispatcher()
					.getEventHandler()
					.handle(new RMAppAttemptRegistrationEvent(
							applicationAttemptId, request.getHost(), request
									.getRpcPort(), request.getTrackingUrl()));
			...
			response.setApplicationACLs(app
					.getRMAppAttempt(applicationAttemptId)
					.getSubmissionContext().getAMContainerSpec()
					.getApplicationACLs());
			response.setQueue(app.getQueue());
			if (UserGroupInformation.isSecurityEnabled()) {
				LOG.info("Setting client token master key");
				response.setClientToAMTokenMasterKey(java.nio.ByteBuffer
						.wrap(rmContext.getClientToAMTokenSecretManager()
								.getMasterKey(applicationAttemptId)
								.getEncoded()));
			}

			// For work-preserving AM restart, retrieve previous attempts'
			// containers
			...
			if (!transferredContainers.isEmpty()) {
				...
				for (Container container : transferredContainers) {
					try {
						nmTokens.add(rmContext.getNMTokenSecretManager()
								.createAndGetNMToken(app.getUser(),
										applicationAttemptId, container));
					} catch (IllegalArgumentException e) {
						// if it's a DNS issue, throw UnknowHostException
						// directly and that
						// will be automatically retried by RMProxy in RPC
						// layer.
				...
				LOG.info("Application " + appID + " retrieved "
						+ transferredContainers.size()
						+ " containers from previous" + " attempts and "
						+ nmTokens.size() + " NM tokens.");
			}
			return response;
		}
	}
		...
		synchronized (lock) {
			if (!hasApplicationMasterRegistered(applicationAttemptId)) {
				...
				RMAuditLogger.logFailure(
						this.rmContext.getRMApps()
								.get(applicationAttemptId.getApplicationId())
								.getUser(), AuditConstants.UNREGISTER_AM, "",
						"ApplicationMasterService", message,
						applicationAttemptId.getApplicationId(),
						applicationAttemptId);
				throw new InvalidApplicationMasterRequestException(message);
			...
			rmContext
					.getDispatcher()
					.getEventHandler()
					.handle(new RMAppAttemptUnregistrationEvent(
							applicationAttemptId, request.getTrackingUrl(),
							request.getFinalApplicationStatus(), request
									.getDiagnostics()));

			// For UnmanagedAMs, return true so they don't retry
			return FinishApplicationMasterResponse.newInstance(rmApp
	...
	public AllocateResponse allocate(AllocateRequest request)
		...
		synchronized (lock) {
			...
			if (!hasApplicationMasterRegistered(appAttemptId)) {
				String message = "Application Master is trying to allocate before registering for: "
			...
			if ((request.getResponseId() + 1) == lastResponse.getResponseId()) {
				/* old heartbeat */
				return lastResponse;
			} else if (request.getResponseId() + 1 < lastResponse
					.getResponseId()) {
				LOG.error("Invalid responseid from appAttemptId "
						+ appAttemptId);
				// Oh damn! Sending reboot isn't enough. RM state is corrupted.
				// TODO:
				// Reboot is not useful since after AM reboots, it will send
				// register
				// and
			...
			this.rmContext
					.getDispatcher()
					.getEventHandler()
					.handle(new RMAppAttemptStatusupdateEvent(appAttemptId,
							request.getProgress()));

			List<ResourceRequest> ask = request.getAskList();
			List<ContainerId> release = request.getReleaseList();

			ResourceBlacklistRequest blacklistRequest = request
					.getResourceBlacklistRequest();
			List<String> blacklistAdditions = (blacklistRequest != null) ? blacklistRequest
					.getBlacklistAdditions() : Collections.EMPTY_LIST;
			List<String> blacklistRemovals = (blacklistRequest != null) ? blacklistRequest
					.getBlacklistRemovals() : Collections.EMPTY_LIST;
			...
			try {
				...
				LOG.warn("Invalid resource ask by application " + appAttemptId,
						e);
				throw e;
			}
			...
			try {
				...
				LOG.warn("Invalid blacklist request by application "
						+ appAttemptId, e);
				throw e;
			}
			...
			if (!app.getApplicationSubmissionContext()
				...
				try {
					RMServerUtils.validateContainerReleaseRequest(release,
							appAttemptId);
				} catch (InvalidContainerReleaseException e) {
					LOG.warn("Invalid container release by application "
							+ appAttemptId, e);
					throw e;
				}
			}
			...
			Allocation allocation = this.rScheduler.allocate(appAttemptId, ask,
			...
			if (app.pullRMNodeUpdates(updatedNodes) > 0) {
				...
				for (RMNode rmNode : updatedNodes) {
					SchedulerNodeReport schedulerNodeReport = rScheduler
					...
					NodeReport report = BuilderUtils.newNodeReport(
							rmNode.getNodeID(), rmNode.getState(),
							rmNode.getHttpAddress(), rmNode.getRackName(),
							used, rmNode.getTotalCapability(), numContainers,
			...
			allocateResponse.setAvailableResources(allocation
					.getResourceLimit());

			allocateResponse.setNumClusterNodes(this.rScheduler
					.getNumClusterNodes());
			...
			/*
			 * As we are updating the response inside the lock object so we
			 * don't need to worry about unregister call occurring in between
			 * (which removes the lock object).
			 */
	...
	private PreemptionMessage generatePreemptionMessage(Allocation allocation) {
		...
		if (allocation.getStrictContainerPreemptions() != null) {
			...
			StrictPreemptionContract pStrict = recordFactory
			...
			for (ContainerId cId : allocation.getStrictContainerPreemptions()) {
				PreemptionContainer pc = recordFactory
		...
		if (allocation.getResourcePreemptions() != null
			...
			PreemptionContract contract = recordFactory
			...
			for (ResourceRequest crr : allocation.getResourcePreemptions()) {
				PreemptionResourceRequest prr = recordFactory
						.newRecordInstance(PreemptionResourceRequest.class);
	...
	public void registerAppAttempt(ApplicationAttemptId attemptId) {
		...
		rmContext.getNMTokenSecretManager().registerApplicationAttempt(
				attemptId);
	}
	...
	public void unregisterAttempt(ApplicationAttemptId attemptId) {
		...
		rmContext.getNMTokenSecretManager().unregisterApplicationAttempt(
				attemptId);
	}
