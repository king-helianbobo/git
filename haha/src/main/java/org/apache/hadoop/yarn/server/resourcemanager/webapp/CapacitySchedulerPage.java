...
class CapacitySchedulerPage extends RmView {
		...
		private String getPercentage(ResourceInfo numerator,
			...
			if (denominator.getMemory() != 0) {
				percentString.append(
						" (<span title='of used resources in this queue'>")
						.append(StringUtils.format(
								"%.2f",
								numerator.getMemory() * 100.0
		...
		protected void render(Block html) {
			...
			for (UserInfo entry : users) {
				activeUserList
						...
						.append(" &lt;")
						.append(getPercentage(entry.getResourcesUsed(),
								usedResources))
						.append(", Schedulable Apps: "
								+ entry.getNumActiveApplications())
						.append(", Non-Schedulable Apps: "
								+ entry.getNumPendingApplications())
						.append("&gt;<br style='display:block'>"); // Force line
																	// break
			}

			ResponseInfo ri = info(
					"\'" + lqinfo.getQueuePath().substring(5)
							+ "\' Queue Status")
					._("Queue State:", lqinfo.getQueueState())
					._("Used Capacity:",
							percent(lqinfo.getUsedCapacity() / 100))
					...
					._("Used Resources:",
							StringEscapeUtils.escapeHtml(lqinfo
									.getResourcesUsed().toString()))
					...
					._("Max Schedulable Applications Per User:",
							Integer.toString(lqinfo
									.getMaxActiveApplicationsPerUser()))
					._("Configured Capacity:",
							percent(lqinfo.getCapacity() / 100))
			...
			// clear the info contents so this queue's info doesn't accumulate
			// into another queue's info
			ri.clear();
		}
	}
	...
	public static class QueueBlock extends HtmlBlock {
		...
		public void render(Block html) {
			ArrayList<CapacitySchedulerQueueInfo> subQueues = (csqinfo.qinfo == null) ? csqinfo.csinfo
					.getQueues().getQueueInfoList() : csqinfo.qinfo.getQueues()
					.getQueueInfoList();
			...
			for (CapacitySchedulerQueueInfo info : subQueues) {
				...
				LI<UL<Hamlet>> li = ul
						.li()
						.a(_Q)
						.$style(width(absMaxCap * Q_MAX_WIDTH))
						.$title(join("Absolute Capacity:", percent(absCap)))
						.span()
						.$style(join(Q_GIVEN, ";font-size:1px;", width(absCap
								/ absMaxCap)))
						._('.')
						._()
						.span()
						.$style(join(width(absUsedCap / absMaxCap),
								";font-size:1px;left:0%;",
								absUsedCap > absCap ? Q_OVER : Q_UNDER))._('.')
						._().span(".q", info.getQueuePath().substring(5))._()
						.span().$class("qstats").$style(left(Q_STATS_POS))
						._(join(percent(used), "已使用"))._();// used
	...
	static class QueuesBlock extends HtmlBlock {
		...
		public void render(Block html) {
			...
			UL<DIV<DIV<Hamlet>>> ul = html.div("#cs-wrapper.ui-widget")
					.div(".ui-widget-header.ui-corner-top")
					._("Application Queues")._()
					.div("#cs.ui-widget-content.ui-corner-bottom").ul();
			if (cs == null) {
				ul.li().a(_Q).$style(width(Q_MAX_WIDTH)).span().$style(Q_END)
						._("100% ")._().span(".q", "default")._()._();
			} else {
				...
				ul.li()
						...
						.// Used (over capacity)
						span()
						.$class("qlegend ui-corner-all ui-state-default")
						._("最大容量")
						._()
						.// Max Capacity
						_()
						.li()
						.a(_Q)
						.$style(width(Q_MAX_WIDTH))
						.span()
						.$style(join(width(used), ";left:0%;",
								used > 1 ? Q_OVER : Q_UNDER))._(".")._()
						.span(".q", "root")._().span().$class("qstats")
						.$style(left(Q_STATS_POS))
						._(join(percent(used), "已使用"))._().// used
						_(QueueBlock.class)._();
			}
			ul._()._().script().$type("text/javascript")._("$('#cs').hide();")
