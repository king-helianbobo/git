...
public abstract class WebApp extends ServletModule {
	...
	static final Splitter pathSplitter = Splitter.on('/').trimResults()
			.omitEmptyStrings();
	...
	public String[] getServePathSpecs() {
		return this.servePathSpecs.toArray(new String[this.servePathSpecs
				.size()]);
	}

	/**
	 * Set a path to redirect the user to if they just go to "/". For instance
	 * "/" goes to "/yarn/apps". This allows the filters to more easily
	 * differentiate the different webapps.
	 * 
	 * @param path
	...
	protected void configureWebAppServlets() {
		...
		if (this.wsName != null) {
			...
			// through the guice container. In this case its everything except
	...
	/**
	 * /controller/action/:args => [/controller/action, controller, action,
	 * args] /controller/:args => [/controller, controller, index, args]
	 */
