...
public class FiCaSchedulerApp extends SchedulerApplicationAttempt {
	...
	synchronized public RMContainer allocate(NodeType type,
		...
		// Inform the container
		rmContainer.handle(new RMContainerEvent(container.getId(),
	...
	public synchronized boolean unreserve(FiCaSchedulerNode node,
		...
		if (reservedContainers != null) {
			RMContainer reservedContainer = reservedContainers.remove(node
					.getNodeID());

			// unreserve is now triggered in new scenarios (preemption)
			// as a consequence reservedcontainer might be null, adding
			// NP-checks
			if (reservedContainer != null
				...
				Resource resource = reservedContainer.getContainer()
						.getResource();
				Resources.subtractFrom(currentReservation, resource);

				LOG.info("Application " + getApplicationId() + " unreserved "
						+ " on node " + node + ", currently has "
						+ reservedContainers.size() + " at priority "
						+ priority + "; currentReservation "
						+ currentReservation);
				return true;
			}
		}
	...
	public synchronized float getLocalityWaitFactor(Priority priority,
		...
		int requiredResources = Math.max(this.getResourceRequests(priority)
				.size() - 1, 0);
	...
	public synchronized Resource getTotalPendingRequests() {
		...
		for (ResourceRequest rr : appSchedulingInfo.getAllResourceRequests()) {
			...
			if (ResourceRequest.isAnyLocation(rr.getResourceName())) {
				Resources.addTo(
						ret,
						Resources.multiply(rr.getCapability(),
								rr.getNumContainers()));
			}
		}
		return ret;
	...
	/**
	 * This method produces an Allocation that includes the current view of the
	 * resources that will be allocated to and preempted from this application.
	 * 
	 * @param rc
	...
	public synchronized Allocation getAllocation(ResourceCalculator rc,
		...
		int numCont = (int) Math.ceil(Resources.divide(rc, clusterResource,
				tot, minimumAllocation));
		...
		return new Allocation(allocation.getContainerList(), getHeadroom(),
				null, currentContPreemption, Collections.singletonList(rr),
