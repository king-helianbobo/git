...
public class NMTokenSecretManagerInRM extends BaseNMTokenSecretManager {
	...
	public NMTokenSecretManagerInRM(Configuration conf) {
		...
		// Add an activation delay. This is to address the following race: RM
		// may
		...
		// might not have come again to RM to update the shared secret: so AM
		// has a
		// valid password generated off new secret but NM doesn't know about the
		// secret yet.
		// Adding delay = 1.5 * expiry interval makes sure that all active NMs
		// get
		// the updated shared-key.
		this.activationDelay = (long) (conf.getLong(
	...
	public void rollMasterKey() {
		...
		try {
			...
			if (this.currentMasterKey == null) { // Setting up for the first
													// time.
				...
				this.timer.schedule(new NextKeyActivator(),
	...
	public void clearNodeSetForAttempt(ApplicationAttemptId attemptId) {
		super.writeLock.lock();
		try {
			HashSet<NodeId> nodeSet = this.appAttemptToNodeKeyMap
					.get(attemptId);
			if (nodeSet != null) {
	...
	private void clearApplicationNMTokenKeys() {
		...
		Iterator<HashSet<NodeId>> nodeSetI = this.appAttemptToNodeKeyMap
				.values().iterator();
	...
	private class NextKeyActivator extends TimerTask {
		@Override
		public void run() {
			// Activation will happen after an absolute time interval. It will
			// be good
			// if we can force activation after an NM updates and acknowledges a
			// roll-over. But that is only possible when we move to per-NM keys.
			// TODO:
		...
		try {
			...
			if (nodeSet != null) {
				if (!nodeSet.contains(container.getNodeId())) {
					LOG.info("Sending NMToken for nodeId : "
							+ container.getNodeId() + " for container : "
							+ container.getId());
					Token token = createNMToken(container.getId()
							.getApplicationAttemptId(), container.getNodeId(),
	...
	public void registerApplicationAttempt(ApplicationAttemptId appAttemptId) {
		try {
			this.writeLock.lock();
			this.appAttemptToNodeKeyMap
					.put(appAttemptId, new HashSet<NodeId>());
		} finally {
	...
	public boolean isApplicationAttemptNMTokenPresent(
		...
		try {
			this.readLock.lock();
			HashSet<NodeId> nodes = this.appAttemptToNodeKeyMap
					.get(appAttemptId);
	...
	public void removeNodeKey(NodeId nodeId) {
		try {
			this.writeLock.lock();
			Iterator<HashSet<NodeId>> appNodeKeySetIterator = this.appAttemptToNodeKeyMap
					.values().iterator();
