...
public class RMSecretManagerService extends AbstractService {
	...
	public RMSecretManagerService(Configuration conf, RMContextImpl rmContext) {
		...
		rmDTSecretManager = createRMDelegationTokenSecretManager(conf,
				rmContext);
	...
	public void serviceStart() throws Exception {
		...
		try {
			...
			throw new YarnRuntimeException(
					"Failed to start secret manager threads", ie);
		}
		super.serviceStart();
	}
	...
	protected RMDelegationTokenSecretManager createRMDelegationTokenSecretManager(
		...
		long secretKeyInterval = conf.getLong(
				YarnConfiguration.DELEGATION_KEY_UPDATE_INTERVAL_KEY,
		...
		long tokenRenewInterval = conf.getLong(
				YarnConfiguration.DELEGATION_TOKEN_RENEW_INTERVAL_KEY,
