...
public class ZKRMStateStore extends RMStateStore {
	...
	/**
	 * 
	 * ROOT_DIR_PATH |--- VERSION_INFO |--- RM_ZK_FENCING_LOCK |--- RM_APP_ROOT
	 * | |----- (#ApplicationId1) | | |----- (#ApplicationAttemptIds) | | |
	 * |----- (#ApplicationId2) | | |----- (#ApplicationAttemptIds) | .... |
	 * |--- RM_DT_SECRET_MANAGER_ROOT |----- RM_DT_SEQUENTIAL_NUMBER_ZNODE_NAME
	 * |----- RM_DELEGATION_TOKENS_ROOT_ZNODE_NAME | |----- Token_1 | |-----
	 * Token_2 | .... | |----- RM_DT_MASTER_KEYS_ROOT_ZNODE_NAME | |----- Key_1
	 * | |----- Key_2 ....
	 * 
	 */
	...
	public static final int CREATE_DELETE_PERMS = ZooDefs.Perms.CREATE
			| ZooDefs.Perms.DELETE;
	private final String zkRootNodeAuthScheme = new DigestAuthenticationProvider()
			.getScheme();
	 ...
	 * 
	 * To be called only when HA is enabled and the configuration doesn't set
	...
	protected List<ACL> constructZkRootNodeACL(Configuration conf,
		...
		for (ACL acl : sourceACLs) {
			zkRootNodeAcl.add(new ACL(ZKUtil.removeSpecificPerms(
					acl.getPerms(), CREATE_DELETE_PERMS), acl.getId()));
		}
	...
	public synchronized void initInternal(Configuration conf) throws Exception {
		...
		numRetries = conf.getInt(YarnConfiguration.RM_ZK_NUM_RETRIES,
				YarnConfiguration.DEFAULT_ZK_RM_NUM_RETRIES);
		znodeWorkingPath = conf.get(
		...
		createFencingNodePathOp = Op.create(fencingNodePath, new byte[0],
		...
		if (HAUtil.isHAEnabled(conf)) {
			...
			if (zkRootNodeAclConf != null) {
				zkRootNodeAclConf = ZKUtil
						.resolveConfIndirection(zkRootNodeAclConf);
				try {
		...
		rmDTSecretManagerRoot = getNodePath(zkRootNodePath,
	...
	private void createRootDir(final String rootPath) throws Exception {
		...
		try {
			new ZKAction<String>() {
				@Override
				public String run() throws KeeperException,
						InterruptedException {
					return zkClient.create(rootPath, null, zkAcl,
	...
	private synchronized void fence() throws Exception {
		...
		new ZKAction<Void>() {
			...
			public Void run() throws KeeperException, InterruptedException {
				try {
					zkClient.multi(Collections
							.singletonList(deleteFencingNodePathOp));
	...
	protected synchronized void storeVersion() throws Exception {
		...
		if (zkClient.exists(versionNodePath, true) != null) {
			...
			createWithRetries(versionNodePath, data, zkAcl,
					CreateMode.PERSISTENT);
		}
	}
	...
	protected synchronized RMStateVersion loadVersion() throws Exception {
		...
		if (zkClient.exists(versionNodePath, true) != null) {
			...
			RMStateVersion version = new RMStateVersionPBImpl(
					RMStateVersionProto.parseFrom(data));
			return version;
		}
		return null;
	}
	...
	private void loadRMDelegationKeyState(RMState rmState) throws Exception {
		List<String> childNodes = getChildrenWithRetries(dtMasterKeysRootPath,
				true);
		for (String childNodeName : childNodes) {
			String childNodePath = getNodePath(dtMasterKeysRootPath,
					childNodeName);
	...
	private void loadRMDelegationTokenState(RMState rmState) throws Exception {
		List<String> childNodes = zkClient.getChildren(
				delegationTokensRootPath, true);
		for (String childNodeName : childNodes) {
			String childNodePath = getNodePath(delegationTokensRootPath,
					childNodeName);
			byte[] childData = getDataWithRetries(childNodePath, true);
			...
			try {
				if (childNodeName.startsWith(DELEGATION_TOKEN_PREFIX)) {
					...
					rmState.rmSecretManagerState.delegationTokenState.put(
		...
		for (String childNodeName : childNodes) {
			...
			if (childNodeName.startsWith(ApplicationId.appIdStrPrefix)) {
				...
				if (LOG.isDebugEnabled()) {
					LOG.debug("Loading application from znode: "
							+ childNodeName);
				}
				...
				ApplicationState appState = new ApplicationState(
						...
						appStateData.getDiagnostics(),
						appStateData.getFinishTime());
				if (!appId.equals(appState.context.getApplicationId())) {
					throw new YarnRuntimeException(
							"The child node name is different "
		...
		for (String attemptIDStr : attempts) {
			if (attemptIDStr
				...
				ApplicationAttemptState attemptState = new ApplicationAttemptState(
						...
						credentials, attemptStateData.getStartTime(),
				...
				appState.attempts
						.put(attemptState.getAttemptId(), attemptState);
			}
		}
	...
	public synchronized void storeApplicationStateInternal(ApplicationId appId,
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Storing info for app: " + appId + " at: "
					+ nodeCreatePath);
		}
	...
	public synchronized void updateApplicationStateInternal(
			ApplicationId appId, ApplicationStateDataPBImpl appStateDataPB)
			throws Exception {
	...
	public synchronized void storeApplicationAttemptStateInternal(
			...
			throws Exception {
		String appDirPath = getNodePath(rmAppRoot, appAttemptId
	...
	public synchronized void updateApplicationAttemptStateInternal(
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Storing final state info for attempt: "
					+ appAttemptIdStr + " at: " + nodeUpdatePath);
		}
		byte[] attemptStateData = attemptStateDataPB.getProto().toByteArray();
	...
	public synchronized void removeApplicationStateInternal(
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Removing info for app: " + appId + " at: "
	...
	protected synchronized void storeRMDelegationTokenAndSequenceNumberState(
		...
		ArrayList<Op> opList = new ArrayList<Op>();
		addStoreOrUpdateOps(opList, rmDTIdentifier, renewDate,
	...
	protected synchronized void removeRMDelegationTokenState(
		...
		ArrayList<Op> opList = new ArrayList<Op>();
		String nodeRemovePath = getNodePath(delegationTokensRootPath,
		...
		if (zkClient.exists(nodeRemovePath, true) != null) {
			...
			LOG.info("Attempted to delete a non-existing znode "
					+ nodeRemovePath);
		}
		doMultiWithRetries(opList);
	}
	...
	protected void updateRMDelegationTokenAndSequenceNumberInternal(
		...
		if (zkClient.exists(nodeRemovePath, true) == null) {
			...
			addStoreOrUpdateOps(opList, rmDTIdentifier, renewDate,
					latestSequenceNumber, false);
			LOG.info("Attempted to update a non-existing znode "
					+ nodeRemovePath);
		} else {
			// in case znode exists
			addStoreOrUpdateOps(opList, rmDTIdentifier, renewDate,
					latestSequenceNumber, true);
		}
		doMultiWithRetries(opList);
	}
	...
	private void addStoreOrUpdateOps(ArrayList<Op> opList,
		...
		try {
			...
			if (isUpdate) {
				...
				opList.add(Op.create(nodeCreatePath, tokenOs.toByteArray(),
						zkAcl, CreateMode.PERSISTENT));
			}
	...
	protected synchronized void removeRMDTMasterKeyState(
			DelegationKey delegationKey) throws Exception {
		String nodeRemovePath = getNodePath(dtMasterKeysRootPath,
				DELEGATION_KEY_PREFIX + delegationKey.getKeyId());
		if (LOG.isDebugEnabled()) {
	...
	public synchronized void processWatchEvent(WatchedEvent event)
		...
		LOG.info("Watcher event type: " + eventType + " with state:"
				+ event.getState() + " for path:" + event.getPath() + " for "
				+ this);
		...
		if (eventType == Event.EventType.None) {
			...
			case SyncConnected:
				...
				if (oldZkClient != null) {
					// the SyncConnected must be from the client that sent
					// Disconnected
	 ...
	 */
	private synchronized void doMultiWithRetries(final List<Op> opList)
			throws Exception {
	...
	@Private
	@Unstable
	public void createWithRetries(final String path, final byte[] data,
	...
	private List<String> getChildrenWithRetries(final String path,
			final boolean watch) throws Exception {
		...
		T runWithRetries() throws Exception {
			...
			while (true) {
				try {
					...
					if (HAUtil.isHAEnabled(getConfig())) {
						// NoAuthException possibly means that this store is
						// fenced due to
