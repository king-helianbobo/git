...
/**
 * AMRM-tokens are per ApplicationAttempt. If users redistribute their tokens,
 * it is their headache, god save them. I mean you are not supposed to
...
public class AMRMTokenSecretManager extends SecretManager<AMRMTokenIdentifier> {
	...
	public void start() {
		this.timer.scheduleAtFixedRate(new MasterKeyRoller(), 0,
				rollingInterval);
	}
	...
	public synchronized void applicationMasterFinished(
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Application finished, removing password for "
					+ appAttemptId);
		}
	...
	/**
	 * Create a password for a given {@link AMRMTokenIdentifier}. Used to send
	 * to the AppicationAttempt which can give it back during authentication.
	 */
	@Override
	public synchronized byte[] createPassword(AMRMTokenIdentifier identifier) {
		ApplicationAttemptId applicationAttemptId = identifier
