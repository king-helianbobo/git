...
public class NodesListManager extends AbstractService implements
	...
	public void refreshNodes(Configuration yarnConf) throws IOException,
		...
		synchronized (hostsReader) {
			...
			includesFile = yarnConf.get(
					YarnConfiguration.RM_NODES_INCLUDE_FILE_PATH,
					YarnConfiguration.DEFAULT_RM_NODES_INCLUDE_FILE_PATH);
			excludesFile = yarnConf.get(
					YarnConfiguration.RM_NODES_EXCLUDE_FILE_PATH,
					YarnConfiguration.DEFAULT_RM_NODES_EXCLUDE_FILE_PATH);
			...
			hostsReader.refresh(
					includesFile.isEmpty() ? null : this.rmContext
							...
							.getConfigurationInputStream(this.conf,
									includesFile),
					excludesFile.isEmpty() ? null : this.rmContext
							.getConfigurationProvider()
							.getConfigurationInputStream(this.conf,
									excludesFile));
			setDecomissionedNMsMetrics();
	...
	public boolean isValidNode(String hostName) {
		synchronized (hostsReader) {
			...
			return (hostsList.isEmpty() || hostsList.contains(hostName) || hostsList
					.contains(ip))
					&& !(excludeList.contains(hostName) || excludeList
							.contains(ip));
		}
	}
		...
		case NODE_UNUSABLE:
			...
			for (RMApp app : rmContext.getRMApps().values()) {
				this.rmContext
						.getDispatcher()
						.getEventHandler()
						.handle(new RMAppNodeUpdateEvent(
								app.getApplicationId(), eventNode,
	...
	private void disableHostsFileReader(Exception ex) {
		...
		try {
			this.includesFile = conf
					.get(YarnConfiguration.DEFAULT_RM_NODES_INCLUDE_FILE_PATH);
			this.excludesFile = conf
					.get(YarnConfiguration.DEFAULT_RM_NODES_EXCLUDE_FILE_PATH);
			this.hostsReader = createHostsFileReader(this.includesFile,
					this.excludesFile);
			setDecomissionedNMsMetrics();
