...
public class AdminService extends CompositeService implements
	...
	private void throwStandbyException() throws StandbyException {
		throw new StandbyException("ResourceManager " + rmId
				+ " is not Active!");
	}
	...
	public synchronized void transitionToActive(
		...
		try {
			...
			throw new ServiceFailedException(
					"Can not execute refreshAdminAcls", ex);
		}
		...
		try {
			...
			// call all refresh*s for active RM to get the updated
			// configurations.
			refreshAll();
			...
			RMAuditLogger.logFailure(user.getShortUserName(),
					"transitionToActive", adminAcl.toString(),
	...
	public synchronized void transitionToStandby(
		...
		try {
			...
			RMAuditLogger
					.logFailure(user.getShortUserName(), "transitionToStandby",
	...
	public RefreshQueuesResponse refreshQueues(RefreshQueuesRequest request)
		...
		RefreshQueuesResponse response = recordFactory
				.newRecordInstance(RefreshQueuesResponse.class);
		try {
	...
	public RefreshSuperUserGroupsConfigurationResponse refreshSuperUserGroupsConfiguration(
		...
		if (!isRMActive()) {
			RMAuditLogger
					.logFailure(user.getShortUserName(), argName,
							adminAcl.toString(), "AdminService",
	...
	public RefreshServiceAclsResponse refreshServiceAcls(
		...
		if (!getConfig().getBoolean(
				...
				false)) {
			throw RPCUtil
					.getRemoteException(new IOException(
		...
		if (!isRMActive()) {
			RMAuditLogger
					.logFailure(UserGroupInformation.getCurrentUser()
							.getShortUserName(), argName, adminAcl.toString(),
							"AdminService",
		...
		rmContext.getApplicationMasterService().refreshServiceAcls(conf,
				policyProvider);
		rmContext.getResourceTrackerService().refreshServiceAcls(conf,
				policyProvider);

		return recordFactory
				.newRecordInstance(RefreshServiceAclsResponse.class);
	}
	...
	public UpdateNodeResourceResponse updateNodeResource(
			UpdateNodeResourceRequest request) throws YarnException,
			IOException {
		Map<NodeId, ResourceOption> nodeResourceMap = request
				.getNodeResourceMap();
		...
		for (NodeId nodeId : nodeIds) {
			...
			if (node == null) {
				...
				throw RPCUtil
						.getRemoteException("Resource update get failed on all nodes due to change resource "
		...
		for (Map.Entry<NodeId, ResourceOption> entry : nodeResourceMap
				.entrySet()) {
			ResourceOption newResourceOption = entry.getValue();
			NodeId nodeId = entry.getKey();
			RMNode node = this.rmContext.getRMNodes().get(nodeId);
			if (node == null) {
				LOG.warn("Resource update get failed on an unrecognized node: "
						+ nodeId);
			} else {
				node.setResourceOption(newResourceOption);
				LOG.info("Update resource successfully on node("
						+ node.getNodeID() + ") with resource("
						+ newResourceOption.toString() + ")");
			}
		}
		UpdateNodeResourceResponse response = recordFactory
				.newRecordInstance(UpdateNodeResourceResponse.class);
		return response;
	}

	private synchronized Configuration getConfiguration(Configuration conf,
			String confFileName) throws YarnException, IOException {
		InputStream confFileInputStream = this.rmContext
				.getConfigurationProvider().getConfigurationInputStream(conf,
						confFileName);
