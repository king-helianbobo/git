...
public class RMServerUtils {
	...
	public static List<RMNode> queryRMNodes(RMContext context,
		...
		// inactiveNodes contains nodes that are DECOMMISSIONED, LOST, OR
		// REBOOTED
		...
		if (blacklistRequest != null) {
			...
			if (plus != null && plus.contains(ResourceRequest.ANY)) {
				throw new InvalidResourceBlacklistRequestException(
						"Cannot add " + ResourceRequest.ANY
								+ " to the blacklist!");
			}
		}
	}
	...
	public static void validateContainerReleaseRequest(
			List<ContainerId> containerReleaseList,
	...
	public static UserGroupInformation verifyAccess(AccessControlList acl,
		...
		if (!acl.isUserAllowed(user)) {
			LOG.warn("User " + user.getShortUserName()
