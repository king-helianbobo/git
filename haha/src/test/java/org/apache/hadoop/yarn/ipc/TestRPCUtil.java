...
public class TestRPCUtil {
	...
	public void testUnexpectedRemoteExceptionUnwrapping() {
		...
		verifyRemoteExceptionUnwrapping(RemoteException.class,
				exception.getName());
	}

	@Test
	public void testRemoteYarnExceptionWithoutStringConstructor() {
		// Derivatives of YarnException should always define a string
		// constructor.
	...
	private void verifyRemoteExceptionUnwrapping(
		...
		RemoteException re = new RemoteException(realExceptionClassName,
				message);
		ServiceException se = new ServiceException(re);
		...
		Assert.assertTrue(
				"Expected message [" + message + "] but found "
						+ t.getMessage(), t.getMessage().contains(message));
	}
