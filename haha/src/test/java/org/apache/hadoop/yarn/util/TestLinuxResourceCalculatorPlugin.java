...
/**
 * A JUnit test to test {@link LinuxResourceCalculatorPlugin} Create the fake
 * /proc/ information and verify the parsing and calculation
 */
public class TestLinuxResourceCalculatorPlugin {
	...
	static class FakeLinuxResourceCalculatorPlugin extends
		...
		public FakeLinuxResourceCalculatorPlugin(String procfsMemFile,
			...
			super(procfsMemFile, procfsCpuFile, procfsStatFile,
					jiffyLengthInMillis);
		}
	...
	static {
		...
		FAKE_STATFILE = TEST_ROOT_DIR + File.separator + "STATINFO_"
				+ randomNum;
		plugin = new FakeLinuxResourceCalculatorPlugin(FAKE_MEMFILE,
				FAKE_CPUFILE, FAKE_STATFILE, FAKE_JIFFY_LENGTH);
	}
	...
	public void parsingProcStatAndCpuFile() throws IOException {
		...
		for (int i = 0; i < numProcessors; i++) {
			fileContent += String.format(CPUINFO_FORMAT, i,
					cpuFrequencyKHz / 1000D) + "\n";
		}
		...
		assertEquals(plugin.getCumulativeCpuTime(), FAKE_JIFFY_LENGTH
				* (uTime + nTime + sTime));
		...
		assertEquals(plugin.getCumulativeCpuTime(), FAKE_JIFFY_LENGTH
				* (uTime + nTime + sTime));
		assertEquals(plugin.getCpuUsage(), 25F, 0.0); // CPU usage is not
														// updated.
	}
