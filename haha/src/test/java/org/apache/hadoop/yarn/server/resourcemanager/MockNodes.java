...
public class MockNodes {
	...
	private static class MockRMNodeImpl implements RMNode {
		...
		public MockRMNodeImpl(NodeId nodeId, String nodeAddr,
				String httpAddress, ResourceOption perNode, String rackName,
				String healthReport, long lastHealthReportTime, int cmdPort,
				String hostName, NodeState state) {
			this.nodeId = nodeId;
	...
	private static RMNode buildRMNode(int rack, final Resource perNode,
		...
		String healthReport = (state == NodeState.UNHEALTHY) ? null
				: "HealthyMe";
		return new MockRMNodeImpl(nodeID, nodeAddr, httpAddress,
				ResourceOption.newInstance(perNode,
						RMNode.OVER_COMMIT_TIMEOUT_MILLIS_DEFAULT), rackName,
