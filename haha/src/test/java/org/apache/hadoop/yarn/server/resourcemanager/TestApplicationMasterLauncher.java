...
public class TestApplicationMasterLauncher {
	...
	private static final class MyContainerManagerImpl implements
		...
		public StartContainersResponse startContainers(
			...
			StartContainerRequest request = requests
					.getStartContainerRequests().get(0);
			LOG.info("Container started by MyContainerManager: " + request);
			launched = true;
			Map<String, String> env = request.getContainerLaunchContext()
					.getEnvironment();
			...
			attemptIdAtContainerManager = containerId.getApplicationAttemptId()
					.toString();
			nmHostAtContainerManager = tokenId.getNmHostAddress();
			submitTimeAtContainerManager = Long.parseLong(env
					.get(ApplicationConstants.APP_SUBMIT_TIME_ENV));
			maxAppAttempts = Integer.parseInt(env
					.get(ApplicationConstants.MAX_APP_ATTEMPTS_ENV));
	...
	public void testAMLaunchAndCleanup() throws Exception {
		...
		Assert.assertEquals(app.getRMAppAttempt(appAttemptId)
				.getMasterContainer().getId().toString(),
				containerManager.containerIdAtContainerManager);
		...
		MockAM am = new MockAM(rm.getRMContext(),
				rm.getApplicationMasterService(), appAttemptId);
	...
	public void testallocateBeforeAMRegistration() throws Exception {
		...
		int request = 2;
		try {
			AllocateResponse ar = am.allocate("h1", 1000, request,
					new ArrayList<ContainerId>());
		} catch (Exception e) {
			Assert.assertEquals(
					"Application Master is trying to allocate before "
							+ "registering for: "
		...
		nm1.nodeHeartbeat(true);
		try {
			AllocateResponse amrs = am.allocate(
