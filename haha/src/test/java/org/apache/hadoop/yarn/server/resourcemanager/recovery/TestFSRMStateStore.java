...
public class TestFSRMStateStore extends RMStateStoreTestBase {
	...
	class TestFSRMStateStoreTester implements RMStateStoreHelper {
		...
		public TestFSRMStateStoreTester(MiniDFSCluster cluster)
				throws Exception {
	...
	public void testFSRMStateStore() throws Exception {
		...
		try {
			...
			// If the state store is FileSystemRMStateStore then add corrupted
			// entry.
			...
			ApplicationAttemptId attemptId3 = ConverterUtils
					.toApplicationAttemptId(appAttemptIdStr3);
			Path appDir = fsTester.store.getAppDir(attemptId3
					.getApplicationId().toString());
			Path tempAppAttemptFile = new Path(appDir, attemptId3.toString()
					+ ".tmp");
	...
	protected void modifyAppState() throws Exception {
		...
		Path appDir = fsTester.store.getAppDir(attemptId1.getApplicationId()
				.toString());
		Path appAttemptFile1 = new Path(appDir, attemptId1.toString() + ".new");
		FileSystemRMStateStore fileSystemRMStateStore = (FileSystemRMStateStore) fsTester
				.getRMStateStore();
		fileSystemRMStateStore.renameFile(appAttemptFile1,
				new Path(appAttemptFile1.getParent(), appAttemptFile1.getName()
						+ ".new"));
	}
	...
	protected void modifyRMDelegationTokenState() throws Exception {
		...
		Path nodeCreatePath = fsTester.store.getNodePath(
				fsTester.store.rmDTSecretManagerRoot,
				FileSystemRMStateStore.DELEGATION_TOKEN_PREFIX + 0);
		FileSystemRMStateStore fileSystemRMStateStore = (FileSystemRMStateStore) fsTester
				.getRMStateStore();
		fileSystemRMStateStore.renameFile(nodeCreatePath, new Path(
				nodeCreatePath.getParent(), nodeCreatePath.getName() + ".new"));
	}
	...
	public void testFSRMStateStoreClientRetry() throws Exception {
		...
		cluster.waitActive();
		try {
			TestFSRMStateStoreTester fsTester = new TestFSRMStateStoreTester(
					cluster);
			final RMStateStore store = fsTester.getRMStateStore();
			store.setRMDispatcher(new TestDispatcher());
			final AtomicBoolean assertionFailedInThread = new AtomicBoolean(
					false);
					...
					try {
						store.storeApplicationStateInternal(
								...
								(ApplicationStateDataPBImpl) ApplicationStateDataPBImpl
										.newApplicationStateData(111, 111,
												"user", null,
												RMAppState.ACCEPTED,
												"diagnostics", 333));
					} catch (Exception e) {
						// TODO 0 datanode exception will not be retried by dfs
						// client, fix
						// that separately.
						if (!e.getMessage()
								.contains(
