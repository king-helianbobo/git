...
public class TestFSSchedulerApp {
	...
	public void testDelayScheduling() {
		...
		FSSchedulerApp schedulerApp = new FSSchedulerApp(applicationAttemptId,
				"user1", queue, null, null);
		...
		for (int i = 0; i < 5; i++) {
			...
			assertEquals(NodeType.NODE_LOCAL,
					schedulerApp.getAllowedLocalityLevel(prio, 10,
							nodeLocalityThreshold, rackLocalityThreshold));
		}
