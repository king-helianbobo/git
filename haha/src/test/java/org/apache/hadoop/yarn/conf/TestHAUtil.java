...
public class TestHAUtil {
	...
	public void setUp() {
		...
		for (String confKey : YarnConfiguration.getServiceAddressConfKeys(conf)) {
			...
			conf.set(HAUtil.addSuffix(confKey, RM1_NODE_ID),
	...
	public void testVerifyAndSetConfiguration() throws Exception {
		...
		try {
			...
			assertEquals(
					...
					HAUtil.BAD_CONFIG_MESSAGE_PREFIX
							+ HAUtil.getInvalidValueMessage(
									YarnConfiguration.RM_HA_IDS,
		...
		try {
			...
			String confKey = HAUtil.addSuffix(YarnConfiguration.RM_ADDRESS,
					RM1_NODE_ID);
			assertEquals(
					"YarnRuntimeException by Configuration#set()",
					HAUtil.BAD_CONFIG_MESSAGE_PREFIX
							+ HAUtil.getNeedToSetValueMessage(HAUtil.addSuffix(
									YarnConfiguration.RM_HOSTNAME, RM1_NODE_ID)
		...
		try {
			...
			assertEquals(
					...
					HAUtil.BAD_CONFIG_MESSAGE_PREFIX
							+ HAUtil.getRMHAIdNeedToBeIncludedMessage(
									"[rm2, rm3]", RM1_NODE_ID), e.getMessage());
		}
	}

	@Test
	public void testGetConfKeyForRMInstance() {
		assertTrue("RM instance id is not suffixed", HAUtil
				.getConfKeyForRMInstance(YarnConfiguration.RM_ADDRESS, conf)
				.contains(HAUtil.getRMHAId(conf)));
		assertFalse("RM instance id is suffixed", HAUtil
				.getConfKeyForRMInstance(YarnConfiguration.NM_ADDRESS, conf)
				.contains(HAUtil.getRMHAId(conf)));
	}
}
