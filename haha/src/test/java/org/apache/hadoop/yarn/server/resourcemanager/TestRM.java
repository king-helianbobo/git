...
public class TestRM {
	...
	// Test even if AM container is allocated with containerId not equal to 1,
	// the
	...
	public void testNMTokenSentForNormalContainer() throws Exception {
		...
		// Call getNewContainerId to increase container Id so that the AM
		// container
		...
		while (true) {
			AllocateResponse response = am.allocate("127.0.0.1", 2000,
	...
	public void testNMToken() throws Exception {
		...
		try {
			...
			AllocateResponse response = am.allocate("h1", 1000, 2,
					releaseContainerList);
			Assert.assertEquals(0, response.getAllocatedContainers().size());
			allocateContainersAndValidateNMTokens(am, containersReceivedForNM1,
					2, nmTokens, nm1);
			...
			// We will be simulating NM restart so restarting newly added
			// h2:1234
			// NM 2 now registers.
			...
			Assert.assertEquals(0, response.getAllocatedContainers().size());
			allocateContainersAndValidateNMTokens(am, containersReceivedForNM2,
					2, nmTokens, nm2);
			Assert.assertEquals(2, nmTokens.size());

			// Simulating NM-2 restart.
			nm2 = rm.registerNode("h2:1234", 10000);
			// Wait for reconnect to make it through the RM and create a new
			// RMNode
			...
			while (nmTokenSecretManager.isApplicationAttemptNMTokenPresent(
					attempt.getAppAttemptId(), nm2.getNodeId())
					&& interval-- > 0) {
				LOG.info("waiting for nmToken to be cleared for : "
						+ nm2.getNodeId());
				Thread.sleep(WAIT_SLEEP_MS);
			}
			Assert.assertTrue(nmTokenSecretManager
			...
			// Now rolling over NMToken masterKey. it should resend the NMToken
			// in
			// next allocate call.
			Assert.assertTrue(nmTokenSecretManager
			...
			for (Container container : containersReceivedForNM1) {
				nm1.nodeHeartbeat(attempt.getAppAttemptId(), container.getId()
						.getId(), ContainerState.COMPLETE);
			}
			for (Container container : containersReceivedForNM2) {
				nm2.nodeHeartbeat(attempt.getAppAttemptId(), container.getId()
						.getId(), ContainerState.COMPLETE);
			}
	...
	protected void allocateContainersAndValidateNMTokens(MockAM am,
			ArrayList<Container> containersReceived,
			int totalContainerRequested, HashMap<String, Token> nmTokens,
			MockNM nm) throws Exception, InterruptedException {
			...
			if (!response.getNMTokens().isEmpty()) {
				for (NMToken nmToken : response.getNMTokens()) {
					...
					if (nmTokens.containsKey(nodeId)) {
						Assert.fail("Duplicate NMToken received for : "
								+ nodeId);
					}
	...
	public void testActivatingApplicationAfterAddingNM() throws Exception {
		...
		// app1 and app2 should be scheduled, but because no resource is
		// available,
		...
		MockNM nm1 = new MockNM("h1:1234", 15120,
				rm1.getResourceTrackerService());
		MockNM nm2 = new MockNM("h2:5678", 15120,
				rm1.getResourceTrackerService());
		nm1.registerNode();
		nm2.registerNode();
	...
	public void testInvalidateAMHostPortWhenAMFailedOrKilled() throws Exception {
		...
		rm1.waitForState(am3.getApplicationAttemptId(),
	...
	public void testInvalidatedAMHostPortOnAMRestart() throws Exception {
		...
		ApplicationReport report1 = rm1.getClientRMService()
				.getApplicationReport(request1).getApplicationReport();
	...
	public void testApplicationKillAtAcceptedState() throws Exception {
		...
		final Dispatcher dispatcher = new AsyncDispatcher() {
			...
			public EventHandler getEventHandler() {
				...
				doNothing().when(handler)
						.handle(argThat(new EventArgMatcher()));
				return handler;
			}
		};
		...
		// Now kill the application before new attempt is launched, the app
		// report
		// returns the invalid AM host and port.
		KillApplicationRequest request = KillApplicationRequest
				.newInstance(application.getApplicationId());
		rm.getClientRMService().forceKillApplication(request);

		// Specific test for YARN-1689 follows
		// Now let's say a race causes AM to register now. This should not crash
		// RM.
		am.registerAppAttempt(false);

		// We explicitly intercepted the kill-event to RMAppAttempt, so app
		// should
		...
		rm.getRMContext()
				.getDispatcher()
				.getEventHandler()
				.handle(new RMAppEvent(application.getApplicationId(),
