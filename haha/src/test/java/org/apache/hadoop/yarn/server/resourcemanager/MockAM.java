...
public class MockAM {
	...
	public void waitForState(RMAppAttemptState finalState) throws Exception {
		...
		System.out.println("AppAttempt State is : "
	...
	public RegisterApplicationMasterResponse registerAppAttempt(boolean wait)
		...
		UserGroupInformation ugi = UserGroupInformation
				.createRemoteUser(attemptId.toString());
		Token<AMRMTokenIdentifier> token = context.getRMApps()
				.get(attemptId.getApplicationId()).getRMAppAttempt(attemptId)
				.getAMRMToken();
	...
	public AllocateResponse allocate(String host, int memory,
			int numContainers, List<ContainerId> releases) throws Exception {
		List<ResourceRequest> reqs = createReq(new String[] { host }, memory,
				1, numContainers);
	...
	public List<ResourceRequest> createReq(String[] hosts, int memory,
		...
		for (String host : hosts) {
			ResourceRequest hostReq = createResourceReq(host, memory, priority,
					containers);
			reqs.add(hostReq);
			ResourceRequest rackReq = createResourceReq("/default-rack",
					memory, priority, containers);
			reqs.add(rackReq);
		}

		ResourceRequest offRackReq = createResourceReq(ResourceRequest.ANY,
				memory, priority, containers);
		reqs.add(offRackReq);
		return reqs;
	...
	public AllocateResponse allocate(List<ResourceRequest> resourceRequest,
		...
		final AllocateRequest req = AllocateRequest.newInstance(++responseId,
				0F, resourceRequest, releases, null);
		UserGroupInformation ugi = UserGroupInformation
				.createRemoteUser(attemptId.toString());
		Token<AMRMTokenIdentifier> token = context.getRMApps()
				.get(attemptId.getApplicationId()).getRMAppAttempt(attemptId)
