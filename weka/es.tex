...
\par ES插件本质上是一个zip文件,它包含了若干个Jar包,为安装插件,机器上必须安装Maven.因此,Maven工程的main目录里须包含三个目录(java,resources,assemblies),而且必须在配置文件pom.xml添加如下行:
\begin{verbatim}
...
\end{verbatim}
\par 最重要的是必须新建src/main/assemblies/plugin.xml文件,并添加如下语句:
\begin{verbatim}
<?xml version="1.0"?>
<assembly>
  <id>plugin</id>
