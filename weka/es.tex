...
\end{verbatim}
\par 告诉Maven插件为当前工程，然后在配置文件pom.xml的<build>/<plugins>/<plugin>/<artifactId>maven-assembly-plugin</artifactId>标签中添加如下内容，因为maven-assembly-plugin插件负责打包Maven工程:
\begin{verbatim}
<descriptors>
   <descriptor>
...
\end{verbatim}
\par 然后新建src/main/assembly/plugin.xml文件，写入如下语句：
\begin{verbatim}
<?xml version="1.0"?>
<assembly>
  <id>plugin</id>
