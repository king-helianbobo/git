...
\end{verbatim}
\subsubsection{ElasticSearch查询分析}
假如索引中有如下词组：[quick] [brown] [fox] [jump] [over] [lazy] [dog]，
\paragraph{Prefix Query}
当想搜索“qu”时，Prefix Query查询所有前缀为“qu”的Term，因此如果索引中拥有“quack”，“quote”和“quarter”等Term时，那么查询将匹配到这些Term，查询结果为\textbf{quick OR quack OR quote OR quarter}。如果索引中类似Term很多时，查询效率很差。而且Prefix Query不能匹配中间字符，如“ball”不能匹配“baseball”，虽然可以采用通配符的方式（Wildcard query），但伸缩能力有限。
\paragraph{N-Grams}
用户输入英文时能匹配到任意前缀，例如对[quick]建立索引，其Term应该为：[q]/[qu]/[qui]/[quic]/[quick]，这种Analyzer称为Edge NGram。还有一种Analyzer称为N-Grams，以“brown”这个单词为例，设置(minGram=1和maxGram=2)，N-Grams输出[b]/[r]/[o]/[w]/[n]/[br]/[ro]/[ow]/[wn]，而Edge NGram输出[b]/[br]，只有将maxGram设置为5，Edge NGram才能输出[b]/[br]/[bro]/[brow]/[brown]。具体使用哪种Analyzer，则依赖于具体场景，当用户输入own时，如果使用N-Grams，则能匹配到brown，而Edge NGram不能，针对document的不同域，应该设置不同的Analyzer，在网页document中，一般只对标题和关键词建立NGrams索引。例如定义了autocomplete这个analyzer，然后通过multi\_field在index的name域中添加了额外的autocomplete分析器。
\begin{verbatim}
{
  "settings":{
    "analysis":{
...
\end{verbatim}
\par 当搜索时，可以使用name来访问name域的默认版本，或者使用name.autocomplete来访问另外一个版本。
