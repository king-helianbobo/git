...
\end{verbatim}
\par 告诉Maven插件为当前工程，然后在配置文件pom.xml的<build>/<plugins>/<plugin>/<artifactId>maven-assembly-plugin</artifactId>标签中添加如下内容，为何在这个标签下添加内容,只是因为maven-assembly-plugin插件负责打包Maven工程,其中的plugin.xml为打包工程时额外执行的部分:
\begin{verbatim}
<descriptors>
   <descriptor>
...
\end{verbatim}
\par 新建src/main/assembly/plugin.xml文件，写入如下语句,告诉Maven对编译生成的classes目录下的所有内容打包：
\begin{verbatim}
<?xml version="1.0"?>
<assembly>
  <id>plugin</id>
...
\end{verbatim}
\par 正确的Maven工程目录树如下,做完这步后,调用命令:,将zip文件作为插件注入到ElasticSearch中.
\subsection{ES与hive}
