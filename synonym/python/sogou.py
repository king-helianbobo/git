...
def getPyTable(data):
    ...
    while pos < length:
        ...
        pos += 2
        pinyin_len = struct.unpack('H',data[pos]+data[pos+1])[0]
        pos += 2
        py = byte2str(data[pos:pos+pinyin_len])
        print py,pinyin_len,index
        GPy_Table[index]=py
        pos += pinyin_len
...
def getChinese(data):
    ...
    while pos < length:
        ...
        #print '[same]:',same,py,py_table_len
        #中文词组
        pos += py_table_len
...
if __name__ == '__main__':
    ...
    for count,py,word in GTable:
        ...
        #f.write(unicode('{%(count)s}' %{'count':count}+py+'\t'+ word))
        freq='%d' %count
        print freq,py,word
        f.write(unicode(word+'\t'+py+'\t'+ freq).encode('UTF-8'))
        f.write('\n')
    f.close()    
