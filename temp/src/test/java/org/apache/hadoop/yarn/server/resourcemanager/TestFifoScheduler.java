...
public class TestFifoScheduler {
	...
	public void test() throws Exception {
		...
		MockAM am1 = rm.sendAMLaunched(attempt1.getAppAttemptId());
		am1.registerAppAttempt();
		SchedulerNodeReport report_nm1 = rm.getResourceScheduler()
		...
		SchedulerNodeReport report_nm2 = rm.getResourceScheduler()
		...
		Assert.assertEquals(2 * GB, report_nm2.getAvailableResource()
				.getMemory());
	...
	private void testMinimumAllocation(YarnConfiguration conf, int testAlloc)
		...
		int checkAlloc = conf.getInt(
				YarnConfiguration.RM_SCHEDULER_MINIMUM_ALLOCATION_MB,
				YarnConfiguration.DEFAULT_RM_SCHEDULER_MINIMUM_ALLOCATION_MB);
		Assert.assertEquals(checkAlloc, report_nm1.getUsedResource()
				.getMemory());
	...
	public void testBlackListNodes() throws Exception {
		...
		RMNode n1 = MockNodes.newNodeInfo(rack_num_0,
				MockNodes.newResource(4 * GB), 1, host_0_0);
		fs.handle(new NodeAddedSchedulerEvent(n1));

		// host_0_1 in rack0
		String host_0_1 = "127.0.0.2";
		RMNode n2 = MockNodes.newNodeInfo(rack_num_0,
				MockNodes.newResource(4 * GB), 1, host_0_1);
		fs.handle(new NodeAddedSchedulerEvent(n2));

		// host_1_0 in rack1
		String host_1_0 = "127.0.0.3";
		RMNode n3 = MockNodes.newNodeInfo(rack_num_1,
				MockNodes.newResource(4 * GB), 1, host_1_0);
		fs.handle(new NodeAddedSchedulerEvent(n3));

		// host_1_1 in rack1
		String host_1_1 = "127.0.0.4";
		RMNode n4 = MockNodes.newNodeInfo(rack_num_1,
				MockNodes.newResource(4 * GB), 1, host_1_1);
		fs.handle(new NodeAddedSchedulerEvent(n4));

		// Add one application
		ApplicationId appId1 = BuilderUtils.newApplicationId(100, 1);
		ApplicationAttemptId appAttemptId1 = BuilderUtils
				.newApplicationAttemptId(appId1, 1);
		SchedulerEvent appEvent = new AppAddedSchedulerEvent(appId1, "queue",
				"user");
		fs.handle(appEvent);
		...
		Assert.assertEquals("allocation1", 0, allocation1.getContainers()
				.size());
		...
		Allocation allocation2 = fs.allocate(appAttemptId1, emptyAsk, emptyId,
				null, null);
		Assert.assertEquals("allocation2", 1, allocation2.getContainers()
				.size());
		...
		// this time, rack0 is also in blacklist, so only host_1_1 is available
		// to
		// be assigned
		...
		Allocation allocation3 = fs.allocate(appAttemptId1, emptyAsk, emptyId,
				null, null);
		Assert.assertEquals("allocation3", 0, allocation3.getContainers()
				.size());
		...
		Allocation allocation4 = fs.allocate(appAttemptId1, emptyAsk, emptyId,
				null, null);
		Assert.assertEquals("allocation4", 0, allocation4.getContainers()
				.size());
		...
		Allocation allocation5 = fs.allocate(appAttemptId1, emptyAsk, emptyId,
				null, null);
		Assert.assertEquals("allocation5", 0, allocation5.getContainers()
				.size());
	...
	public void testHeadroom() throws Exception {
		...
		ApplicationAttemptId appAttemptId2 = BuilderUtils
				.newApplicationAttemptId(appId2, 1);
		SchedulerEvent appEvent2 = new AppAddedSchedulerEvent(appId2, "queue",
				"user");
		fs.handle(appEvent2);
