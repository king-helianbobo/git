...
public class TestMoveApplication {
	...
	public void setUp() throws Exception {
		...
		conf.setClass(YarnConfiguration.RM_SCHEDULER,
				FifoSchedulerWithMove.class, FifoSchedulerWithMove.class);
	...
	public void testMoveTooLate() throws Exception {
		...
		clientRMService.forceKillApplication(KillApplicationRequest
				.newInstance(appId));
		...
		try {
			clientRMService
					.moveApplicationAcrossQueues(MoveApplicationAcrossQueuesRequest
							.newInstance(appId, "newqueue"));
			fail("Should have hit exception");
		} catch (YarnException ex) {
			assertEquals(YarnException.class, ex.getClass());
			assertEquals("App in KILLED state cannot be moved.",
					ex.getMessage());
		}
	}
	...
	public void testMoveRejectedByPermissions() throws Exception {
		...
		// Submit application
		final Application application = new Application("user1",
				resourceManager);
		application.submit();

		final ClientRMService clientRMService = resourceManager
				.getClientRMService();
		try {
			UserGroupInformation
					.createRemoteUser("otheruser")
					.doAs(new PrivilegedExceptionAction<MoveApplicationAcrossQueuesResponse>() {
						@Override
			...
			assertEquals(AccessControlException.class, ex.getCause().getCause()
					.getClass());
		}
	}
