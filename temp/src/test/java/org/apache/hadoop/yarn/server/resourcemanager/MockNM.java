...
public class MockNM {
	...
	public MockNM(String nodeIdStr, int memory, int vcores,
			ResourceTrackerService resourceTracker) {
		this(nodeIdStr, memory, vcores, resourceTracker, YarnVersionInfo
				.getVersion());
	}
	...
	public RegisterNodeManagerResponse registerNode(
		...
		RegisterNodeManagerRequest req = Records
				.newRecord(RegisterNodeManagerRequest.class);
		...
		RegisterNodeManagerResponse registrationResponse = resourceTracker
				.registerNodeManager(req);
		this.currentContainerTokenMasterKey = registrationResponse
				.getContainerTokenMasterKey();
		this.currentNMTokenMasterKey = registrationResponse
				.getNMTokenMasterKey();
	...
	public NodeHeartbeatResponse nodeHeartbeat(boolean isHealthy)
		...
		return nodeHeartbeat(
				new HashMap<ApplicationId, List<ContainerStatus>>(), isHealthy,
				++responseId);
	}
	...
	public NodeHeartbeatResponse nodeHeartbeat(ApplicationAttemptId attemptId,
		...
		HashMap<ApplicationId, List<ContainerStatus>> nodeUpdate = new HashMap<ApplicationId, List<ContainerStatus>>(
				1);
		ContainerStatus containerStatus = BuilderUtils.newContainerStatus(
				BuilderUtils.newContainerId(attemptId, containerId),
	...
	public NodeHeartbeatResponse nodeHeartbeat(
		...
		NodeHeartbeatRequest req = Records
				.newRecord(NodeHeartbeatRequest.class);
		...
		NodeHealthStatus healthStatus = Records
		...
		NodeHeartbeatResponse heartbeatResponse = resourceTracker
				.nodeHeartbeat(req);

		MasterKey masterKeyFromRM = heartbeatResponse
				.getContainerTokenMasterKey();
