...
public class TestAggregatedLogFormat {
	...
	public void testForCorruptedAggregatedLogs() throws Exception {
		...
		ContainerId testContainerId = TestContainerId
				.newContainerId(1, 1, 1, 1);
		Path t = new Path(srcFileRoot, testContainerId
	...
	private void writeSrcFileAndALog(Path srcFilePath, String fileName,
		...
		LogValue logValue = spy(new LogValue(
				Collections.singletonList(srcFileRoot.toString()),
	...
	// Verify the output generated by readAContainerLogs(DataInputStream,
	// Writer)
	@Test
	public void testReadAcontainerLogs1() throws Exception {
		...
		LogKey logKey = new LogKey(testContainerId);
		LogValue logValue = new LogValue(Collections.singletonList(srcFileRoot
				.toString()), testContainerId, ugi.getShortUserName());
		...
		Assert.assertTrue("Log Contents not matched",
				s.contains("Log Contents"));
	...
	public void testContainerLogsFileAccess() throws IOException {
		...
		// Creating files for container1. Log aggregator will try to read log
		// files
		// with illegal user.
		ApplicationId applicationId = ApplicationId.newInstance(1, 1);
		ApplicationAttemptId applicationAttemptId = ApplicationAttemptId
				.newInstance(applicationId, 1);
		ContainerId testContainerId1 = ContainerId.newInstance(
				applicationAttemptId, 1);
		Path appDir = new Path(srcFileRoot, testContainerId1
				.getApplicationAttemptId().getApplicationId().toString());
		...
		BufferedReader in = new BufferedReader(new FileReader(new File(
				remoteAppLogFile.toUri().getRawPath())));
		String line;
		...
		String stdoutFile1 = StringUtils.join(
				File.separator,
				Arrays.asList(new String[] {
						workDir.getAbsolutePath(),
						"srcFiles",
						testContainerId1.getApplicationAttemptId()
								.getApplicationId().toString(),
						testContainerId1.toString(), stderr }));
		String message1 = "Owner '" + expectedOwner + "' for path "
				+ stdoutFile1 + " did not match expected owner '" + randomUser
				+ "'";

		String stdoutFile2 = StringUtils.join(
				File.separator,
				Arrays.asList(new String[] {
						workDir.getAbsolutePath(),
						"srcFiles",
						testContainerId1.getApplicationAttemptId()
								.getApplicationId().toString(),
						testContainerId1.toString(), stdout }));
		String message2 = "Owner '" + expectedOwner + "' for path "
