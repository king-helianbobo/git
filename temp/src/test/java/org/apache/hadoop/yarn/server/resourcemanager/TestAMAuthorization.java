...
public class TestAMAuthorization {
	...
	public static final class MyContainerManager implements
		...
		public StartContainersResponse startContainers(
			...
			containerTokens = request.getStartContainerRequests().get(0)
					.getContainerLaunchContext().getTokens();
	...
	public void testAuthorizedAccess() throws Exception {
		...
		Assert.assertEquals("Register response has bad ACLs", "*", response
	...
	public void testUnauthorizedAccess() throws Exception {
		...
		ApplicationMasterProtocol client = currentUser
				.doAs(new PrivilegedAction<ApplicationMasterProtocol>() {
					...
					public ApplicationMasterProtocol run() {
						return (ApplicationMasterProtocol) rpc.getProxy(
								ApplicationMasterProtocol.class, serviceAddr,
								conf);
					}
				});
		...
		try {
			...
			// Because there are no tokens, the request should be rejected as
			// the
		...
		// TODO: Add validation of invalid authorization when there's more data
		// in
		// the AMRMToken
	}
