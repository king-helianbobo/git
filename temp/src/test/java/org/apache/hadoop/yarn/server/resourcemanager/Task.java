...
public class Task {
	...
	public enum State {
		PENDING, ALLOCATED, RUNNING, COMPLETE
	};
	...
	public Task(Application application, Priority priority, String[] hosts) {
		...
		LOG.info("Task " + taskId + " added to application "
				+ this.applicationId + " with " + this.hosts.size()
				+ " hosts, " + racks.size() + " racks");
	}
	...
	public void stop() {
		if (getState() != State.RUNNING) {
			throw new IllegalStateException(
					"Trying to stop a non-running task: " + getTaskId()
