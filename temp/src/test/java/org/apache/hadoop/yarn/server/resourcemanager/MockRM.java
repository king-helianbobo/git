...
public class MockRM extends ResourceManager {
	...
	public MockRM(Configuration conf, RMStateStore store) {
		super();
		init(conf instanceof YarnConfiguration ? conf : new YarnConfiguration(
				conf));
		if (store != null) {
	...
	public void waitForState(ApplicationId appId, RMAppState finalState)
		...
		while (!finalState.equals(app.getState()) && timeoutSecs++ < 40) {
			System.out.println("App : " + appId + " State is : "
					+ app.getState() + " Waiting for state : " + finalState);
			Thread.sleep(2000);
		}
	...
	public void waitForState(ApplicationAttemptId attemptId,
		...
		RMApp app = getRMContext().getRMApps()
		...
		while (!finalState.equals(attempt.getAppAttemptState())
				&& timeoutSecs++ < 40) {
			System.out.println("AppAttempt : " + attemptId + " State is : "
					+ attempt.getAppAttemptState() + " Waiting for state : "
					+ finalState);
			Thread.sleep(1000);
		}
		System.out
				.println("Attempt State is : " + attempt.getAppAttemptState());
		Assert.assertEquals("Attempt state is not correct (timedout)",
	...
	public void waitForState(MockNM nm, ContainerId containerId,
		...
		RMContainer container = getResourceScheduler().getRMContainer(
				containerId);
		int timeoutSecs = 0;
		while (container == null && timeoutSecs++ < 20) {
			nm.nodeHeartbeat(true);
			container = getResourceScheduler().getRMContainer(containerId);
			System.out.println("Waiting for container " + containerId
					+ " to be allocated.");
			Thread.sleep(100);
		}
	...
	public RMApp submitApp(int masterMemory, boolean unmanaged)
			throws Exception {
		return submitApp(masterMemory, "", UserGroupInformation
				.getCurrentUser().getShortUserName(), unmanaged);
	}
	...
	public RMApp submitApp(int masterMemory, String name, String user,
			Map<ApplicationAccessType, String> acls, boolean unmanaged,
			String queue, int maxAppAttempts, Credentials ts) throws Exception {
	...
	public RMApp submitApp(int masterMemory, String name, String user,
			...
			boolean isAppIdProvided, ApplicationId applicationId)
			throws Exception {
	...
	public MockNM registerNode(String nodeIdStr, int memory, int vCores)
			throws Exception {
		MockNM nm = new MockNM(nodeIdStr, memory, vCores,
	...
	public KillApplicationResponse killApp(ApplicationId appId)
			throws Exception {
		ApplicationClientProtocol client = getClientRMService();
	...
	public void sendAMLaunchFailed(ApplicationAttemptId appAttemptId)
		...
		getRMContext()
				.getDispatcher()
				.getEventHandler()
				.handle(new RMAppAttemptLaunchFailedEvent(appAttemptId,
						"Failed"));
	}
	...
	protected ResourceTrackerService createResourceTrackerService() {
		...
		RMContainerTokenSecretManager containerTokenSecretManager = getRMContext()
				.getContainerTokenSecretManager();
		containerTokenSecretManager.rollMasterKey();
		NMTokenSecretManagerInRM nmTokenSecretManager = getRMContext()
