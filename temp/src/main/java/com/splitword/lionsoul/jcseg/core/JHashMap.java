...
/**
 * Separate Chaining table - implementation the hash table the elements match is
 * based on the equals method of the Object (so better to rewrite the equals
 * method for the complex Object)
 * 
...
public class JHashMap {
	 ...
	 * 
	 * @param key
	 * @return if there is a mapping for the key return the value of the Entry
	 *         or return null
	 */
	 ...
	 * 
	 * @returns null if the HashMap contains no mapping for the key. else the
	 *          associated Entry
	 */
	...
	/**
	 * Associates the specified value with the specified key in the table if the
	 * map contains the mapping for the key, the old value is replaced
	 * 
	 * @param key
	 * @param val
	...
	void addEntry(String key, IWord val, int hash) {
		...
		if (size++ >= threshold)
			reHash();
		/*
		 * Attention: do not use the following condition check it will cause
		 * memory use up error. here use threshold instead
		 */
	 ...
	 * @param key
	 *            the item to remove
	 * @return if there is a mapping for the key return the old value else
	 *         return null
	 */
	...
	/**
	 * if table.length times fillFactor is larger than the size, we need to
