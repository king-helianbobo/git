...
import com.splitword.lionsoul.jcseg.core.CNNMFilter;
import com.splitword.lionsoul.jcseg.core.ENSCFilter;
import com.splitword.lionsoul.jcseg.core.IChunk;
import com.splitword.lionsoul.jcseg.core.ILexicon;
...
import com.splitword.lionsoul.jcseg.core.PairPunctuationFilter;
import com.splitword.lionsoul.jcseg.util.IStringBuffer;
import com.splitword.lionsoul.jcseg.util.IntArrayList;

/**
 * abstract segment class, implemented ISegment interface <br />
 * implements all the common method that simple segment and Complex segment
...
public abstract class ASegment implements ISegment {
	...
	/**
	 * read next char from current position
	 * 
	 * @throws IOException
	 */
	...
	public IWord next() throws IOException {
		if (wordPool.size() > 0) // if pool have word
			return wordPool.removeFirst();
		int c, pos;
		while ((c = readNext()) != -1) { // read next word in BufferReader
			if (ENSCFilter.isWhitespace(c))
				continue;
			pos = idx;
			if (isCJKChar(c)) { // if is CJK string
				char[] chars = nextCJKSentence(c); // 获得最大长度的汉字字符串
				int cjkidx = 0;
				IWord w = null;
				while (cjkidx < chars.length) {
					...
					if (CNNMFilter.convertCNToInteger(chars[cjkidx]) > -1
						...
						if ((ctrlMask & ISegment.CHECK_CF_MASK) != 0) { // 如果出现了'分之'
							w = new Word(numStr, IWord.T_CN_NUMERIC);
							...
							if (config.CNFRA_TO_ARABIC) {
								...
								.convertCNToInteger(chars[cjkidx + 1]) > -1
								|| dic.match(ILexicon.CJK_UNITS,
										chars[cjkidx + 1] + "")) {
							/*
							 * check the Chinese numeric and single units. type
							 * to find Chinese and unit composed word.
							 */
							StringBuilder sb = new StringBuilder();
							sb.append(numStr);
							boolean matched = false;
							// find the word that like '五四运动'
							for (int j = numStr.length(); (cjkidx + j) < chars.length
									&& j < config.MAX_LENGTH; j++) { // numStr后的字符串
								...
								if ((cjkidx + numStr.length()) < chars.length
										&& dic.match(ILexicon.CJK_UNITS,
												chars[cjkidx + numStr.length()]
														+ "")) {
									char units = chars[cjkidx + numStr.length()];
									numStr += units;
									arbic += units;
								}
							...
							if (dic.match(ILexicon.STOP_WORD, numStr)) {
								cjkidx += numStr.length();
								continue;
							}
							if (w == null) { // 将数字串提取出来
								w = new Word(numStr, IWord.T_CN_NUMERIC);
					...
					if (config.I_CN_NAME && w.getLength() <= 2
						...
						// the w is a Chinese last name，如果w是中国的姓
					...
					// check the stop words(clear it when Config.CLEAR_STOPWORD
					// is true)
					 ...
					 * @istep 4:
					 * 
					 * check and append the pinyin and the synonym words.
					 */
				...
				w.setPosition(pos);
				return w;
			} else if (ENSCFilter.isCnPunctuation(c)) { // Chinese punctuation.
				String str = ((char) c) + "";
				...
				/*
				 * @reader: (2013-09-25) unrecognized char will cause unknow
				 * problem for different system. keep it or clear it ? if you
				 * use jcseg for search, better shut it down.
				 */
				String str = ((char) c) + "";
				if (config.CLEAR_STOPWORD && dic.match(ILexicon.STOP_WORD, str))
					continue;
	...
	protected IWord[] getNextMatch(char[] chars, int index) {
		 ...
		 * just return the Word with a value of temp as an unrecognized word.
		 */
		if (mList.isEmpty()) {
	 ...
	 * until the char is not a CJK char.<br />
	 * 
	 * @param c
	 * @return char[]
	 * @throws IOException
	...
	protected char[] nextCJKSentence(int c) throws IOException {
		...
		isb.clear();
		int ch;
		isb.append((char) c); // store current character
		// reset the CE check mask.
		ctrlMask &= ~ISegment.CHECK_CE_MASk;
		while ((ch = readNext()) != -1) {
			if (ENSCFilter.isWhitespace(ch))
				break;
			if (!isCJKChar(ch)) { // if not CJK char
				pushBack(ch); // push back this char ,haha, rollback
	...
	protected IWord nextBasicLatin(int c) throws IOException {
		...
		boolean _check = false; // 是否出现了控制位
		boolean _wspace = false; // 是否出现了空格

		// Secondary segmentation
		int _ctype = 0;
		int tcount = 1; // number of different char type
		int _TYPE = ENSCFilter.getEnCharType(c); // current char type
		ctrlMask &= ~ISegment.START_SS_MASK; // reset secondary segment mask
		while ((ch = readNext()) != -1) {
		...
		/*
		 * @step 2: 1. clear the useless English punctuation from the end. 2.
		...
		if (!_check) {
			 ...
			 * recognize many other units that is not Chinese like '℉,℃'
			 */
			if (chkunits
		...
		String _temp = null;
		/*
		 * Attention: make sure that (ch = readNext()) is after j <
		 * Config.MIX_CN_LENGTH. or it cause the miss of the next char.
		 * 
		 * @reader: (2013-09-25) we do not check the type of the char read next.
		 * so, words started with English and its length except the start
		 * English part less than config.MIX_CN_LENGTH in the EC dictionary
		 * could be recognized.
		 */
		ialist.clear();
		int j = 0;
		int mc = 0; // the number of char that read from the stream.
		for (; j < config.MIX_CN_LENGTH && (ch = readNext()) != -1; j++) {
			/*
			 * Attention: it is a chance that jcseg works find for we break the
			 * loop directly when we meet a whitespace. 1. if a EC word is
			 * found, unit check process will be ignore. 2. if matches no EC
			 * word, certainly return of readNext() will make sure the units
		...
		ibuffer = null; // Let go do it's work.
		 ...
		 * English punctuation if matches no single units or EC word.
		 */
		if (w == null) {
			w = new Word(__str, IWord.T_BASIC_LATIN);
			w.setPartSpeech(IWord.EN_POSPEECH);
	 ...
	 * count until the char in the specified position is not another number or
	 * whitespace. <br />
	 * 
	 * @param chars
	...
	protected String nextCNNumeric(char[] chars, int index) throws IOException {
		...
		for (int j = index + 1; j < chars.length; j++) {
			...
			if (CNNMFilter.convertCNToInteger(chars[j]) == -1) {
				...
				isb.append(chars[j]);// append the buffer.
			}
		}
		return isb.toString();
	}
