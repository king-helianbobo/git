...
import com.splitword.lionsoul.jcseg.core.IRule;

/**
 * the first filter rule - the maxmum match rule for JCSeg. this rule will
 * return the chunks that own the largest word length.
 * 
...
public class MMRule implements IRule {
	...
	public IChunk[] call(IChunk[] chunks) {
		int maxLength = chunks[0].getLength(); // 选择数组长度最大的那个
		int j;
