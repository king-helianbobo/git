...
public class AMLauncher implements Runnable {
	...
	private void cleanup() throws IOException, YarnException {
		...
		StopContainersResponse response = containerMgrProxy
				.stopContainers(stopRequest);
		if (response.getFailedRequests() != null
	...
	protected ContainerManagementProtocol getContainerMgrProxy(
		...
		final InetSocketAddress containerManagerBindAddress = NetUtils
				.createSocketAddrForHost(node.getHost(), node.getPort());

		final YarnRPC rpc = YarnRPC.create(conf); // TODO: Don't create again
													// and again.

		UserGroupInformation currentUser = UserGroupInformation
				.createRemoteUser(containerId.getApplicationAttemptId()
						.toString());
	...
	private ContainerLaunchContext createAMContainerLaunchContext(
		...
		ContainerLaunchContext container = applicationMasterContext
				.getAMContainerSpec();
	...
	private void setupTokens(ContainerLaunchContext container,
		...
		ApplicationId applicationId = application.getAppAttemptId()
				.getApplicationId();
		environment.put(
				ApplicationConstants.APP_SUBMIT_TIME_ENV,
				String.valueOf(rmContext.getRMApps().get(applicationId)
						.getSubmitTime()));
		environment.put(
