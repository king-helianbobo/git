...
public class DelegationTokenRenewer extends AbstractService {
	...
	// appId=>List<tokens>
	private Set<DelegationTokenToRenew> delegationTokens = Collections
			.synchronizedSet(new HashSet<DelegationTokenToRenew>());
	...
	protected synchronized void serviceInit(Configuration conf)
			throws Exception {
		this.tokenKeepAliveEnabled = conf.getBoolean(
				YarnConfiguration.LOG_AGGREGATION_ENABLED,
				YarnConfiguration.DEFAULT_LOG_AGGREGATION_ENABLED);
		this.tokenRemovalDelayMs = conf.getInt(
	...
	protected ThreadPoolExecutor createNewThreadPoolService(Configuration conf) {
		int nThreads = conf
				.getInt(YarnConfiguration.RM_DELEGATION_TOKEN_RENEWER_THREAD_COUNT,
						YarnConfiguration.DEFAULT_RM_DELEGATION_TOKEN_RENEWER_THREAD_COUNT);

		ThreadFactory tf = new ThreadFactoryBuilder().setNameFormat(
				"DelegationTokenRenewer #%d").build();
		ThreadPoolExecutor pool = new ThreadPoolExecutor((5 < nThreads ? 5
				: nThreads), nThreads, 3L, TimeUnit.SECONDS,
	...
	private void setLocalSecretManagerAndServiceAddr() {
		RMDelegationTokenIdentifier.Renewer.setSecretManager(rmContext
				.getRMDelegationTokenSecretManager(), rmContext
	...
	protected void serviceStart() throws Exception {
		...
		if (tokenKeepAliveEnabled) {
			delayedRemovalThread = new Thread(new DelayedTokenRemovalRunnable(
					getConfig()), "DelayedTokenCanceller");
		...
		public DelegationTokenToRenew(ApplicationId jId, Token<?> token,
			...
			if (this.token == null || this.applicationId == null
				...
				throw new IllegalArgumentException(
						"Invalid params to renew token" + ";token="
								+ this.token + ";appId=" + this.applicationId
								+ ";conf=" + this.conf);
			}
		}
			...
			while (true) {
				try {
					...
					if (LOG.isDebugEnabled()) {
						LOG.debug("Canceling token "
								+ tokenWithConf.token.getService());
					}
					...
					LOG.warn("Got exception "
							+ StringUtils.stringifyException(t) + ". Exiting..");
					System.exit(-1);
				}
			}
		}
	 ...
	 * @throws IOException
	 */
	public void addApplicationAsync(ApplicationId applicationId,
			Credentials ts, boolean shouldCancelAtEnd) {
	...
	private void handleAppSubmitEvent(DelegationTokenRenewerAppSubmitEvent evt)
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Registering tokens for renewal for:" + " appId = "
					+ applicationId);
		}
		...
		// At RM restart it is safe to assume that all the previously added
		// tokens
		// are valid
		...
		for (Token<?> token : tokens) {
			if (token.isManaged()) {
				tokenList.add(new DelegationTokenToRenew(applicationId, token,
						getConfig(), now, shouldCancelAtEnd));
			}
		}
		if (!tokenList.isEmpty()) {
			// Renewing token and adding it to timer calls are separated
			// purposefully
	...
	private class RenewalTimerTask extends TimerTask {
		...
		public synchronized void run() {
			...
			try {
				renewToken(dttr);
				if (LOG.isDebugEnabled()) {
					LOG.debug("Renewing delegation-token for:"
							+ token.getService() + "; new expiration;"
							+ dttr.expirationDate);
				}

				setTimerForTokenRenewal(dttr);// set the next one
			} catch (Exception e) {
				LOG.error("Exception renewing token" + token
						+ ". Not rescheduled", e);
	...
	protected void setTimerForTokenRenewal(DelegationTokenToRenew token)
		...
		long renewIn = token.expirationDate - expiresIn / 10; // little bit
																// before the
																// expiration
	...
	private void removeFailedDelegationToken(DelegationTokenToRenew t) {
		...
		if (LOG.isDebugEnabled())
			LOG.debug("removing failed delegation token for appid="
					+ applicationId + ";t=" + t.token.getService());
		...
		synchronized (delegationTokens) {
			...
			while (it.hasNext()) {
				...
				if (dttr.applicationId.equals(applicationId)) {
					if (LOG.isDebugEnabled()) {
						LOG.debug("Removing delegation token for appId="
								+ applicationId + "; token="
	...
	private class DelayedTokenRemovalRunnable implements Runnable {
		...
		public void run() {
			...
			while (!Thread.currentThread().isInterrupted()) {
				Iterator<Entry<ApplicationId, Long>> it = delayedRemovalMap
						.entrySet().iterator();
				toCancel.clear();
				while (it.hasNext()) {
	...
	private final class DelegationTokenRenewerRunnable implements Runnable {
		...
		private void handleDTRenewerAppSubmitEvent(
			...
			/*
			 * For applications submitted with delegation tokens we are not
			 * submitting the application to scheduler from RMAppManager.
			 * Instead we are doing it from here. The primary goal is to make
			 * token renewal as a part of application submission asynchronous so
			 * that client thread is not blocked during app submission.
			 */
			try {
				...
				rmContext
						...
						.handle(new RMAppRejectedEvent(
								event.getApplicationId(), t.getMessage()));
			}
		}
	}
	...
	enum DelegationTokenRenewerEventType {
		VERIFY_AND_START_APPLICATION, FINISH_APPLICATION
	}
