...
public class ResourceTrackerService extends AbstractService implements
	...
	private static final Log LOG = LogFactory
			.getLog(ResourceTrackerService.class);
	...
	void handleContainerStatus(ContainerStatus containerStatus) {
		...
		if (masterContainer.getId().equals(containerStatus.getContainerId())
			...
			RMAppAttemptContainerFinishedEvent evt = new RMAppAttemptContainerFinishedEvent(
					appAttemptId, containerStatus);
	...
	public RegisterNodeManagerResponse registerNodeManager(
		...
		if (!request.getContainerStatuses().isEmpty()) {
			...
			for (ContainerStatus containerStatus : request
		...
		if (!minimumNodeManagerVersion.equals("NONE")) {
			...
			if ((nodeManagerVersion == null)
				...
				String message = "Disallowed NodeManager Version "
						+ nodeManagerVersion
		...
		RMNode rmNode = new RMNodeImpl(nodeId, rmContext, host, cmPort,
				httpPort, resolve(host), ResourceOption.newInstance(capability,
		...
		String message = "NodeManager from node " + host + "(cmPort: " + cmPort
				+ " httpPort: " + httpPort + ") "
				+ "registered with capability: " + capability
	...
	public NodeHeartbeatResponse nodeHeartbeat(NodeHeartbeatRequest request)
		...
		NodeHeartbeatResponse nodeHeartBeatResponse = YarnServerBuilderUtils
				.newNodeHeartbeatResponse(
						lastNodeHeartbeatResponse.getResponseId() + 1,
		...
		this.rmContext
				.getDispatcher()
				.getEventHandler()
				.handle(new RMNodeStatusEvent(nodeId, remoteNodeStatus
	...
	private void populateKeys(NodeHeartbeatRequest request,
		...
		if (nextMasterKeyForNode != null
				&& (request.getLastKnownContainerTokenMasterKey().getKeyId() != nextMasterKeyForNode
						.getKeyId())) {
			nodeHeartBeatResponse
					.setContainerTokenMasterKey(nextMasterKeyForNode);
		}
		...
		if (nextMasterKeyForNode != null
				&& (request.getLastKnownNMTokenMasterKey().getKeyId() != nextMasterKeyForNode
						.getKeyId())) {
