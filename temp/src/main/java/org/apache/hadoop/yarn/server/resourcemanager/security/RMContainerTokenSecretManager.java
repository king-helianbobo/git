...
public class RMContainerTokenSecretManager extends
	...
	public RMContainerTokenSecretManager(Configuration conf) {
		...
		// container created with a password generated off new master key, but
		// NM
		...
		LOG.info("ContainerTokenKeyRollingInterval: " + this.rollingInterval
				+ "ms and ContainerTokenKeyActivationDelay: "
	...
	public Token createContainerToken(ContainerId containerId, NodeId nodeId,
		...
		try {
			tokenIdentifier = new ContainerTokenIdentifier(containerId,
					nodeId.toString(), appSubmitter, capability,
					expiryTimeStamp, this.currentMasterKey.getMasterKey()
							.getKeyId(), ResourceManager.getClusterTimeStamp());
		...
		return BuilderUtils
				.newContainerToken(nodeId, password, tokenIdentifier);
	}
}