...
public class AppInfo {
	...
	public AppInfo(RMApp app, Boolean hasAccess, String schemePrefix) {
		...
		if (app != null) {
			...
			this.trackingUrlIsNotReady = trackingUrl == null
					|| trackingUrl.isEmpty()
			...
			if (!trackingUrlIsNotReady) {
				this.trackingUrl = WebAppUtils.getURLWithScheme(schemePrefix,
						trackingUrl);
			...
			if (app.getApplicationTags() != null
				...
				this.applicationTags = Joiner.on(',').join(
			...
			if (hasAccess) {
				...
				if (attempt != null) {
					...
					if (masterContainer != null) {
						...
						this.amContainerLogs = WebAppUtils
								.getRunningLogURL(schemePrefix
										+ masterContainer.getNodeHttpAddress(),
										ConverterUtils.toString(masterContainer
												.getId()), app.getUser());
						this.amHostHttpAddress = masterContainer
								.getNodeHttpAddress();
					}
					...
					if (resourceReport != null) {
						Resource usedResources = resourceReport
								.getUsedResources();
						allocatedMB = usedResources.getMemory();
						allocatedVCores = usedResources.getVirtualCores();
						runningContainers = resourceReport
