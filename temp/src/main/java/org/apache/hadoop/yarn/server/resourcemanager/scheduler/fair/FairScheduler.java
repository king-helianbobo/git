...
public class FairScheduler extends AbstractYarnScheduler {
	...
	// reserved
	public static final Resource CONTAINER_RESERVED = Resources
			.createResource(-1);
	...
	private Resource clusterCapacity = RecordFactoryProvider.getRecordFactory(
			null).newRecordInstance(Resource.class);
	...
	private Comparator nodeAvailableResourceComparator = new NodeAvailableResourceComparator(); // Node
																								// available
																								// resource
																								// comparator
	...
	private FSSchedulerApp getCurrentAttemptForContainer(ContainerId containerId) {
		SchedulerApplication app = applications.get(containerId
	...
	boolean isStarvedForMinShare(FSLeafQueue sched) {
		Resource desiredShare = Resources.min(RESOURCE_CALCULATOR,
				clusterCapacity, sched.getMinShare(), sched.getDemand());
	...
	boolean isStarvedForFairShare(FSLeafQueue sched) {
		Resource desiredFairShare = Resources.min(RESOURCE_CALCULATOR,
				clusterCapacity, Resources.multiply(sched.getFairShare(), .5),
				sched.getDemand());
		return Resources.lessThan(RESOURCE_CALCULATOR, clusterCapacity,
	...
	protected synchronized void preemptTasksIfNecessary() {
		...
		for (FSLeafQueue sched : queueMgr.getLeafQueues()) {
			resToPreempt = Resources.add(resToPreempt,
					resToPreempt(sched, curTime));
		}
	...
	protected void preemptResources(Collection<FSLeafQueue> scheds,
		...
		while (warnedIter.hasNext()) {
			...
			if (container.getState() == RMContainerState.RUNNING
					&& Resources.greaterThan(RESOURCE_CALCULATOR,
							clusterCapacity, toPreempt, Resources.none())) {
				warnOrKillContainer(container, apps.get(container),
						queues.get(container));
				preemptedThisRound.add(container);
				Resources.subtractFrom(toPreempt, container.getContainer()
						.getResource());
			} else {
				warnedIter.remove();
			}
		...
		while (runningIter.hasNext()
			...
			if (!preemptedThisRound.contains(container)
					&& Resources.greaterThan(RESOURCE_CALCULATOR,
							clusterCapacity, sched.getResourceUsage(),
							sched.getFairShare())) {
	...
	private void warnOrKillContainer(RMContainer container, FSSchedulerApp app,
		...
		LOG.info("Preempting container (prio="
				+ container.getContainer().getPriority() + "res="
				+ container.getContainer().getResource() + ") from queue "
				+ queue.getName());
	...
	protected Resource resToPreempt(FSLeafQueue sched, long curTime) {
		...
		if (curTime - sched.getLastTimeAtMinShare() > minShareTimeout) {
			Resource target = Resources.min(RESOURCE_CALCULATOR,
					clusterCapacity, sched.getMinShare(), sched.getDemand());
			resDueToMinShare = Resources.max(RESOURCE_CALCULATOR,
					clusterCapacity, Resources.none(),
					Resources.subtract(target, sched.getResourceUsage()));
		}
		if (curTime - sched.getLastTimeAtHalfFairShare() > fairShareTimeout) {
			Resource target = Resources.min(RESOURCE_CALCULATOR,
					clusterCapacity, sched.getFairShare(), sched.getDemand());
			resDueToFairShare = Resources.max(RESOURCE_CALCULATOR,
					clusterCapacity, Resources.none(),
					Resources.subtract(target, sched.getResourceUsage()));
		}
		Resource resToPreempt = Resources.max(RESOURCE_CALCULATOR,
				clusterCapacity, resDueToMinShare, resDueToFairShare);
		if (Resources.greaterThan(RESOURCE_CALCULATOR, clusterCapacity,
				resToPreempt, Resources.none())) {
			String message = "Should preempt " + resToPreempt
					+ " res for queue " + sched.getName()
	...
	protected synchronized void addApplicationAttempt(
		...
		FSSchedulerApp attempt = new FSSchedulerApp(applicationAttemptId, user,
		...
		rmContext
				.getDispatcher()
				.getEventHandler()
				.handle(new RMAppAttemptEvent(applicationAttemptId,
	...
	FSLeafQueue assignToQueue(RMApp rmApp, String queueName, String user) {
		...
		try {
			QueuePlacementPolicy placementPolicy = allocConf
					.getPlacementPolicy();
		...
		if (appRejectMsg != null && rmApp != null) {
			...
			rmContext
					.getDispatcher()
					.getEventHandler()
					.handle(new RMAppRejectedEvent(rmApp.getApplicationId(),
							appRejectMsg));
			return null;
		}
	...
	private synchronized void completedContainer(RMContainer rmContainer,
		...
		FSSchedulerApp application = getCurrentAttemptForContainer(container
				.getId());
		ApplicationId appId = container.getId().getApplicationAttemptId()
				.getApplicationId();
		if (application == null) {
	...
	public Allocation allocate(ApplicationAttemptId appAttemptId,
		...
		synchronized (application) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("allocate:" + " applicationAttemptId=" + appAttemptId
						+ " #ask=" + ask.size());

				LOG.debug("Preempting "
						+ application.getPreemptionContainers().size()
						+ " container(s)");
			}
			...
			return new Allocation(allocation.getContainerList(),
					application.getHeadroom(), preemptionContainerIds, null,
					null, allocation.getNMTokenList());
		}
	}
	...
	private synchronized void nodeUpdate(RMNode nm) {
		if (LOG.isDebugEnabled()) {
			LOG.debug("nodeUpdate: " + nm + " cluster capacity: "
					+ clusterCapacity);
		}
		...
		List<UpdatedContainerInfo> containerInfoList = nm
			...
			for (NodeId nodeId : nodeIdList) {
				if (nodes.containsKey(nodeId)) {
					...
					try {
						...
						LOG.warn("Error while attempting scheduling for node "
								+ node + ": " + ex.toString(), ex);
					}
				}
			}
	...
	private class NodeAvailableResourceComparator implements Comparator<NodeId> {
		...
		public int compare(NodeId n1, NodeId n2) {
			return RESOURCE_CALCULATOR.compare(clusterCapacity, nodes.get(n2)
					.getAvailableResource(), nodes.get(n1)
	...
	private synchronized void attemptScheduling(FSSchedulerNode node) {
		...
		AppSchedulable reservedAppSchedulable = node
				.getReservedAppSchedulable();
		if (reservedAppSchedulable != null) {
			Priority reservedPriority = node.getReservedContainer()
					.getReservedPriority();
			if (!reservedAppSchedulable.hasContainerForNode(reservedPriority,
					node)) {
				// Don't hold the reservation if app can no longer use it
				LOG.info("Releasing reservation that cannot be satisfied for application "
						+ reservedAppSchedulable.getApp()
				...
				LOG.info("Trying to fulfill reservation for application "
						+ reservedAppSchedulable.getApp()
								.getApplicationAttemptId() + " on node: "
						+ node);
	...
	public FSSchedulerApp getSchedulerApp(ApplicationAttemptId appAttemptId) {
		SchedulerApplication app = applications.get(appAttemptId
				.getApplicationId());
		if (app != null) {
			return (FSSchedulerApp) app.getCurrentAppAttempt();
		}
	...
	public SchedulerAppReport getSchedulerAppInfo(
		...
		if (attempt == null) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("Request for appInfo of unknown attempt "
						+ appAttemptId);
			}
			return null;
		}
	...
	private void updateRootQueueMetrics() {
		rootMetrics.setAvailableResourcesToQueue(Resources.subtract(
	...
	public void handle(SchedulerEvent event) {
		...
		case CONTAINER_EXPIRED:
			...
			break;
		default:
			LOG.error("Unknown event arrived at FairScheduler: "
					+ event.toString());
		}
	}
	...
	public synchronized void reinitialize(Configuration conf,
		...
		if (!initialized) {
			...
			continuousSchedulingEnabled = this.conf
					.isContinuousSchedulingEnabled();
			continuousSchedulingSleepMs = this.conf
			...
			// If we fail to load allocations file on initialize, we want to
			// fail
			// immediately. After a successful load, exceptions on future
			// reloads
	...
	public synchronized String moveApplication(ApplicationId appId,
		...
		synchronized (attempt) {
			...
			if (oldQueue.getRunnableAppSchedulables().contains(
				...
				verifyMoveDoesNotViolateConstraints(attempt, oldQueue,
						targetQueue);
			}
	...
	private void verifyMoveDoesNotViolateConstraints(FSSchedulerApp app,
		...
		while (cur != lowestCommonAncestor) {
			// maxRunningApps
			if (cur.getNumRunnableApps() == allocConf.getQueueMaxApps(cur
					.getQueueName())) {
				throw new YarnException("Moving app attempt " + appAttId
						+ " to queue " + queueName
			...
			// maxShare
			if (!Resources.fitsIn(
					Resources.add(cur.getResourceUsage(), consumption),
					cur.getMaxShare())) {
				throw new YarnException("Moving app attempt " + appAttId
						+ " to queue " + queueName
						+ " would violate queue maxShare constraints on"
						+ " queue " + cur.getQueueName());
			}
	...
	private void executeMove(SchedulerApplication app, FSSchedulerApp attempt,
		...
		if (wasRunnable && !nowRunnable) {
			throw new IllegalStateException(
					"Should have already verified that app "
