...
public class FiCaSchedulerNode extends SchedulerNode {
	...
	private Resource availableResource = recordFactory
	...
	public FiCaSchedulerNode(RMNode node, boolean usePortForNodeName) {
		...
		this.availableResource.setVirtualCores(node.getTotalCapability()
				.getVirtualCores());
		totalResourceCapability = Resource.newInstance(node
				.getTotalCapability().getMemory(), node.getTotalCapability()
				.getVirtualCores());
	...
	public synchronized void allocateContainer(ApplicationId applicationId,
		...
		LOG.info("Assigned container " + container.getId() + " of capacity "
				...
				+ rmNode.getNodeAddress() + ", which currently has "
				+ numContainers + " containers, " + getUsedResource()
				+ " used and " + getAvailableResource() + " available");
	}
	...
	public synchronized void reserveResource(
		...
		if (this.reservedContainer != null) {
			// Sanity check
			if (!reservedContainer.getContainer().getNodeId()
					.equals(getNodeID())) {
				throw new IllegalStateException("Trying to reserve"
						+ " container " + reservedContainer + " on node "
			...
			if (!this.reservedContainer
					...
					.equals(reservedContainer.getContainer().getId()
							.getApplicationAttemptId())) {
				throw new IllegalStateException("Trying to reserve"
	...
	public synchronized void unreserveResource(
		...
		if (reservedContainer != null
			...
			ApplicationAttemptId reservedApplication = reservedContainer
					.getContainer().getId().getApplicationAttemptId();
			if (!reservedApplication.equals(application
					.getApplicationAttemptId())) {
