...
public class MaxRunningAppsEnforcer {
	...
	public boolean canAppBeRunnable(FSQueue queue, String user) {
		AllocationConfiguration allocConf = scheduler
	...
	/**
	 * Tracks the given new non runnable app so that it can be made runnable
	 * when it would not violate max running app limits.
	 */
	...
	public void updateRunnabilityOnAppRemoval(FSSchedulerApp app,
		...
		// childqueueX might have no pending apps itself, but if a queue higher
		// up
		...
		// An app removal will only possibly allow another app to become
		// runnable if
		...
		if (userNumRunning == allocConf.getUserMaxApps(user) - 1) {
			List<AppSchedulable> userWaitingApps = usersNonRunnableApps
					.get(user);
		...
		while (iter.hasNext()) {
			...
			if (canAppBeRunnable(next.getQueue(), next.getUser())) {
				...
				// No more than one app per list will be able to be made
				// runnable, so
		...
		// We remove the apps from their pending lists afterwards so that we
		// don't
		...
		for (AppSchedulable appSched : noLongerPendingApps) {
			...
			if (!usersNonRunnableApps.remove(appSched.getApp().getUser(),
					appSched)) {
				LOG.error("Waiting app "
						+ appSched
						+ " expected to be in "
	 ...
	 * given queue and user has been removed.
	 */
	...
	private void gatherPossiblyRunnableAppLists(FSQueue queue,
		...
		if (queue.getNumRunnableApps() < scheduler.getAllocationConfiguration()
			...
			if (queue instanceof FSLeafQueue) {
				appLists.add(((FSLeafQueue) queue)
	...
	/**
	 * Takes a list of lists, each of which is ordered by start time, and
	 * returns their elements in order of start time.
	 * 
	 * We maintain positions in each of the lists. Each next() call advances the
	 * position in one of the lists. We maintain a heap that orders lists by the
	 * start time of the app in the current position in that list. This allows
	 * us to pick which list to advance in O(log(num lists)) instead of O(num
	 * lists) time.
	 */
	static class MultiListStartTimeIterator implements Iterator<FSSchedulerApp> {
		...
		public FSSchedulerApp next() {
			...
			if (curPositionsInAppLists[nextListIndex] < appLists[nextListIndex]
					.size()) {
				indexAndTime.time = appLists[nextListIndex].get(
						curPositionsInAppLists[nextListIndex]).getStartTime();
			} else {
