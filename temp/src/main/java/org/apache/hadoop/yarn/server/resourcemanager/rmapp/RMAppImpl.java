...
public class RMAppImpl implements RMApp, Recoverable {
	...
	// These states stored are only valid when app is at killing or
	// final_saving.
	...
	private static final StateMachineFactory<RMAppImpl, RMAppState, RMAppEventType, RMAppEvent> stateMachineFactory = new StateMachineFactory<RMAppImpl, RMAppState, RMAppEventType, RMAppEvent>(
			...
			.addTransition(
					...
					EnumSet.of(RMAppState.SUBMITTED, RMAppState.ACCEPTED,
							RMAppState.FINISHED, RMAppState.FAILED,
			...
			.addTransition(
					...
					// event because RMAppRecoveredTransition is returning
					// ACCEPTED state
			...
			// ACCECPTED state can once again receive APP_ACCEPTED event,
			// because on
			...
			.addTransition(RMAppState.FINISHING, RMAppState.FINISHING,
					...
					// ignore Kill as we have already saved the final Finished
					// state in
					// state store.
							RMAppEventType.KILL))
	...
	public ApplicationReport createAndGetApplicationReport(
		...
		try {
			...
			if (allowAccess) {
				if (this.currentAttempt != null) {
					...
					if (UserGroupInformation.isSecurityEnabled()) {
						// get a token so the client can communicate with the
						// app attempt
						// NOTE: token may be unavailable if the attempt is not
						// running
						...
						if (attemptClientToAMToken != null) {
							clientToAMToken = BuilderUtils
									.newClientToAMToken(attemptClientToAMToken
											.getIdentifier(),
											attemptClientToAMToken.getKind()
													.toString(),
											attemptClientToAMToken
													.getPassword(),
											attemptClientToAMToken.getService()
													.toString());
						}
					}
			...
			return BuilderUtils.newApplicationReport(this.applicationId,
					...
					createApplicationState(), diags, trackingUrl,
					this.startTime, this.finishTime, finishState,
					appUsageReport, origTrackingUrl, progress,
					this.applicationType, amrmToken, applicationTags);
		} finally {
			this.readLock.unlock();
		}
	}
	...
	private void createAndStartNewAttempt(
		...
		createNewAttempt();
		handler.handle(new RMAppStartAttemptEvent(currentAttempt
				.getAppAttemptId(), transferStateFromPreviousAttempt));
	}
	...
	private static final class RMAppRecoveredTransition implements
		...
		public RMAppState transition(RMAppImpl app, RMAppEvent event) {
			...
			for (RMAppAttempt attempt : app.getAppAttempts().values()) {
				// synchronously recover attempt to ensure any incoming external
				// events
				// to be processed after the attempt processes the recover
				// event.
			...
			if (app.recoveredFinalState != null) {
				new FinalTransition(app.recoveredFinalState).transition(app,
						event);
			...
			if (app.currentAttempt != null
					&& (app.currentAttempt.getState() == RMAppAttemptState.KILLED
							|| app.currentAttempt.getState() == RMAppAttemptState.FINISHED || (app.currentAttempt
							.getState() == RMAppAttemptState.FAILED && app.attempts
							.size() == app.maxAppAttempts))) {
			...
			// No existent attempts means the attempt associated with this app
			// was not
			...
			// YARN-1507 is saving the application state after the application
			// is
			// accepted. So after YARN-1507, an app is saved meaning it is
			// accepted.
	...
	private void rememberTargetTransitionsAndStoreState(RMAppEvent event,
		...
		// create GetApplicationAttemptReport specifically for getting per
		// attempt
		// info.
		String diags = null;
		...
		ApplicationState appState = new ApplicationState(this.submitTime,
				this.startTime, this.submissionContext, this.user,
	...
	private static class AttemptFinishedAtFinalSavingTransition extends
		...
		public void transition(RMAppImpl app, RMAppEvent event) {
			if (app.targetedFinalState.equals(RMAppState.FAILED)
				...
				// Ignore Attempt_Finished event if we were supposed to reach
				// FAILED
				// FINISHED state
				return;
			}

			// pass in the earlier attempt_unregistered event, as it is needed
			// in
			// AppFinishedFinalStateSavedTransition later on
			app.rememberTargetTransitions(event,
					new AppFinishedFinalStateSavedTransition(
	...
	private static class AppFinishedFinalStateSavedTransition extends
		...
		@Override
		public void transition(RMAppImpl app, RMAppEvent event) {
			new AttemptUnregisteredTransition().transition(app,
					attemptUnregistered);
	...
	private static class FinalTransition extends RMAppTransition {
		...
		public void transition(RMAppImpl app, RMAppEvent event) {
			...
			app.rmContext.getRMApplicationHistoryWriter().applicationFinished(
					app, finalState);
		};
	}
	...
	private static final class AttemptFailedTransition implements
		...
		public RMAppState transition(RMAppImpl app, RMAppEvent event) {
			if (!app.submissionContext.getUnmanagedAM()
				...
				// attempt.
				// Note that the previous failed attempt may still be collecting
				// the
				// container events from the scheduler and update its data
				// structures
				...
				app.rememberTargetTransitionsAndStoreState(event,
						new AttemptFailedFinalStateSavedTransition(),
	...
	public boolean isAppFinalStateStored() {
		...
		return state.equals(RMAppState.FINISHING)
				|| state.equals(RMAppState.FINISHED)
				|| state.equals(RMAppState.FAILED)
