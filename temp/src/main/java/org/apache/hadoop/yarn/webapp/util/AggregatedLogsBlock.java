...
public class AggregatedLogsBlock extends HtmlBlock {
	...
	protected void render(Block html) {
		...
		try {
			...
			if (!conf.getBoolean(YarnConfiguration.LOG_AGGREGATION_ENABLED,
				...
				html.h1()
						._("Aggregation is not enabled. Try the nodemanager at "
								+ nodeId)._();
				return;
			}
			...
			try {
				reader = new AggregatedLogFormat.LogReader(conf,
						LogAggregationUtils.getRemoteNodeLogFileForApp(
								...
								nodeId, LogAggregationUtils
										.getRemoteNodeLogDirSuffix(conf)));
			...
			if (callerUGI != null
					&& !aclsManager.checkAccess(callerUGI,
							ApplicationAccessType.VIEW_APP, owner,
							applicationId)) {
				html.h1()
						._("User [" + remoteUser
								+ "] is not authorized to view the logs for "
								+ logEntity)._();
				return;
			}
				...
				if (logReader == null) {
					html.h1()
							._("Logs not available for "
									+ logEntity
									+ ". Could be caused by the rentention policy")
							._();
					return;
				}

				boolean foundLog = readContainerLogs(html, logReader,
						logLimits, desiredLogType);
	...
	private boolean readContainerLogs(Block html,
			...
			LogLimits logLimits, String desiredLogType) throws IOException {
		...
		while (logType != null) {
			if (desiredLogType == null || desiredLogType.isEmpty()
				...
				long start = logLimits.start < 0 ? logLength + logLimits.start
						: logLimits.start;
				start = start < 0 ? 0 : start;
				start = start > logLength ? logLength : start;
				long end = logLimits.end < 0 ? logLength + logLimits.end
						: logLimits.end;
				end = end < 0 ? 0 : end;
				...
				if (toRead < logLength) {
					html.p()
							._("Showing " + toRead + " bytes of " + logLength
									+ " total. Click ")
							.a(url("logs", $(NM_NODENAME), $(CONTAINER_ID),
									$(ENTITY_STRING), $(APP_OWNER), logType,
									"?start=0"), "here")
				...
				int len = 0;
				int currentToRead = toRead > bufferSize ? bufferSize
						: (int) toRead;
				...
				while (toRead > 0
					...
					toRead = toRead - len;
					currentToRead = toRead > bufferSize ? bufferSize
							: (int) toRead;
				}
	...
	private NodeId verifyAndGetNodeId(Block html) {
		...
		try {
			...
			html.h1()
					._("Cannot get container logs. Invalid nodeId: "
							+ nodeIdStr)._();
			return null;
		}
		return nodeId;
