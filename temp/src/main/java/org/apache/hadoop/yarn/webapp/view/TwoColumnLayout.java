...
/**
 * A simpler two column layout implementation with a header, a navigation bar on
 * the left, content on the right, and a footer. Works with resizable themes.
 * 
...
public class TwoColumnLayout extends HtmlPage {
	 ...
	 * 
	 * @see
	 * org.apache.hadoop.yarn.webapp.view.HtmlPage#render(org.apache.hadoop.
	...
	protected void render(Page.HTML<_> html) {
		...
		html.table("#layout.ui-widget-content").thead().tr().td().$colspan(2)
				._(header())._()._()._().tfoot().tr().td().$colspan(2)
				._(footer())._()._()._().tbody().tr().td().$id("navcell")
				._(nav())._().td().$class("content")._(content())._()._()._()
				._()._();
	}
