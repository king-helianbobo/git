...
public class CapacityScheduler extends AbstractYarnScheduler implements
	...
	private Resource clusterResource = RecordFactoryProvider.getRecordFactory(
	...
	@Private
	public static final String ROOT_QUEUE = CapacitySchedulerConfiguration.PREFIX
	...
	private void validateExistingQueues(Map<String, CSQueue> queues,
		...
		for (String queue : queues.keySet()) {
			if (!newQueues.containsKey(queue)) {
				throw new IOException(queue
						+ " cannot be found during refresh!");
			}
		}
	}
	...
	static CSQueue parseQueue(CapacitySchedulerContext csContext,
		...
		if (childQueueNames == null || childQueueNames.length == 0) {
			...
			ParentQueue parentQueue = new ParentQueue(csContext, queueName,
	...
	private synchronized void addApplication(ApplicationId applicationId,
		...
		if (queue == null) {
			String message = "Application " + applicationId
					+ " submitted by user " + user + " to unknown queue: "
					+ queueName;
		...
		try {
			...
			this.rmContext
					.getDispatcher()
					.getEventHandler()
					.handle(new RMAppRejectedEvent(applicationId, ace
							.toString()));
			return;
		}
	...
	public Allocation allocate(ApplicationAttemptId applicationAttemptId,
		...
		SchedulerUtils.normalizeRequests(ask, getResourceCalculator(),
				getClusterResources(), getMinimumResourceCapability(),
				maximumAllocation);
	...
	private synchronized void nodeUpdate(RMNode nm) {
		if (LOG.isDebugEnabled()) {
			LOG.debug("nodeUpdate: " + nm + " clusterResources: "
					+ clusterResource);
		}
	...
	private synchronized void allocateContainersToNode(FiCaSchedulerNode node) {
		...
		if (reservedContainer != null) {
			...
			CSAssignment assignment = queue.assignContainers(clusterResource,
					node);

			RMContainer excessReservation = assignment.getExcessReservation();
			if (excessReservation != null) {
				Container container = excessReservation.getContainer();
				queue.completedContainer(clusterResource, assignment
						.getApplication(), node, excessReservation,
						SchedulerUtils.createAbnormalContainerStatus(
								container.getId(),
		...
		if (node.getReservedContainer() == null) {
			if (Resources.greaterThanOrEqual(calculator, getClusterResources(),
				...
				if (LOG.isDebugEnabled()) {
					LOG.debug("Trying to schedule on node: "
							+ node.getNodeName() + ", available: "
	...
	private synchronized void addNode(RMNode nodeManager) {
		this.nodes.put(nodeManager.getNodeID(), new FiCaSchedulerNode(
				nodeManager, usePortForNodeName));
	...
	public FiCaSchedulerApp getApplicationAttempt(
			ApplicationAttemptId applicationAttemptId) {
		SchedulerApplication app = applications.get(applicationAttemptId
				.getApplicationId());
		if (app != null) {
	...
	public void killContainer(RMContainer cont) {
		...
		completedContainer(cont, SchedulerUtils.createPreemptedContainerStatus(
				cont.getContainerId(), "Container being forcibly preempted:"
						+ cont.getContainerId()), RMContainerEventType.KILL);
	}
	...
	private CapacitySchedulerConfiguration loadCapacitySchedulerConfiguration(
		...
		try {
			InputStream CSInputStream = this.rmContext
