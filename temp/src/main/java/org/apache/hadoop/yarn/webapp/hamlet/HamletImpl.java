 ...
 * 
 * Zero copy but allocation on every element, which could be optimized to use a
 * thread-local element pool.
 * 
...
public class HamletImpl extends HamletSpec {
	...
	private static final Splitter SS = Splitter.on('.').omitEmptyStrings()
			.trimResults();
	...
	public class EImp<T extends _> implements _Child {
		...
		protected void _p(boolean quote, Object... args) {
			...
			for (Object s : args) {
				...
				out.print(quote ? escapeHtml(String.valueOf(s)) : String
						.valueOf(s));
	...
	protected void printStartTag(String name, EnumSet<EOpt> opts) {
		indent(opts);
		sb.setLength(0);
		out.print(sb.append('<').append(name).toString()); // for easier mock
															// test
	}
	 ...
	 * @return an two element array [id, "space-separated classes"]. Either
