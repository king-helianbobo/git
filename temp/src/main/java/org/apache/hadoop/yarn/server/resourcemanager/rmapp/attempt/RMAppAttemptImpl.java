...
public class RMAppAttemptImpl implements RMAppAttempt, Recoverable {
	...
	private Set<NodeId> ranNodes = new HashSet<NodeId>();
	private List<ContainerStatus> justFinishedContainers = new ArrayList<ContainerStatus>();
	...
	private static final StateMachineFactory<RMAppAttemptImpl, RMAppAttemptState, RMAppAttemptEventType, RMAppAttemptEvent> stateMachineFactory = new StateMachineFactory<RMAppAttemptImpl, RMAppAttemptState, RMAppAttemptEventType, RMAppAttemptEvent>(
			...
			.addTransition(
					RMAppAttemptState.NEW,
					EnumSet.of(RMAppAttemptState.FINISHED,
							RMAppAttemptState.KILLED, RMAppAttemptState.FAILED,
							RMAppAttemptState.LAUNCHED),
					RMAppAttemptEventType.RECOVER,
					new AttemptRecoveredTransition())
			...
			.addTransition(
					...
					RMAppAttemptState.FINAL_SAVING,
					RMAppAttemptEventType.KILL,
					new FinalSavingTransition(new KillAllocatedAMTransition(),
			...
			.addTransition(
					RMAppAttemptState.RUNNING,
					EnumSet.of(RMAppAttemptState.RUNNING,
							RMAppAttemptState.FINAL_SAVING),
					RMAppAttemptEventType.CONTAINER_FINISHED,
			...
			.addTransition(
					RMAppAttemptState.FINAL_SAVING,
					EnumSet.of(RMAppAttemptState.FINISHING,
							RMAppAttemptState.FAILED, RMAppAttemptState.KILLED,
							RMAppAttemptState.FINISHED),
			...
			.addTransition(
					...
					EnumSet.of(
							RMAppAttemptEventType.UNREGISTERED,
							RMAppAttemptEventType.STATUS_UPDATE,
							// should be fixed to reject container allocate
							// request at Final
							// Saving in scheduler
							RMAppAttemptEventType.CONTAINER_ALLOCATED,
							RMAppAttemptEventType.CONTAINER_ACQUIRED,
			...
			// For work-preserving AM restart, failed attempt are still
			// capturing
			// CONTAINER_FINISHED event and record the finished containers for
			// the
			...
			.addTransition(
					RMAppAttemptState.FINISHING,
					RMAppAttemptState.FINISHING,
					EnumSet.of(RMAppAttemptEventType.UNREGISTERED,
							RMAppAttemptEventType.STATUS_UPDATE,
							RMAppAttemptEventType.CONTAINER_ALLOCATED,
							// ignore Kill as we have already saved the final
							// Finished state in
							// state store.
							RMAppAttemptEventType.KILL))
	...
	public String getTrackingUrl() {
		this.readLock.lock();
		try {
			return (getSubmissionContext().getUnmanagedAM()) ? this.originalTrackingUrl
					: this.proxiedTrackingUrl;
		} finally {
			this.readLock.unlock();
		}
	}
	...
	public String getWebProxyBase() {
		this.readLock.lock();
		try {
			return ProxyUriUtils.getPath(applicationAttemptId
					.getApplicationId());
		} finally {
			this.readLock.unlock();
		}
	...
	private String generateProxyUriWithScheme(
		...
		try {
			...
			URI trackingUri = StringUtils.isEmpty(trackingUriWithoutScheme) ? null
					: ProxyUriUtils.getUriFromAMUrl(scheme,
							trackingUriWithoutScheme);
	...
	// This is only used for RMStateStore. Normal operation must invoke the
	// secret
	...
	public Token<ClientToAMTokenIdentifier> createClientToken(String client) {
		...
		try {
			...
			ClientToAMTokenSecretManagerInRM secretMgr = this.rmContext
	...
	public void handle(RMAppAttemptEvent event) {
		...
		try {
			...
			if (oldState != getAppAttemptState()) {
				LOG.info(appAttemptID + " State change from " + oldState
						+ " to " + getAppAttemptState());
			}
		} finally {
			this.writeLock.unlock();
		}
	...
	public ApplicationResourceUsageReport getApplicationResourceUsageReport() {
		this.readLock.lock();
		try {
			ApplicationResourceUsageReport report = scheduler
					.getAppResourceUsageReport(this.getAppAttemptId());
			if (report == null) {
				Resource none = Resource.newInstance(0, 0);
				report = ApplicationResourceUsageReport.newInstance(0, 0, none,
						none, none);
			}
			return report;
		} finally {
	...
	public void recover(RMState state) throws Exception {
		...
		ApplicationAttemptState attemptState = appState
				.getAttempt(getAppAttemptId());
		assert attemptState != null;
		LOG.info("Recovering attempt: " + getAppAttemptId()
				+ " with final state: " + attemptState.getState());
	...
	private void recoverAppAttemptCredentials(Credentials appAttemptTokens)
		...
		if (UserGroupInformation.isSecurityEnabled()) {
			...
			clientTokenMasterKey = rmContext.getClientToAMTokenSecretManager()
					.registerMasterKey(applicationAttemptId,
							clientTokenMasterKeyBytes);
		}

		// Only one AMRMToken is stored per-attempt, so this should be fine.
		// Can't
		...
		rmContext.getAMRMTokenSecretManager().addPersistedPassword(
				this.amrmToken);
	}
	...
	private static final class AttemptStartedTransition extends BaseTransition {
		...
		public void transition(RMAppAttemptImpl appAttempt,
			...
			// create AMRMToken
			AMRMTokenIdentifier id = new AMRMTokenIdentifier(
					appAttempt.applicationAttemptId);
	...
	private static final class ScheduleTransition
		...
		public RMAppAttemptState transition(RMAppAttemptImpl appAttempt,
			...
			if (!appAttempt.submissionContext.getUnmanagedAM()) {
				...
				// SchedulerUtils.validateResourceRequests is not necessary
				// because
	...
	private static final class AMContainerAllocatedTransition
		...
		public RMAppAttemptState transition(RMAppAttemptImpl appAttempt,
			...
			// CONTAINER_ALLOCATED is emitted after an RMContainer is
			// constructed,
			...
			// fetch it since container may not be fetchable for some reason
			// like
			// DNS unavailable causing container token not generated. As such,
			// we
	...
	private void retryFetchingAMContainer(final RMAppAttemptImpl appAttempt) {
		// start a new thread so that we are not blocking main dispatcher
		// thread.
		new Thread() {
			@Override
			public void run() {
				...
				appAttempt.eventHandler
						.handle(new RMAppAttemptContainerAllocatedEvent(
								appAttempt.applicationAttemptId));
			}
		}.start();
	}
	...
	private static class AttemptRecoveredTransition
		...
		public RMAppAttemptState transition(RMAppAttemptImpl appAttempt,
			 ...
			 * was started but AM container may or may not have started /
			 * finished. Therefore we should wait for it to finish.
			 */
			if (appAttempt.recoveredFinalState != null) {
				appAttempt.progress = 1.0f;
				RMApp rmApp = appAttempt.rmContext.getRMApps().get(
						appAttempt.getAppAttemptId().getApplicationId());
				// We will replay the final attempt only if last attempt is in
				// final
				// state but application is not in final state.
				if (rmApp.getCurrentAppAttempt() == appAttempt
						&& !RMAppImpl.isAppInFinalState(rmApp)) {
					(new BaseFinalTransition(appAttempt.recoveredFinalState))
							.transition(appAttempt, event);
				}
				return appAttempt.recoveredFinalState;
			} else {
				/*
				 * Since the application attempt's final state is not saved that
				 * means for AM container (previous attempt) state must be one
				 * of these. 1) AM container may not have been launched (RM
				 * failed right before this). 2) AM container was successfully
				 * launched but may or may not have registered / unregistered.
				 * In whichever case we will wait (by moving attempt into
				 * LAUNCHED state) and mark this attempt failed (assuming non
				 * work preserving restart) only after 1) Node manager during
				 * re-registration heart beats back saying am container
				 * finished. 2) OR AMLivelinessMonitor expires this attempt
				 * (when am doesn't heart beat back).
				 */
	...
	private void rememberTargetTransitionsAndStoreState(
			RMAppAttemptEvent event, Object transitionToDo,
		...
		case UNREGISTERED:
			...
			finalTrackingUrl = sanitizeTrackingUrl(unregisterEvent
					.getFinalTrackingUrl());
	...
	private static class FinalSavingTransition extends BaseTransition {
		...
		public void transition(RMAppAttemptImpl appAttempt,
				RMAppAttemptEvent event) {
			// For cases Killed/Failed, targetedFinalState is the same as the
			// state to
			// be stored
			appAttempt.rememberTargetTransitionsAndStoreState(event,
					transitionToDo, targetedFinalState, targetedFinalState);
		}
	}
	...
	private static class FinalStateSavedTransition
		...
		public RMAppAttemptState transition(RMAppAttemptImpl appAttempt,
			...
			if (storeEvent.getUpdatedException() != null) {
				LOG.error(
						"Failed to update the final state of application attempt: "
	...
	private static class AMLaunchedTransition extends BaseTransition {
		...
		public void transition(RMAppAttemptImpl appAttempt,
			...
			// otherwise client may hold an invalid ClientToken after RM
			// restarts.
	...
	private static final class UnmanagedAMAttemptSavedTransition extends
			AMLaunchedTransition {
		@Override
		public void transition(RMAppAttemptImpl appAttempt,
				RMAppAttemptEvent event) {
			appAttempt.checkAttemptStoreError(event);
			// TODO Today unmanaged AM client is waiting for app state to be
			// Accepted to
			// launch the AM. This is broken since we changed to start the
			// attempt
			// after the application is Accepted. We may need to introduce an
			// attempt
			// report that client can rely on to query the attempt state and
			// choose to
	...
	private static final class AMRegisteredTransition extends BaseTransition {
		...
		public void transition(RMAppAttemptImpl appAttempt,
			...
			appAttempt.originalTrackingUrl = sanitizeTrackingUrl(registrationEvent
					.getTrackingurl());
			appAttempt.proxiedTrackingUrl = appAttempt
					.generateProxyUriWithScheme(appAttempt.originalTrackingUrl);
			...
			// TODO:FIXME: Note for future. Unfortunately we only do a
			// state-store
			// write at AM launch time, so we don't save the AM's tracking URL
			// anywhere
			// as that would mean an extra state-store write. For now, we hope
			// that in
	...
	private static String getAMContainerCrashedDiagnostics(
		...
		String diagnostics = "AM Container for "
				+ finishEvent.getApplicationAttemptId() + " exited with "
				+ " exitCode: " + status.getExitStatus() + " due to: "
				+ status.getDiagnostics() + "." + "Failing this attempt.";
		return diagnostics;
	}
	...
	private static String getAMExpiredDiagnostics(RMAppAttemptEvent event) {
		String diag = "ApplicationMaster for attempt "
				+ event.getApplicationAttemptId() + " timed out";
		return diag;
	}
	...
	private static class UnexpectedAMRegisteredTransition extends
		...
		public void transition(RMAppAttemptImpl appAttempt,
			...
			appAttempt.diagnostics
					.append(getUnexpectedAMRegisteredDiagnostics());
			super.transition(appAttempt, event);
		}
	...
	private static final class AMUnregisteredTransition
		...
		public RMAppAttemptState transition(RMAppAttemptImpl appAttempt,
			...
			ApplicationId applicationId = appAttempt.getAppAttemptId()
					.getApplicationId();

			// Tell the app immediately that AM is unregistering so that app
			// itself
			// can save its state as soon as possible. Whether we do it like
			// this, or
			// we wait till AppAttempt is saved, it doesn't make any difference
			// on the
			// app side w.r.t failure conditions. The only event going out of
			// AppAttempt to App after this point of time is AM/AppAttempt
			// Finished.
	...
	private static class FinalStateSavedAfterAMUnregisterTransition extends
		...
		public void transition(RMAppAttemptImpl appAttempt,
				RMAppAttemptEvent event) {
			// Unregister from the AMlivenessMonitor and register with
			// AMFinishingMonitor
			appAttempt.rmContext.getAMLivelinessMonitor().unregister(
					appAttempt.applicationAttemptId);
			...
			// Do not make any more changes to this transition code. Make all
			// changes
			// to the following method. Unless you are absolutely sure that you
			// have
			// stuff to do that shouldn't be used by the callers of the
			// following
			// method.
			appAttempt.updateInfoOnAMUnregister(event);
		}
	...
	private static final class ContainerFinishedTransition
		...
		public RMAppAttemptState transition(RMAppAttemptImpl appAttempt,
			...
			ContainerStatus containerStatus = containerFinishedEvent
					.getContainerStatus();

			// Is this container the AmContainer? If the finished container is
			// same as
			// the AMContainer, AppAttempt fails
			if (appAttempt.masterContainer != null
					&& appAttempt.masterContainer.getId().equals(
							containerStatus.getContainerId())) {
				// Remember the follow up transition and save the final attempt
				// state.
				appAttempt.rememberTargetTransitionsAndStoreState(event,
	...
	private static final class ContainerFinishedAtFailedTransition extends
			BaseTransition {
		@Override
		public void transition(RMAppAttemptImpl appAttempt,
				RMAppAttemptEvent event) {
	...
	private static class ContainerFinishedAtFinalSavingTransition extends
		...
		public void transition(RMAppAttemptImpl appAttempt,
			...
			// If this is the AM container, it means the AM container is
			// finished,
			// but we are not yet acknowledged that the final state has been
			// saved.
			// Thus, we still return FINAL_SAVING state here.
			if (appAttempt.masterContainer.getId().equals(
					containerStatus.getContainerId())) {
				if (appAttempt.targetedFinalState
						.equals(RMAppAttemptState.FAILED)
						|| appAttempt.targetedFinalState
								.equals(RMAppAttemptState.KILLED)) {
					// ignore Container_Finished Event if we were supposed to
					// reach
					// FAILED/KILLED state.
					return;
				}

				// pass in the earlier AMUnregistered Event also, as this is
				// needed for
				...
				appAttempt.rememberTargetTransitions(event,
						new AMFinishedAfterFinalSavingTransition(
								appAttempt.eventCausingFinalSaving),
						RMAppAttemptState.FINISHED);
				return;
			}
			// Normal container.
	...
	private static class AMExpiredAtFinalSavingTransition extends
		...
		public void transition(RMAppAttemptImpl appAttempt,
			...
			// pass in the earlier AMUnregistered Event also, as this is needed
			// for
			// AMFinishedAfterFinalSavingTransition later on
			appAttempt.rememberTargetTransitions(event,
					new AMFinishedAfterFinalSavingTransition(
	...
	private void checkAttemptStoreError(RMAppAttemptEvent event) {
		...
		if (storeEvent.getStoredException() != null) {
			// This needs to be handled for HA and give up master status if we
			// got
			// fenced
	...
	public ApplicationAttemptReport createApplicationAttemptReport() {
		...
		try {
			// AM container maybe not yet allocated. and also unmangedAM doesn't
			// have
			// am container.
			ContainerId amId = masterContainer == null ? null : masterContainer
					.getId();
			attemptReport = ApplicationAttemptReport.newInstance(this
					.getAppAttemptId(), this.getHost(), this.getRpcPort(), this
					.getTrackingUrl(), this.getDiagnostics(),
					YarnApplicationAttemptState.valueOf(this.getState()
							.toString()), amId);
		} finally {
			this.readLock.unlock();
		}
