...
/**
 * The servlet that dispatch request to various controllers according to the
 * user defined routes in the router.
 */
...
public class Dispatcher extends HttpServlet {
	...
	public void service(HttpServletRequest req, HttpServletResponse res)
		...
		Controller.RequestContext rc = injector
				.getInstance(Controller.RequestContext.class);
		if (setCookieParams(rc, req) > 0) {
			Cookie ec = rc.cookies().get(ERROR_COOKIE);
			if (ec != null) {
				rc.setStatus(Integer.parseInt(rc.cookies().get(STATUS_COOKIE)
						.getValue()));
	...
	public static void redirectToErrorPage(HttpServletResponse res,
			Throwable e, String path, boolean devMode) {
		String st = devMode ? ErrorPage.toStackTrace(e, 1024 * 3) // spec: min
																	// 4KB
	...
	private void setMoreParams(RequestContext rc, String pathInfo, Dest dest) {
		...
		String[] parts = Iterables.toArray(WebApp.pathSplitter.split(pathInfo
				.substring(dest.prefix.length())), String.class);
