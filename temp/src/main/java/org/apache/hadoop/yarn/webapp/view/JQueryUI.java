...
public class JQueryUI extends HtmlBlock {
	...
	public static final String C_PROGRESSBAR = _PROGRESSBAR.replace('.', ' ')
			.trim();
	public static final String _PROGRESSBAR_VALUE = ".ui-progressbar-value.ui-widget-header.ui-corner-left";
	public static final String C_PROGRESSBAR_VALUE = _PROGRESSBAR_VALUE
			.replace('.', ' ').trim();
	...
	public static void jsnotice(HTML html) {
		html.div("#jsnotice.ui-state-error")
	...
	protected void initDataTables(List<String> list) {
		...
		for (String id : split($(DATATABLES_ID))) {
			if (Html.isValidId(id)) {
				...
				init = new StringBuffer(init).insert(pos, stateSaveInit)
						.toString();
				list.add(join(id, "DataTable =  $('#", id, "').dataTable(",
						init, ").fnSetFilteringDelay(188);"));
		...
		if (!selector.isEmpty()) {
			...
			list.add(join("  $('", escapeJavaScript(selector), "').dataTable(",
					init, ").fnSetFilteringDelay(288);"));
	...
	public static StringBuilder tableInit() {
		return new StringBuilder("{bJQueryUI:true, ").append(
				"sPaginationType: 'full_numbers', iDisplayLength:20, ").append(
