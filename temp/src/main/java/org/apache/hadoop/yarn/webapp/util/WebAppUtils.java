...
public class WebAppUtils {
	...
	public static void setNMWebAppHostNameAndPort(Configuration conf,
			String hostName, int port) {
		if (YarnConfiguration.useHttps(conf)) {
			conf.set(YarnConfiguration.NM_WEBAPP_HTTPS_ADDRESS, hostName + ":"
					+ port);
		} else {
	...
	public static List<String> getProxyHostsAndPortsForAmFilter(
		...
		if (proxyAddr == null || proxyAddr.isEmpty()) {
			...
			if (HAUtil.isHAEnabled(conf)) {
				List<String> haAddrs = RMHAUtils
						.getRMHAWebappAddresses(new YarnConfiguration(conf));
				for (String addr : haAddrs) {
					try {
						InetSocketAddress socketAddr = NetUtils
								.createSocketAddr(addr);
			...
			// If couldn't resolve any of the addresses or not using RM HA,
			// fallback
			if (addrs.isEmpty()) {
	...
	public static String getResolvedRMWebAppURLWithoutScheme(
			Configuration conf, Policy httpPolicy) {
	...
	public static String getAHSWebAppURLWithoutScheme(Configuration conf) {
		if (YarnConfiguration.useHttps(conf)) {
			return conf
					.get(YarnConfiguration.TIMELINE_SERVICE_WEBAPP_HTTPS_ADDRESS,
	...
	public static String getAggregatedLogURL(String serverHttpAddress,
		...
		if (serverHttpAddress == null || serverHttpAddress.isEmpty()
				...
				|| entity == null || entity.isEmpty() || user == null
				|| user.isEmpty()) {
			return null;
		}
		return PATH_JOINER.join(serverHttpAddress, "applicationhistory",
				"logs", allocatedNode, containerId, entity, user);
	}
