 ...
 * queue. It provides a common interface so that algorithms such as fair sharing
 * can be applied both within a queue and across queues. There are currently two
 * types of Schedulables: JobSchedulables, which represent a single job, and
 * QueueSchedulables, which allocate among jobs in their queue.
 * 
 ...
 * assignTask(). 2) It provides information about the job/queue to the
 * scheduler, including: - Demand (maximum number of tasks required) - Number of
 * currently running tasks - Minimum share (for queues) - Job/queue weight (for
 * fair sharing) - Start time and priority (for FIFO) 3) It can be assigned a
 * fair share, for use with fair scheduling.
 * 
...
public abstract class Schedulable {
	...
	public String toString() {
		return String.format("[%s, demand=%s, running=%s, share=%s, w=%s]",
				getName(), getDemand(), getResourceUsage(), fairShare,
				getWeights());
	}
}
