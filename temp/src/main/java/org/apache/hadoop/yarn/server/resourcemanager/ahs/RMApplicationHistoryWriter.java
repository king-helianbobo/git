...
public class RMApplicationHistoryWriter extends CompositeService {
	...
	protected ApplicationHistoryStore createApplicationHistoryStore(
		...
		if (historyServiceEnabled) {
			try {
				Class<? extends ApplicationHistoryStore> storeClass = conf
						.getClass(YarnConfiguration.APPLICATION_HISTORY_STORE,
		...
		if (historyServiceEnabled) {
			dispatcher.getEventHandler().handle(
					new WritingApplicationStartEvent(app.getApplicationId(),
							ApplicationStartData.newInstance(
									app.getApplicationId(), app.getName(),
									app.getApplicationType(), app.getQueue(),
									app.getUser(), app.getSubmitTime(),
									app.getStartTime())));
		}
	}
	...
	public void applicationFinished(RMApp app, RMAppState finalState) {
		if (historyServiceEnabled) {
			dispatcher.getEventHandler()
					.handle(new WritingApplicationFinishEvent(app
							.getApplicationId(), ApplicationFinishData
							.newInstance(app.getApplicationId(), app
									.getFinishTime(), app.getDiagnostics()
									.toString(), app
									.getFinalApplicationStatus(), RMServerUtils
	...
	public void applicationAttemptStarted(RMAppAttempt appAttempt) {
		if (historyServiceEnabled) {
			dispatcher.getEventHandler().handle(
					new WritingApplicationAttemptStartEvent(appAttempt
							.getAppAttemptId(), ApplicationAttemptStartData
							.newInstance(appAttempt.getAppAttemptId(),
									appAttempt.getHost(), appAttempt
											.getRpcPort(), appAttempt
	...
	public void applicationAttemptFinished(RMAppAttempt appAttempt,
		...
		if (historyServiceEnabled) {
			dispatcher
					.getEventHandler()
					.handle(new WritingApplicationAttemptFinishEvent(appAttempt
							.getAppAttemptId(),
							ApplicationAttemptFinishData.newInstance(appAttempt
									.getAppAttemptId(), appAttempt
									.getDiagnostics().toString(), appAttempt
									.getTrackingUrl(), appAttempt
									.getFinalApplicationStatus(), RMServerUtils
									.createApplicationAttemptState(finalState))));
		}
	}
		...
		if (historyServiceEnabled) {
			dispatcher.getEventHandler().handle(
					new WritingContainerStartEvent(container.getContainerId(),
							ContainerStartData.newInstance(
		...
		if (historyServiceEnabled) {
			dispatcher.getEventHandler().handle(
					new WritingContainerFinishEvent(container.getContainerId(),
							ContainerFinishData.newInstance(
									container.getContainerId(),
	...
	protected static class MultiThreadedDispatcher extends CompositeService
		...
		private class CompositEventHandler implements EventHandler<Event> {
			...
			public void handle(Event event) {
				// Use hashCode (of ApplicationId) to dispatch the event to the
				// child
				// dispatcher, such that all the writing events of one
				// application will
				// be handled by one thread, the scheduled order of the these
				// events
				// will be preserved
