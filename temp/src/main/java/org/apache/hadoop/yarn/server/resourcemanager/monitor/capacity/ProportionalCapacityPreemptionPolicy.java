 ...
 * paired with the {@code CapacityScheduler}. At every invocation of
 * {@code editSchedule()} it computes the ideal amount of resources assigned to
 * each queue (for each queue in the hierarchy), and determines whether
 * preemption is needed. Overcapacity is distributed among queues in a weighted
 * fair manner, where the weight is the amount of guaranteed capacity for the
 * queue. Based on this ideal assignment it determines whether preemption is
 * required and select a set of containers from each application that would be
 * killed if the corresponding amount of resources is not freed up by the
 * application.
 * 
 * If not in {@code observeOnly} mode, it triggers preemption requests via a
 * {@link ContainerPreemptEvent} that the {@code ResourceManager} will ensure to
 * deliver to the application (or to execute).
 * 
...
public class ProportionalCapacityPreemptionPolicy implements
	 ...
	 * {@link #MAX_IGNORED_OVER_CAPACITY}). For example, a termination factor of
	 * 0.5 will reclaim almost 95% of resources within 5 *
	 * {@link #WAIT_TIME_BEFORE_KILL}, even absent natural termination.
	 */
	...
	public void init(Configuration config,
		...
		maxIgnoredOverCapacity = config.getDouble(MAX_IGNORED_OVER_CAPACITY,
				0.1);
		naturalTerminationFactor = config.getDouble(NATURAL_TERMINATION_FACTOR,
				0.2);
		...
		percentageClusterPreemptionAllowed = config.getFloat(
				TOTAL_PREEMPTION_PER_ROUND, (float) 0.1);
	...
	public void editSchedule() {
		...
		Resource clusterResources = Resources.clone(scheduler
				.getClusterResources());
	...
	private void containerBasedPreemptOrKill(CSQueue root,
		...
		List<TempQueue> queues = recursivelyComputeIdealAssignment(tRoot,
				totalPreemptionAllowed);

		// based on ideal allocation select containers to be preempted from each
		// queue and each application
		Map<ApplicationAttemptId, Set<RMContainer>> toPreempt = getContainersToPreempt(
		...
		for (Map.Entry<ApplicationAttemptId, Set<RMContainer>> e : toPreempt
			...
			for (RMContainer container : e.getValue()) {
				...
				if (preempted.get(container) != null
						&& preempted.get(container) + maxWaitTime < clock
								.getTime()) {
					// kill it
					dispatcher
							.handle(new ContainerPreemptEvent(e.getKey(),
									container,
									ContainerPreemptEventType.KILL_CONTAINER));
					preempted.remove(container);
				} else {
					// otherwise just send preemption events
					dispatcher.handle(new ContainerPreemptEvent(e.getKey(),
							container,
		...
		for (Iterator<RMContainer> i = preempted.keySet().iterator(); i
				.hasNext();) {
	...
	/**
	 * This method recursively computes the ideal assignment of resources to
	 * each level of the hierarchy. This ensures that leafs that are
	 * over-capacity but with parents within capacity will not be preempted.
	 * Preemptions are allowed within each subtree according to local over/under
	 * capacity.
	 * 
	 * @param root
	 ...
	 * {@code List<TempQueue>}) the ideal assignment of resources. This is done
	 ...
	 *            resource calculator
	 * @param queues
	 *            a list of cloned queues to be assigned capacity to (this is an
	 *            out param)
	 * @param totalPreemptionAllowed
	...
	private void computeIdealResourceDistribution(ResourceCalculator rc,
		...
		// assign all cluster resources until no more demand, or no resources
		// are left
		while (!qAlloc.isEmpty()
			...
			// we compute normalizedGuarantees capacity based on currently
			// active
			// queues
			resetCapacity(rc, unassigned, qAlloc);

			// offer for each queue their capacity first and in following
			// invocations
			// their share of over-capacity
			for (Iterator<TempQueue> i = qAlloc.iterator(); i.hasNext();) {
				TempQueue sub = i.next();
				Resource wQavail = Resources.multiply(unassigned,
						sub.normalizedGuarantee);
				Resource wQidle = sub.offer(wQavail, rc, tot_guarant);
				Resource wQdone = Resources.subtract(wQavail, wQidle);
				// if the queue returned a value > 0 it means it is fully
				// satisfied
				// and it is removed from the list of active queues qAlloc
				if (!Resources.greaterThan(rc, tot_guarant, wQdone,
						Resources.none())) {
					i.remove();
				}
		...
		for (TempQueue t : queues) {
			if (Resources.greaterThan(rc, tot_guarant, t.current,
					t.idealAssigned)) {
	...
	/**
	 * Based a resource preemption target drop reservations of containers and if
	 * necessary select containers for preemption from applications in each
	...
	private Map<ApplicationAttemptId, Set<RMContainer>> getContainersToPreempt(
		...
		for (TempQueue qT : queues) {
			...
			if (Resources.greaterThan(rc, clusterResource, qT.current,
					Resources.multiply(qT.guaranteed,
							1.0 + maxIgnoredOverCapacity))) {
	...
	private Set<RMContainer> preemptFrom(FiCaSchedulerApp app,
		...
		List<RMContainer> reservations = new ArrayList<RMContainer>(
				app.getReservedContainers());
	...
	static void sortContainers(List<RMContainer> containers) {
		Collections.sort(containers, new Comparator<RMContainer>() {
			...
			public int compare(RMContainer a, RMContainer b) {
				Comparator<Priority> c = new org.apache.hadoop.yarn.server.resourcemanager.resource.Priority.Comparator();
				int priorityComp = c.compare(b.getContainer().getPriority(), a
						.getContainer().getPriority());
				if (priorityComp != 0) {
					return priorityComp;
				}
	...
	private TempQueue cloneQueues(CSQueue root, Resource clusterResources) {
		...
		synchronized (root) {
			...
			Resource guaranteed = Resources.multiply(clusterResources,
					root.getAbsoluteCapacity());
			if (root instanceof LeafQueue) {
				LeafQueue l = (LeafQueue) root;
				Resource pending = l.getTotalResourcePending();
				ret = new TempQueue(root.getQueueName(), current, pending,
						guaranteed);
				ret.setLeafQueue(l);
			} else {
	...
	static class TempQueue {
		...
		Resource offer(Resource avail, ResourceCalculator rc,
			...
			Resource accepted = Resources.min(rc, clusterResource, avail,
					Resources.subtract(Resources.add(current, pending),
							idealAssigned));
		...
		public void assignPreemption(float scalingFactor,
			...
			if (Resources.greaterThan(rc, clusterResource, current,
					idealAssigned)) {
				toBePreempted = Resources.multiply(
						Resources.subtract(current, idealAssigned),
						scalingFactor);
			} else {
		...
		void appendLogString(StringBuilder sb) {
			sb.append(queueName).append(", ").append(current.getMemory())
					.append(", ").append(current.getVirtualCores())
					.append(", ").append(pending.getMemory()).append(", ")
