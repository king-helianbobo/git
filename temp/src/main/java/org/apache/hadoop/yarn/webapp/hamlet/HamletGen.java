...
/**
 * Generates a specific hamlet implementation class from a spec class using a
 * generic hamlet implementation class.
 */
...
public class HamletGen {
	...
	public void generate(Class<?> specClass, Class<?> implClass,
		...
		puts(0, "\n", "public class ", hamlet, " extends ", implClassName,
				" implements ", specClass.getSimpleName(),
				"._Html {\n",
				"  public ",
				hamlet,
				"(PrintWriter out, int nestLevel,",
				" boolean wasInline) {\n",
		...
		for (Class<?> cls : spec.getClasses()) {
			...
			if (isElement(className)) {
				...
				puts(indent, "\n", "public class ", className, "<T extends _>",
						" extends EImp<T> implements ", specName, ".",
						className, " {\n", "  public ", className,
						"(String name, T parent,", " EnumSet<EOpt> opts) {\n",
	...
	void genFactoryMethod(String retName, String methodName, int indent) {
		puts(indent, "\n", "private <T extends _> ", retName, "<T> ",
				methodName, "_(T e, boolean inline) {\n", "  return new ",
				retName, "<T>(\"", retName.toLowerCase(Locale.US),
	...
	boolean isInline(String container, String className) {
		if ((container.equals("BODY") || container.equals(hamlet)
				...
				&& (className.equals("INS") || className.equals("DEL") || className
						.equals("SCRIPT"))) {
			return false;
		}
	...
	void genCurElementMethod(String className, Method method, int indent) {
		...
		if (params.length == 0) {
			...
			if (methodName.equals("base")) {
				...
				puts(0, "String href) {");
				puts(indent, "  return setLinkHref(", methodName,
						"(), href)._();\n", "}");
				...
				if (params[0].getSimpleName().equals("Class")) {
					...
					puts(0, "Object... lines) {");
					puts(indent, "  _p(", needsEscaping(className),
							", lines);\n", "  return this;\n", "}");
				}
	...
	public static void main(String[] args) throws Exception {
		...
		new HamletGen().generate(specClass, implClass, outputClass,
				outputPackage);
	}
}
