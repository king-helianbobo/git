...
public class FifoSchedulerInfo extends SchedulerInfo {
	...
	public FifoSchedulerInfo(final ResourceManager rm) {
		...
		this.minQueueMemoryCapacity = fs.getMinimumResourceCapability()
				.getMemory();
		this.maxQueueMemoryCapacity = fs.getMaximumResourceCapability()
				.getMemory();
		...
		for (RMNode ni : rmContext.getRMNodes().values()) {
			...
			this.numContainers += fs.getNodeReport(ni.getNodeID())
					.getNumContainers();
		}
	}
