...
public class FileSystemRMStateStore extends RMStateStore {
	...
	public static final Log LOG = LogFactory
			.getLog(FileSystemRMStateStore.class);
	...
	public synchronized void initInternal(Configuration conf) throws Exception {
		fsWorkingPath = new Path(
				conf.get(YarnConfiguration.FS_RM_STATE_STORE_URI));
	...
	protected synchronized void startInternal() throws Exception {
		// create filesystem only now, as part of service-start. By this time,
		// RM is
	...
	private void loadRMAppState(RMState rmState) throws Exception {
		try {
			...
			for (FileStatus appDir : fs.listStatus(rmAppRoot)) {
				...
				for (FileStatus childNodeStatus : fs.listStatus(appDir
						.getPath())) {
					...
					byte[] childData = readFile(childNodeStatus.getPath(),
							childNodeStatus.getLen());
					if (childNodeName.startsWith(ApplicationId.appIdStrPrefix)) {
						// application
						if (LOG.isDebugEnabled()) {
							LOG.debug("Loading application from node: "
									+ childNodeName);
						}
						...
						// assert child node name is same as actual
						// applicationId
						assert appId
								.equals(appState.context.getApplicationId());
						...
						// assert child node name is same as application attempt
						// id
						assert attemptId.equals(attemptState.getAttemptId());
						attempts.add(attemptState);
					} else {
						LOG.info("Unknown child node with name: "
								+ childNodeName);
					}
				}
			}
			...
			for (ApplicationAttemptState attemptState : attempts) {
				ApplicationId appId = attemptState.getAttemptId()
						.getApplicationId();
	...
	private void checkAndResumeUpdateOperation(Path path) throws Exception {
		...
		for (FileStatus newChildNodeStatus : newChildNodes) {
			...
			String childNodeName = newChildNodeName.substring(0,
					newChildNodeName.length() - ".new".length());
			Path childNodePath = new Path(newChildNodeStatus.getPath()
					.getParent(), childNodeName);
	...
	private void loadRMDTSecretManagerState(RMState rmState) throws Exception {
		...
		for (FileStatus childNodeStatus : childNodes) {
			...
			if (childNodeName
					.startsWith(DELEGATION_TOKEN_SEQUENCE_NUMBER_PREFIX)) {
				rmState.rmSecretManagerState.dtSequenceNumber = Integer
						.parseInt(childNodeName.split("_")[1]);
				continue;
			}

			Path childNodePath = getNodePath(rmDTSecretManagerRoot,
					childNodeName);
	...
	public synchronized void storeRMDelegationTokenAndSequenceNumberState(
		...
		storeOrUpdateRMDelegationTokenAndSequenceNumberState(identifier,
				renewDate, latestSequenceNumber, false);
	}
	...
	private void storeOrUpdateRMDelegationTokenAndSequenceNumberState(
		...
		if (isUpdate) {
			LOG.info("Updating RMDelegationToken_"
					+ identifier.getSequenceNumber());
			updateFile(nodeCreatePath, os.toByteArray());
		} else {
			LOG.info("Storing RMDelegationToken_"
					+ identifier.getSequenceNumber());
	...
	private void writeFile(Path outputPath, byte[] data) throws Exception {
		Path tempPath = new Path(outputPath.getParent(), outputPath.getName()
				+ ".tmp");
		FSDataOutputStream fsOut = null;
		// This file will be overwritten when app/attempt finishes for saving
		// the
		// final status.
	...
	/*
	 * In order to make this update atomic as a part of write we will first
	 * write data to .new file and then rename it. Here we are assuming that
	 * rename is atomic for underlying file system.
	 */
