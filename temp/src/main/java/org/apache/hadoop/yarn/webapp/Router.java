...
class Router {
	...
	static final Pattern controllerRe = Pattern
			.compile("^/[A-Za-z_]\\w*(?:/.*)?");
	...
	static class Dest {
		...
		Dest(String path, Method method, Class<? extends Controller> cls,
			...
			this.pathParams = pathParams != null ? ImmutableList
					.copyOf(pathParams) : EMPTY_LIST;
	...
	/**
	 * Add a route to the router. e.g., add(GET, "/foo/show",
	 * FooController.class, "show", [name...]); The name list is from
	 * /foo/show/:name/...
	 */
	synchronized Dest add(WebApp.HTTP httpMethod, String path,
			Class<? extends Controller> cls, String action, List<String> names) {
		LOG.debug("adding {}({})->{}#{}", new Object[] { path, names, cls,
				action });
	...
	private Dest addController(WebApp.HTTP httpMethod, String path,
		...
		try {
			...
			// TODO: We may want to deal with methods that take parameters in
			// the future
	...
	private void addDefaultView(Dest dest) {
		...
		dest.defaultViewClass = find(View.class, dest.controllerClass
				.getPackage().getName(), join(controllerName + "View"));
	}
			...
			if (prefixMatches(dest, path)) {
				if (methodAllowed(method, dest)) {
					if (isGoodMatch(dest, path)) {
						LOG.debug("prefix match for {}: {}", lower.getKey(),
								dest.action);
						return dest;
					}
					return resolveAction(method, dest, path);
	...
	static boolean methodAllowed(WebApp.HTTP method, Dest dest) {
		...
		return dest.methods.contains(method)
				|| (dest.methods.size() == 1 && dest.methods
						.contains(WebApp.HTTP.GET));
	}

	static boolean prefixMatches(Dest dest, String path) {
		LOG.debug("checking prefix {}{} for path: {}", new Object[] {
				dest.prefix, dest.pathParams, path });
	...
	private Dest resolveDefault(WebApp.HTTP method, String path) {
		...
		if (cls == null) {
			throw new WebAppException(join(path, ": controller for ",
