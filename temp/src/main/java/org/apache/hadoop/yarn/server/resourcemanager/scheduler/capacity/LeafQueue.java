...
public class LeafQueue implements CSQueue {
	...
	public LeafQueue(CapacitySchedulerContext cs, String queueName,
		...
		this.metrics = old != null ? old.getMetrics() : QueueMetrics.forQueue(
				getQueuePath(), parent, cs.getConfiguration()
		...
		float capacity = (float) cs.getConfiguration().getCapacity(
				getQueuePath()) / 100;
		float absoluteCapacity = parent.getAbsoluteCapacity() * capacity;

		float maximumCapacity = (float) cs.getConfiguration()
				.getMaximumCapacity(getQueuePath()) / 100;
		float absoluteMaxCapacity = CSQueueUtils
				.computeAbsoluteMaximumCapacity(maximumCapacity, parent);

		int userLimit = cs.getConfiguration().getUserLimit(getQueuePath());
		float userLimitFactor = cs.getConfiguration().getUserLimitFactor(
				getQueuePath());

		int maxApplications = cs.getConfiguration()
				.getMaximumApplicationsPerQueue(getQueuePath());
		if (maxApplications < 0) {
			int maxSystemApps = cs.getConfiguration()
		...
		float maxAMResourcePerQueuePercent = cs.getConfiguration()
				.getMaximumApplicationMasterResourcePerQueuePercent(
						getQueuePath());
		int maxActiveApplications = CSQueueUtils.computeMaxActiveApplications(
				resourceCalculator, cs.getClusterResources(),
				this.minimumAllocation, maxAMResourcePerQueuePercent,
				absoluteMaxCapacity);
		...
		Map<QueueACL, AccessControlList> acls = cs.getConfiguration().getAcls(
				getQueuePath());

		setupQueueConfigs(cs.getClusterResources(), capacity, absoluteCapacity,
				maximumCapacity, absoluteMaxCapacity, userLimit,
				userLimitFactor, maxApplications, maxAMResourcePerQueuePercent,
				maxApplicationsPerUser, maxActiveApplications,
				maxActiveApplicationsPerUser, state, acls, cs
						.getConfiguration().getNodeLocalityDelay());

		if (LOG.isDebugEnabled()) {
			LOG.debug("LeafQueue:" + " name=" + queueName + ", fullname="
					+ getQueuePath());
		}
	...
	public synchronized List<QueueUserACLInfo> getQueueUserAclInfo(
		...
		QueueUserACLInfo userAclInfo = recordFactory
	...
	public synchronized ArrayList<UserInfo> getUsers() {
		...
		for (Map.Entry<String, User> entry : users.entrySet()) {
			usersToReturn.add(new UserInfo(entry.getKey(), Resources
					.clone(entry.getValue().consumed), entry.getValue()
					.getActiveApplications(), entry.getValue()
	...
	public synchronized void reinitialize(CSQueue newlyParsedQueue,
		...
		setupQueueConfigs(clusterResource, newlyParsedLeafQueue.capacity,
				...
				newlyParsedLeafQueue.userLimit,
				newlyParsedLeafQueue.userLimitFactor,
	...
	public void submitApplication(ApplicationId applicationId, String userName,
		...
		if (!hasAccess(QueueACL.SUBMIT_APPLICATIONS, userUgi)
			...
			throw new AccessControlException("User " + userName
					+ " cannot submit" + " applications to queue "
					+ getQueuePath());
		}
		...
		synchronized (this) {
			...
			if (getNumApplications() >= getMaxApplications()) {
				String msg = "Queue " + getQueuePath() + " already has "
						+ getNumApplications() + " applications,"
	...
	private synchronized void activateApplications() {
		for (Iterator<FiCaSchedulerApp> i = pendingApplications.iterator(); i
				.hasNext();) {
	...
	private synchronized void addApplicationAttempt(
		...
		LOG.info("Application added -" + " appId: "
				+ application.getApplicationId() + " user: " + user + ","
	...
	public void finishApplicationAttempt(FiCaSchedulerApp application,
		...
		// Careful! Locking order is important!
		synchronized (this) {
			removeApplicationAttempt(application,
	...
	public synchronized void removeApplicationAttempt(
		...
		LOG.info("Application removed -" + " appId: "
				+ application.getApplicationId() + " user: "
				+ application.getUser() + " queue: " + getQueueName()
				+ " #user-pending-applications: "
				+ user.getPendingApplications()
				+ " #user-active-applications: " + user.getActiveApplications()
				+ " #queue-pending-applications: "
	...
	private static final CSAssignment NULL_ASSIGNMENT = new CSAssignment(
			Resources.createResource(0, 0), NodeType.NODE_LOCAL);
	...
	private synchronized CSAssignment assignReservedContainer(
		...
		// Doesn't matter... since it's already charged for at time of
		// reservation
	...
	private synchronized boolean assignToQueue(Resource clusterResource,
			Resource required) {
		// Check how of the cluster's absolute capacity we are currently
		// using...
		float potentialNewCapacity = Resources.divide(resourceCalculator,
				clusterResource, Resources.add(usedResources, required),
				clusterResource);
		if (potentialNewCapacity > absoluteMaxCapacity) {
			LOG.info(getQueueName()
					...
					+ Resources.divide(resourceCalculator, clusterResource,
							usedResources, clusterResource) + " required "
					+ required + " potentialNewCapacity: "
					+ potentialNewCapacity + " ( " + " max-capacity: "
					+ absoluteMaxCapacity + ")");
			return false;
		}
		return true;
	...
	private Resource computeUserLimitAndSetHeadroom(
		...
		Resource headroom = Resources.subtract(Resources.min(
				resourceCalculator, clusterResource, userLimit, queueMaxCap),
				userConsumed);

		if (LOG.isDebugEnabled()) {
			LOG.debug("Headroom calculation for user " + user + ": "
					+ " userLimit=" + userLimit + " queueMaxCap=" + queueMaxCap
					+ " consumed=" + userConsumed + " headroom=" + headroom);
		}
	...
	private Resource computeUserLimit(FiCaSchedulerApp application,
		...
		final Resource queueCapacity = Resources.max(resourceCalculator,
				clusterResource, Resources.multiplyAndNormalizeUp(
						resourceCalculator, clusterResource, absoluteCapacity,
						minimumAllocation), required);

		Resource currentCapacity = Resources.lessThan(resourceCalculator,
				clusterResource, usedResources, queueCapacity) ? queueCapacity
		...
		Resource limit = Resources.roundUp(resourceCalculator, Resources.min(
				resourceCalculator, clusterResource, Resources.max(
						resourceCalculator, clusterResource, Resources
								.divideAndCeil(resourceCalculator,
										currentCapacity, activeUsers),
						Resources.divideAndCeil(resourceCalculator, Resources
								.multiplyAndRoundDown(currentCapacity,
										userLimit), 100)), Resources
						.multiplyAndRoundDown(queueCapacity, userLimitFactor)),
				minimumAllocation);

		if (LOG.isDebugEnabled()) {
			String userName = application.getUser();
			LOG.debug("User limit computation for " + userName + " in queue "
					+ getQueueName() + " userLimit=" + userLimit
					+ " userLimitFactor=" + userLimitFactor + " required: "
					+ required + " consumed: "
					+ getUser(userName).getConsumedResources() + " limit: "
					+ limit + " queueCapacity: " + queueCapacity
					+ " qconsumed: " + usedResources + " currentCapacity: "
					+ currentCapacity + " activeUsers: " + activeUsers
	...
	private synchronized boolean assignToUser(Resource clusterResource,
		...
		// Note: We aren't considering the current request since there is a
		// fixed
	...
	boolean needContainers(FiCaSchedulerApp application, Priority priority,
			Resource required) {
		int requiredContainers = application
				.getTotalRequiredResources(priority);
		...
		if (reservedContainers > 0) {
			...
			// Use percentage of node required to bias against large
			// containers...
			// Protect against corner case where you need the whole node with
			// Math.min(nodeFactor, minimumAllocationFactor)
			starvation = (int) ((application.getReReservations(priority) / (float) reservedContainers) * (1.0f - (Math
					.min(nodeFactor, getMinimumAllocationFactor()))));

			if (LOG.isDebugEnabled()) {
				LOG.debug("needsContainers:" + " app.#re-reserve="
						+ application.getReReservations(priority)
						+ " reserved=" + reservedContainers + " nodeFactor="
						+ nodeFactor + " minAllocFactor="
						+ getMinimumAllocationFactor() + " starvation="
						+ starvation);
			}
		}
	...
	private CSAssignment assignContainersOnNode(Resource clusterResource,
		...
		// Data-local
		ResourceRequest nodeLocalResourceRequest = application
		...
		// Rack-local
		ResourceRequest rackLocalResourceRequest = application
		...
		// Off-switch
		ResourceRequest offSwitchResourceRequest = application
				.getResourceRequest(priority, ResourceRequest.ANY);
		if (offSwitchResourceRequest != null) {
			...
			return new CSAssignment(assignOffSwitchContainers(clusterResource,
					...
					reservedContainer), NodeType.OFF_SWITCH);
		}
	...
	private Resource assignNodeLocalContainers(Resource clusterResource,
		...
		if (canAssign(application, priority, node, NodeType.NODE_LOCAL,
				reservedContainer)) {
			return assignContainer(clusterResource, node, application,
					priority, nodeLocalResourceRequest, NodeType.NODE_LOCAL,
					reservedContainer);
		}
	...
	private Resource assignRackLocalContainers(Resource clusterResource,
		...
		if (canAssign(application, priority, node, NodeType.RACK_LOCAL,
				reservedContainer)) {
			return assignContainer(clusterResource, node, application,
					priority, rackLocalResourceRequest, NodeType.RACK_LOCAL,
					reservedContainer);
		}
	...
	private Resource assignOffSwitchContainers(Resource clusterResource,
		...
		if (canAssign(application, priority, node, NodeType.OFF_SWITCH,
				reservedContainer)) {
			return assignContainer(clusterResource, node, application,
					priority, offSwitchResourceRequest, NodeType.OFF_SWITCH,
					reservedContainer);
		}
	...
	boolean canAssign(FiCaSchedulerApp application, Priority priority,
		...
		if (type == NodeType.OFF_SWITCH) {
			...
			float localityWaitFactor = application.getLocalityWaitFactor(
					priority, scheduler.getNumClusterNodes());
		...
		if (type == NodeType.RACK_LOCAL) {
			...
			return (Math.min(scheduler.getNumClusterNodes(),
					getNodeLocalityDelay()) < missedOpportunities);
		}
	...
	private Resource assignContainer(Resource clusterResource,
		...
		if (!Resources.fitsIn(capability, totalResource)) {
			LOG.warn("Node : " + node.getNodeID()
					+ " does not have sufficient resource for request : "
					+ request + " node total capability : "
		...
		int availableContainers = resourceCalculator
				.computeAvailableContainers(available, capability);
		if (availableContainers > 0) {
			...
			LOG.info("assignedContainer" + " application="
					+ application.getApplicationId() + " container="
					+ container + " containerId=" + container.getId()
					+ " queue=" + this + " usedCapacity=" + getUsedCapacity()
			...
			LOG.info("Reserved container " + " application="
					...
					+ request.getCapability() + " queue=" + this.toString()
	...
	private void reserve(FiCaSchedulerApp application, Priority priority,
		...
		// Inform the application
		rmContainer = application.reserve(node, priority, rmContainer,
				container);
	...
	public void completedContainer(Resource clusterResource,
		...
		if (application != null) {
			...
			synchronized (this) {
				...
				// Note: It's safe to assume that all state changes to
				// RMContainer
				// happen under scheduler's lock...
				// So, this is, in effect, a transaction across application &
				// node
				if (rmContainer.getState() == RMContainerState.RESERVED) {
					removed = unreserve(application,
							rmContainer.getReservedPriority(), node,
							rmContainer);
				} else {
				...
				if (removed) {
					...
					LOG.info("completedContainer" + " container=" + container
							...
							+ " queue=" + this + " usedCapacity="
							+ getUsedCapacity() + " absoluteUsedCapacity="
							+ getAbsoluteUsedCapacity() + " used="
	...
	synchronized void allocateResource(Resource clusterResource,
		...
		if (LOG.isDebugEnabled()) {
			LOG.info(getQueueName() + " user=" + userName + " used="
					+ usedResources + " numContainers=" + numContainers
	...
	synchronized void releaseResource(Resource clusterResource,
		...
		LOG.info(getQueueName() + " used=" + usedResources + " numContainers="
	...
	public synchronized void updateClusterResource(Resource clusterResource) {
		...
		maxActiveApplications = CSQueueUtils.computeMaxActiveApplications(
				resourceCalculator, clusterResource, minimumAllocation,
				maxAMResourcePerQueuePercent, absoluteMaxCapacity);
		maxActiveAppsUsingAbsCap = CSQueueUtils.computeMaxActiveApplications(
				resourceCalculator, clusterResource, minimumAllocation,
				maxAMResourcePerQueuePercent, absoluteCapacity);
