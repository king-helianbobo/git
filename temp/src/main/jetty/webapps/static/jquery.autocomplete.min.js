(function(g) {
    g.fn.extend({"AutoComplete":function(q) {
	...
	this.option=g.extend(false, {
	    "width":"auto","maxHeight":null,"itemHeight":null,"listStyle":"normal","listDirection":"down","data":[],"ajaxDataType":"json","ajaxParams":{},"ajaxTimeout":3000,"ajaxType":"GET","maxItems":20,"matchHandler":n,"emphasisHandler":null,"createItemHandler":null,"beforeLoadDataHandler":null,"afterSelectedHandler":null,"async":false,"emphasis":true,"onerror":null,"term":" "
	},r);
	b.apply(this,[q]);
	j.apply(this)
    };
	...
	this.inputView.attr("autocomplete","off").keyup(this._keyup=function(s) {
	    switch(s.keyCode){
	    case 13:case 16:case 17:case 37:case 38:case 39:case 40:break;case 27:
		e.apply(r);
		break;
	    default:
		if(r.option.async) {
    ...
    var i=function(q) {
	var s=this;
	var r=this.searchView.find("ul").empty();
	...
	if(typeof(this.option.width)==="string"&&this.option.width.toLowerCase()==="auto"){
	    console.log(this.inputView.outerWidth()-2);
	...
	if(q.size()) {
	    var s=q.data("data");
	    var variable = s.value.replace(/<[^>].*?>/g,"");
	    //s = s.replace(/<[^>].*?>/g,"");
	    // alert(variable);
	    //skipUrl(s.value);
	    //this.inputView.val(s.value);
	    skipUrl(variable);
	    this.inputView.val(variable);
	...
	var s=this,t=[],r={"async":false,"dataType":s.option.ajaxDataType,"type":s.option.ajaxType,"timeout":this.option.ajaxTimeout,"success":function(u,w,v) {
		...
		g(u).find("item").each(function(){
			...
			if(s.option.ajaxDataType==="json"){t=u}else{throw"遇到未知的ajaxDataType参数！"}
		    }
	...
	g.ajax(this.option.data,r);return t
    };
    var h= function() {
	...
	if(g.isFunction(this.option.beforeLoadDataHandler)){
		...
		o.apply(this,["调用beforeLoadDataHandler错误:"+v]);return
	    }
	}
        if(r) {
	    ...
	    console.log(g.trim(s));
	    console.log(t.option.term);
	    if(typeof(t.option.term)==="string") {
		// if(t.option.matchHandler.apply(t,[str,t.option.term])
		//   ) {
	    	    q.push(x)
		//}
	    }
