...
$(document).ready(function(){
	...
	source: function( request, response ) {
		...
		success: function( data ) {
			...
			return {
			    //			                label: item[0],
			    label: item.replace(/<em>/g,"<span class=red>").replace(/<.?em>/g,"</span>"),
			    value: item.replace(/<em>/g,"").replace(/<.?em>/g,"")
			    //"<span class=red>", "</span>"
			    //			                value: item[0].replace(/<span class=red>/g,"").replace(/<.?span>/g,"")
			    // value: item.replace(/<em class=red>/g,"").replace(/<.?span>/g,"")
			}
		    }));
		}
	    });
	...
	minLength: 1,
	position: {my: "left bottom", at: "left top", of:"#inputpanel", collision: "none" },
	select: function( event, ui ) {
	    $("#text-input").attr('value',this.value)
	},
	open: function() {
	},
	close: function() {
