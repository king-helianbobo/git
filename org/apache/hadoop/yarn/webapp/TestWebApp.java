...
public class TestWebApp {
	...
	static class TablesView extends HtmlPage {
		...
		public void render(Page.HTML<_> html) {
			...
			set(initID(DATATABLES, "t2"),
					join("{bJQueryUI:true, sDom:'t',",
							"aoColumns:[null, {bSortable:false, bSearchable:false}]}"));
			LOG.info(initID(DATATABLES, "t3"));
			set(initID(DATATABLES, "t3"), "{bJQueryUI:true, sDom:'t'}");
			set(initID(DATATABLES, "t4"), "{bJQueryUI:true, sDom:'t'}");
			html.title("Test DataTables")
					.link("/static/yarn.css")
					._(JQueryUI.class)
					.style(".wrapper { padding: 1em }",
							".wrapper h2 { margin: 0.5em 0 }",
							".dataTables_wrapper { min-height: 1em }")
					.div(".wrapper").h2("Default table init").table("#t1")
					.thead().tr().th("Column1").th("Column2")._()._().tbody()
					.tr().td("c1r1").td("c2r1")._().tr().td("c1r2").td("c2r2")
					._()._()._().h2("嵌套表").div(_INFO_WRAP).table("#t2").thead()
					.tr().th(_TH, "Column1")
					.th(_TH, "Column2")
					._()
					._()
					...
					td().$class(C_TABLE).table("#t3").thead().tr().th("子列1")
					.th("子列2")._()._().tbody().tr().td("subc1r1").td("subc2r1")
					._().tr().td("哈哈1").td("哈哈2")._()._()._()._()
					._()
					.tr()
					.td("r2")
					. // ditto
					td().$class(C_TABLE).table("#t4").thead().tr()
					.th("SubColumn1").th("SubColumn2")._()._().tbody().tr()
					.td("subc1r1").td("subc2r1")._().tr().td("subc1r2")
					.td("subc2r2")._()._()._()._()._()._()._()._()._()._();
		}
	}
	...
	public void testCustomRoutes() throws Exception {
		WebApp app = WebApps.$for("test", TestWebApp.class, this, "ws").start(
				new WebApp() {
					@Override
					public void setup() {
	...
	public static void main(String[] args) throws Exception {
		...
		WebApps.$for("", new TestWebApp()).at(9999).inDevMode().start()
				.joinThread();
		// start(new WebApp() {
