...
public class TestSubmitApplicationWithRMHA extends RMHATestBase {
	...
	public void testHandleRMHABeforeSubmitApplicationCallWithSavedApplicationState()
		...
		RMApp app1 = rm2.submitApp(200, "", UserGroupInformation
	...
	private void verifySubmitApp(MockRM rm, RMApp app,
		...
		int count = 0;
		while (true) {
			YarnApplicationState state = rm.getApplicationReport(
					app.getApplicationId()).getYarnApplicationState();
	...
	public void testHandleRMHAafterSubmitApplicationCallWithSavedApplicationState()
		...
		// applicationState.
		ApplicationReport appReport = rm2.getApplicationReport(app0
				.getApplicationId());

		// verify previous submission is successful.
		Assert.assertTrue(appReport.getYarnApplicationState() == YarnApplicationState.ACCEPTED
	...
	public void testHandleRMHAafterSubmitApplicationCallWithoutSavedApplicationState()
		...
		RMApp app0 = rm1.submitApp(200, "", UserGroupInformation
				.getCurrentUser().getShortUserName(), null, false, null,
				configuration.getInt(YarnConfiguration.RM_AM_MAX_ATTEMPTS,
						YarnConfiguration.DEFAULT_RM_AM_MAX_ATTEMPTS), null,
				null, false, false);
		...
		// Expect ApplicationNotFoundException by calling
		...
		// Submit the application with previous ApplicationId to current active
		// RM
		// This will mimic the similar behavior of YarnClient which will
		// re-submit
		...
		RMApp app1 = rm2.submitApp(200, "", UserGroupInformation
				...
				configuration.getInt(YarnConfiguration.RM_AM_MAX_ATTEMPTS,
						YarnConfiguration.DEFAULT_RM_AM_MAX_ATTEMPTS), null,
				null, false, false, true, app0.getApplicationId());

		verifySubmitApp(rm2, app1, app0.getApplicationId());
	}

	/**
	 * Test multiple calls of getApplicationReport, to make sure it is
	 * idempotent
	 */
	@Test
	public void testGetApplicationReportIdempotent() throws Exception {
		...
		ApplicationReport appReport1 = rm1.getApplicationReport(app
				.getApplicationId());
		Assert.assertTrue(appReport1.getYarnApplicationState() == YarnApplicationState.ACCEPTED
				|| appReport1.getYarnApplicationState() == YarnApplicationState.SUBMITTED);

		// call getApplicationReport again
		ApplicationReport appReport2 = rm1.getApplicationReport(app
				.getApplicationId());
		...
		ApplicationReport appReport3 = rm2.getApplicationReport(app
				.getApplicationId());
		Assert.assertEquals(appReport1.getApplicationId(),
		...
		// call getApplicationReport again
		ApplicationReport appReport4 = rm2.getApplicationReport(app
				.getApplicationId());
	...
	public void testHandleRMHADuringSubmitApplicationCallWithoutSavedApplicationState()
		...
		// When failover happens, the RMStateStore has not saved
		// applicationState.
		...
		// submitApplication() when failover happens during the submission
		// process
