...
public class TestApplicationCleanup {
	...
	public void testAppCleanup() throws Exception {
		...
		NodeHeartbeatResponse resp = nm1.nodeHeartbeat(
				attempt.getAppAttemptId(), 1, ContainerState.COMPLETE);
		...
		while ((numCleanedContainers < 2 || numCleanedApps < 1)
			...
			Thread.sleep(100);
			resp = nm1.nodeHeartbeat(true);
			List<ContainerId> deltaContainersToCleanup = resp
					.getContainersToCleanup();
			List<ApplicationId> deltaAppsToCleanup = resp
	...
	public void testContainerCleanup() throws Exception {
		...
		// simulate the situation that can happen if the NM reports that
		// container
		...
		contsToClean = resp.getContainersToCleanup();
		cleanedConts = contsToClean.size();
		// The cleanup list won't be instantaneous as it is given out by
		// scheduler
		// and not RMNodeImpl.
		waitCount = 0;
		while (cleanedConts < 1 && waitCount++ < 200) {
