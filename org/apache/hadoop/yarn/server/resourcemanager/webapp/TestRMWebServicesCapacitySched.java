...
public class TestRMWebServicesCapacitySched extends JerseyTest {
			...
			for (int j = 0; j < children.getLength(); j++) {
				...
				if (qElem.getTagName().equals("queues")) {
					...
					for (int k = 0; k < qListInfos.getLength(); k++) {
						...
						String qName2 = WebServicesTestUtils.getXmlString(
								qElem2, "queueName");
						String q2 = CapacitySchedulerConfiguration.ROOT + "."
								+ qName2;
	...
	public void verifySubQueueXML(Element qElem, String q,
		...
		qi.usedCapacity = WebServicesTestUtils.getXmlFloat(qElem,
				"usedCapacity");
		qi.maxCapacity = WebServicesTestUtils.getXmlFloat(qElem, "maxCapacity");
		qi.absoluteCapacity = WebServicesTestUtils.getXmlFloat(qElem,
				"absoluteCapacity");
		qi.absoluteMaxCapacity = WebServicesTestUtils.getXmlFloat(qElem,
				"absoluteMaxCapacity");
		qi.absoluteUsedCapacity = WebServicesTestUtils.getXmlFloat(qElem,
				"absoluteUsedCapacity");
		qi.numApplications = WebServicesTestUtils.getXmlInt(qElem,
				"numApplications");
		...
		if (hasSubQueues) {
			for (int j = 0; j < children.getLength(); j++) {
				...
				if (qElem2.getTagName().equals("queues")) {
					...
					for (int k = 0; k < qListInfos.getLength(); k++) {
						...
						String qName3 = WebServicesTestUtils.getXmlString(
								qElem3, "queueName");
						String q3 = q + "." + qName3;
			...
			lqi.numActiveApplications = WebServicesTestUtils.getXmlInt(qElem,
					"numActiveApplications");
			lqi.numPendingApplications = WebServicesTestUtils.getXmlInt(qElem,
					"numPendingApplications");
			lqi.numContainers = WebServicesTestUtils.getXmlInt(qElem,
					"numContainers");
			lqi.maxApplications = WebServicesTestUtils.getXmlInt(qElem,
					"maxApplications");
			lqi.maxApplicationsPerUser = WebServicesTestUtils.getXmlInt(qElem,
					"maxApplicationsPerUser");
			lqi.maxActiveApplications = WebServicesTestUtils.getXmlInt(qElem,
					"maxActiveApplications");
			lqi.maxActiveApplicationsPerUser = WebServicesTestUtils.getXmlInt(
					qElem, "maxActiveApplicationsPerUser");
			lqi.userLimit = WebServicesTestUtils.getXmlInt(qElem, "userLimit");
			lqi.userLimitFactor = WebServicesTestUtils.getXmlFloat(qElem,
					"userLimitFactor");
	...
	private void verifyClusterSchedulerGeneric(String type, float usedCapacity,
			float capacity, float maxCapacity, String queueName)
			throws Exception {
	...
	private void verifySubQueue(JSONObject info, String q,
		...
		qi.absoluteUsedCapacity = (float) info
		...
		if (isParentQueue) {
			...
			lqi.maxActiveApplicationsPerUser = info
	...
	private void verifySubQueueGeneric(String q, QueueInfo info,
		...
		assertEquals("absoluteCapacity doesn't match", parentAbsCapacity
	...
	private void verifyLeafQueueGeneric(String q, LeafQueueInfo info)
		...
		int expectedMaxAppsPerUser = (int) (expectedMaxApps
				* (info.userLimit / 100.0f) * info.userLimitFactor);
	...
	/**
	 * Test per user resources and resourcesUsed elements in the web services
	 * XML
	 * 
	 * @throws Exception
	 */
	...
	public void testPerUserResourcesXML() throws Exception {
		...
		try {
			...
			for (int i = 0; i < allUsers.getLength(); ++i) {
				...
				String queueName = getChildNodeByName(
						perUserResources.getParentNode(), "queueName")
						.getTextContent();
				if (queueName.equals("b1")) {
					// b1 should have two users (user1 and user2) which
					// submitted jobs
					assertEquals(2, perUserResources.getChildNodes()
							.getLength());
					...
					for (int j = 0; j < users.getLength(); ++j) {
						...
						String username = getChildNodeByName(user, "username")
								.getTextContent();
						assertTrue(username.equals("user1")
								|| username.equals("user2"));
						// Should be a parsable integer
						Integer.parseInt(getChildNodeByName(
								getChildNodeByName(user, "resourcesUsed"),
								"memory").getTextContent());
						Integer.parseInt(getChildNodeByName(user,
								"numActiveApplications").getTextContent());
						Integer.parseInt(getChildNodeByName(user,
								"numPendingApplications").getTextContent());
					}
				} else {
					// Queues other than b1 should have 0 users
					assertEquals(0, perUserResources.getChildNodes()
							.getLength());
				}
			}
			NodeList allResourcesUsed = dom
					.getElementsByTagName("resourcesUsed");
	...
	public void testPerUserResourcesJSON() throws Exception {
		...
		try {
			...
			JSONObject schedulerInfo = json.getJSONObject("scheduler")
	...
	public void testResourceInfo() {
		...
		// CapacitySchedulerPage and these RM WebServices + docs need to be
		// updated
		// eg. ResourceInfo
