...
public class TestRMWebServicesNodes extends JerseyTest {
	...
	public void testSingleNodeHelper(String nodeid, MockNM nm, String media)
		...
		WebResource r = resource();
		ClientResponse response = r.path("ws").path("v1").path("cluster")
				.path("nodes").path(nodeid).accept(media)
				.get(ClientResponse.class);
	...
	public void testNonexistNodeXML() throws JSONException, Exception {
		...
		try {
			...
			String message = WebServicesTestUtils.getXmlString(element,
					"message");
			String type = WebServicesTestUtils.getXmlString(element,
					"exception");
	...
	private void verifyNonexistNodeException(String message, String type,
		...
		assertTrue("exception message incorrect",
				"java.lang.Exception: nodeId, node_invalid:99, is not found"
						.matches(message));
		assertTrue("exception type incorrect",
				"NotFoundException".matches(type));
	...
	public void testInvalidNode() throws JSONException, Exception {
		...
		try {
			...
			WebServicesTestUtils
					.checkStringMatch(
							"exception message",
							"Invalid NodeId \\[node_invalid_foo\\]. Expected host:port",
							message);
			WebServicesTestUtils.checkStringMatch("exception type",
					"IllegalArgumentException", type);
			WebServicesTestUtils.checkStringMatch("exception classname",
	...
	public void verifyNodesXML(NodeList nodes, MockNM nm) throws JSONException,
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			verifyNodeInfoGeneric(nm, WebServicesTestUtils.getXmlString(
					element, "state"), WebServicesTestUtils.getXmlString(
					element, "rack"), WebServicesTestUtils.getXmlString(
					element, "id"), WebServicesTestUtils.getXmlString(element,
					"nodeHostName"), WebServicesTestUtils.getXmlString(element,
					"nodeHTTPAddress"), WebServicesTestUtils.getXmlLong(
					element, "lastHealthUpdate"),
	...
	public void verifyNodeInfo(JSONObject nodeInfo, MockNM nm)
		...
		verifyNodeInfoGeneric(nm, nodeInfo.getString("state"),
				...
				nodeInfo.getString("healthReport"),
				nodeInfo.getInt("numContainers"),
				nodeInfo.getLong("usedMemoryMB"),
				nodeInfo.getLong("availMemoryMB"),
	...
	public void verifyNodeInfoGeneric(MockNM nm, String state, String rack,
		...
		WebServicesTestUtils.checkStringMatch("state", node.getState()
				.toString(), state);
		WebServicesTestUtils.checkStringMatch("rack", node.getRackName(), rack);
		WebServicesTestUtils.checkStringMatch("id", nm.getNodeId().toString(),
				id);
