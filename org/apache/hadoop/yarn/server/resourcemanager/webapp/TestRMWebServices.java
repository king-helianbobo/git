...
public class TestRMWebServices extends JerseyTest {
	...
	public void testInvalidUri2() throws JSONException, Exception {
		...
		String responseStr = "";
		try {
			responseStr = r.accept(MediaType.APPLICATION_JSON)
					.get(String.class);
			fail("should have thrown exception on invalid uri");
		} catch (UniformInterfaceException ue) {
			ClientResponse response = ue.getResponse();
	...
	public void verifyClusterInfoXML(String xml) throws JSONException,
			Exception {
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			verifyClusterGeneric(
					WebServicesTestUtils.getXmlLong(element, "id"),
	...
	public void testClusterMetricsXML() throws JSONException, Exception {
		WebResource r = resource();
		ClientResponse response = r.path("ws").path("v1").path("cluster")
				.path("metrics").accept("application/xml")
				.get(ClientResponse.class);
		assertEquals(MediaType.APPLICATION_XML_TYPE, response.getType());
		String xml = response.getEntity(String.class);
	...
	public void verifyClusterMetricsXML(String xml) throws JSONException,
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			verifyClusterMetrics(WebServicesTestUtils.getXmlInt(element,
					"appsSubmitted"), WebServicesTestUtils.getXmlInt(element,
					"appsCompleted"), WebServicesTestUtils.getXmlInt(element,
					"reservedMB"), WebServicesTestUtils.getXmlInt(element,
					"availableMB"), WebServicesTestUtils.getXmlInt(element,
					"allocatedMB"), WebServicesTestUtils.getXmlInt(element,
					"containersAllocated"), WebServicesTestUtils.getXmlInt(
					element, "totalMB"), WebServicesTestUtils.getXmlInt(
					element, "totalNodes"), WebServicesTestUtils.getXmlInt(
					element, "lostNodes"), WebServicesTestUtils.getXmlInt(
					element, "unhealthyNodes"), WebServicesTestUtils.getXmlInt(
					element, "decommissionedNodes"),
	...
	public void verifyClusterMetrics(int submittedApps, int completedApps,
		...
		long totalMBExpect = metrics.getAvailableMB()
		...
		assertEquals("activeNodes doesn't match",
				clusterMetrics.getNumActiveNMs(), activeNodes);
	}
	...
	@Test
	public void testClusterSchedulerFifoDefault() throws JSONException,
			Exception {
		WebResource r = resource();
		ClientResponse response = r.path("ws").path("v1").path("cluster")
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			verifyClusterSchedulerFifoGeneric(
					...
					WebServicesTestUtils
							.getXmlInt(element, "availNodeCapacity"),
					WebServicesTestUtils
							.getXmlInt(element, "totalNodeCapacity"),
