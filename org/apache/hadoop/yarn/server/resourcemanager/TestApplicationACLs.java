...
public class TestApplicationACLs {
	...
	public static void setup() throws InterruptedException, IOException {
			...
			protected QueueACLsManager createQueueACLsManager(
				...
				when(
						mockQueueACLsManager.checkAccess(
								...
								any(QueueACL.class), anyString())).thenAnswer(
						new Answer() {
		...
		new Thread() {
			public void run() {
				UserGroupInformation.createUserForTesting(ENEMY,
						new String[] {});
	...
	private void verifyOwnerAccess() throws Exception {
		...
		// List apps as owner
		Assert.assertEquals(
				"App view by owner should list the apps!!",
				1,
	...
	private void verifySuperUserAccess() throws Exception {
		...
		Assert.assertEquals(
				"App view by super-user should list the apps!!",
				2,
				superUserClient
						.getApplications(
								recordFactory
	...
	private void verifyFriendAccess() throws Exception {
		...
		// List apps as friend
		Assert.assertEquals(
				"App view by a friend should list the apps!!",
				3,
				friendClient
						.getApplications(
								recordFactory
	...
	private void verifyEnemyAppReport(ApplicationReport appReport) {
		...
		ApplicationResourceUsageReport usageReport = appReport
	...
	private void verifyAdministerQueueUserAccess() throws Exception {
		...
		Assert.assertEquals(
				"App view by queue-admin-user should list the apps!!",
				5,
