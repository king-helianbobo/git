...
/**
 * unit test - tests addition/deletion/cancellation of renewals of delegation
 * tokens
 * 
 */
...
public class TestDelegationTokenRenewer {
	private static final Log LOG = LogFactory
			.getLog(TestDelegationTokenRenewer.class);
	private static final Text KIND = new Text(
			"TestDelegationTokenRenewer.Token");
	...
	private static class MyDelegationTokenSecretManager extends
	...
	/**
	 * add some extra functionality for testing 1. toString(); 2. cancel() and
	 * isCanceled()
	 */
	...
	/**
	 * fake FileSystem overwrites three methods 1. getDelegationToken() -
	 * generates a token 2. renewDelegataionToken - counts number of calls, and
	 * remembers most recently renewed token. 3. cancelToken -cancels token
	 * (subsequent renew will cause IllegalToken exception
	 */
	...
	static MyToken createTokens(Text renewer) throws IOException {
		...
		DelegationTokenIdentifier dtId1 = new DelegationTokenIdentifier(user1,
				renewer, user1);
	...
	/**
	 * Basic idea of the test: 1. create tokens. 2. Mark one of them to be
	 * renewed in 2 seconds (instead of 24 hours) 3. register them for renewal
	 * 4. sleep for 3 seconds 5. count number of renewals (should 3 initial ones
	 * + one extra) 6. register another token for 2 seconds 7. cancel it
	 * immediately 8. Sleep and check that the 2 seconds renew didn't happen
	 * (totally 5 renewals) 9. check cancellation
	 * 
	 * @throws IOException
	...
	public void testDTRenewal() throws Exception {
		...
		while (attempts-- > 0) {
			try {
				Thread.sleep(3 * 1000); // sleep 3 seconds, so it has time to
										// renew
			} catch (InterruptedException e) {
			}

			// since we cannot guarantee timely execution - let's give few
			// chances
			if (Renewer.counter == numberOfExpectedRenewals)
				break;
		}

		LOG.info("dfs=" + dfs.hashCode() + ";Counter = " + Renewer.counter
				+ ";t=" + Renewer.lastRenewed);
		assertEquals("renew wasn't called as many times as expected(4):",
				numberOfExpectedRenewals, Renewer.counter);
		assertEquals("most recently renewed token mismatch",
				Renewer.lastRenewed, token1);
		...
		while (waitCnt-- > 0) {
			if (!eventQueue.isEmpty()) {
				...
				if (evt.getType() == RMAppEventType.APP_REJECTED) {
					Assert.assertTrue(((RMAppEvent) evt).getApplicationId()
							.equals(appId));
					return;
				}
			} else {
	...
	/**
	 * Basic idea of the test: 1. register a token for 2 seconds with no cancel
	 * at the end 2. cancel it immediately 3. Sleep and check that the 2 seconds
	 * renew didn't happen (totally 5 renewals) 4. check cancellation
	 * 
	 * @throws IOException
	 * @throws URISyntaxException
	 */
	...
	public void testDTRenewalWithNoCancel() throws Exception {
		...
		delegationTokenRenewer.applicationFinished(applicationId_1);
		waitForEventsToGetProcessed(delegationTokenRenewer);
		int numberOfExpectedRenewals = Renewer.counter; // number of renewals so
														// far
		try {
			Thread.sleep(6 * 1000); // sleep 6 seconds, so it has time to renew
		} catch (InterruptedException e) {
		...
		// also renewing of the canceled token should not fail, because it has
		// not
		// been canceled
		token1.renew(conf);
	}
	...
	/**
	 * Basic idea of the test: 0. Setup token KEEP_ALIVE 1. create tokens. 2.
	 * register them for renewal - to be cancelled on app complete 3. Complete
	 * app. 4. Verify token is alive within the KEEP_ALIVE time 5. Verify token
	 * has been cancelled after the KEEP_ALIVE_TIME
	 * 
	 * @throws IOException
	 * @throws URISyntaxException
	 */
	...
	public void testDTKeepAlive1() throws Exception {
		...
		lconf.setLong(
				...
				1000l);
		DelegationTokenRenewer localDtr = createNewDelegationTokenRenewer(
				lconf, counter);
		RMContext mockContext = mock(RMContext.class);
		ClientRMService mockClientRMService = mock(ClientRMService.class);
		when(mockContext.getClientRMService()).thenReturn(mockClientRMService);
		...
		// The token should have been cancelled at this point. Renewal will
		// fail.
		try {
			token1.renew(lconf);
			fail("Renewal of cancelled token should have failed");
	 ...
	 * Basic idea of the test: 0. Setup token KEEP_ALIVE 1. create tokens. 2.
	 * register them for renewal - to be cancelled on app complete 3. Complete
	 * app. 4. Verify token is alive within the KEEP_ALIVE time 5. Send an
	 * explicity KEEP_ALIVE_REQUEST 6. Verify token KEEP_ALIVE time is renewed.
	...
	public void testDTKeepAlive2() throws Exception {
		...
		lconf.setLong(
				YarnConfiguration.RM_DELAYED_DELEGATION_TOKEN_REMOVAL_INTERVAL_MS,
				1000l);
		DelegationTokenRenewer localDtr = createNewDelegationTokenRenewer(conf,
				counter);
		RMContext mockContext = mock(RMContext.class);
		ClientRMService mockClientRMService = mock(ClientRMService.class);
		when(mockContext.getClientRMService()).thenReturn(mockClientRMService);
	...
	@Test(timeout = 20000)
	public void testConcurrentAddApplication() throws IOException,
		...
		// fire up the renewer
		final DelegationTokenRenewer dtr = createNewDelegationTokenRenewer(
				conf, counter);
		RMContext mockContext = mock(RMContext.class);
		ClientRMService mockClientRMService = mock(ClientRMService.class);
		when(mockContext.getClientRMService()).thenReturn(mockClientRMService);
		...
		Thread submitThread = new Thread() {
			@Override
			public void run() {
				dtr.addApplicationAsync(mock(ApplicationId.class), creds1,
						false);
			}
		};
		submitThread.start();
	...
	public void testAppSubmissionWithInvalidDelegationToken() throws Exception {
		...
		ApplicationSubmissionContext appSubContext = ApplicationSubmissionContext
				.newInstance(ApplicationId.newInstance(1234121, 0), "BOGUS",
						"default", Priority.UNDEFINED, amContainer, false,
						true, 1, Resource.newInstance(1024, 1), "BOGUS");
		SubmitApplicationRequest request = SubmitApplicationRequest
				.newInstance(appSubContext);
		try {
