...
public class TestAMRMRPCResponseId {
	...
	private AllocateResponse allocate(ApplicationAttemptId attemptId,
		...
		org.apache.hadoop.security.token.Token<AMRMTokenIdentifier> token = rm
				.getRMContext().getRMApps().get(attemptId.getApplicationId())
	...
	public void testARRMResponseId() throws Exception {
		...
		AllocateRequest allocateRequest = AllocateRequest.newInstance(0, 0F,
				null, null, null);

		AllocateResponse response = allocate(attempt.getAppAttemptId(),
				allocateRequest);
		Assert.assertEquals(1, response.getResponseId());
		Assert.assertTrue(response.getAMCommand() == null);
		allocateRequest = AllocateRequest.newInstance(response.getResponseId(),
				0F, null, null, null);
