...
public class TestRMAdminService {
	...
	public void setup() throws IOException {
		...
		workingPath = new Path(new File("target", this.getClass()
				.getSimpleName() + "-remoteDir").getAbsolutePath());
	...
	public void testAdminRefreshQueuesWithLocalConfigurationProvider()
		...
		CapacityScheduler cs = (CapacityScheduler) rm.getRMContext()
				.getScheduler();
		int maxAppsBefore = cs.getConfiguration()
	...
	public void testAdminAclsWithLocalConfigurationProvider() {
		...
		try {
			rm.adminService.refreshAdminAcls(RefreshAdminAclsRequest
					.newInstance());
		} catch (Exception ex) {
			fail("Using localConfigurationProvider. Should not get any exception.");
		}
	...
	public void testServiceAclsRefreshWithFileSystemBasedConfigurationProvider()
		...
		try {
			...
			ServiceAuthorizationManager clientRMServiceServiceManager = resourceManager
					.getRMContext().getClientRMService().getServer()
			...
			ServiceAuthorizationManager RTService = resourceManager
					.getRMContext().getResourceTrackerService().getServer()
					.getServiceAuthorizationManager();
			verifyServiceACLsRefresh(
					RTService,
					org.apache.hadoop.yarn.api.ApplicationClientProtocolPB.class,
	...
	private void verifyServiceACLsRefresh(ServiceAuthorizationManager manager,
		...
		for (Class<?> protocolClass : manager.getProtocolsWithAcls()) {
			AccessControlList accessList = manager
					.getProtocolsAcls(protocolClass);
	...
	public void testRefreshNodesWithFileSystemBasedConfigurationProvider()
		...
		final File excludeHostsFile = new File(tmpDir.toString(),
				"excludeHosts");
	...
	public void testRMHAWithFileSystemBasedConfiguration() throws IOException,
		...
		try {
			...
			int maxApps = ((CapacityScheduler) rm1.getRMContext()
					.getScheduler()).getConfiguration()
					.getMaximumSystemApplications();
			...
			int maxAppsAfter = ((CapacityScheduler) rm2.getRMContext()
					.getScheduler()).getConfiguration()
					.getMaximumSystemApplications();
	...
	public void testRMStartsWithoutConfigurationFilesProvided() {
		...
		// throwing out Exceptions, if there are no configuration files
		// provided.
	...
	public void testRMInitialsWithFileSystemBasedConfigurationProvider()
		...
		try {
			...
			CapacityScheduler cs = (CapacityScheduler) resourceManager
					.getRMContext().getScheduler();
			int maxAppsAfter = cs.getConfiguration()
					.getMaximumSystemApplications();
			Assert.assertEquals(maxAppsAfter, 5000);
			...
			Assert.assertTrue(ProxyUsers.getProxyGroups()
					.get("hadoop.proxyuser.test.groups").size() == 1);
			Assert.assertTrue(ProxyUsers.getProxyGroups()
					.get("hadoop.proxyuser.test.groups")
