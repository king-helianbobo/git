...
public class TestAppManager {
	...
	public static RMContext mockRMContext(int n, long time) {
		...
		RMContext context = new RMContextImpl(rmDispatcher,
				...
				amFinishingMonitor, null, null, null, null, null, writer) {
			@Override
	...
	public class TestDispatcher implements EventHandler<RMAppEvent> {
		...
		public void handle(RMAppEvent event) {
			...
			System.out.println("in handle routine "
					+ getAppEventType().toString());
		}
	}
	...
	public class TestRMAppManager extends RMAppManager {
		...
		public TestRMAppManager(RMContext context,
				...
				Configuration conf) {
			super(context, scheduler, masterService, applicationACLsManager,
					conf);
		}
	...
	public void setUp() {
		...
		appMonitor = new TestRMAppManager(rmContext,
				new ClientToAMTokenSecretManagerInRM(), scheduler,
	...
	public void testRMAppRetireNone() throws Exception {
		...
		Assert.assertEquals(
				"Number of apps incorrect before checkAppTimeLimit", 10,
	...
	public void testRMAppRetireSome() throws Exception {
		...
		Configuration conf = new YarnConfiguration();
		conf.setInt(
				YarnConfiguration.RM_STATE_STORE_MAX_COMPLETED_APPLICATIONS, 3);
	...
	public void testRMAppRetireSomeDifferentStates() throws Exception {
		...
		Configuration conf = new YarnConfiguration();
		conf.setInt(
				YarnConfiguration.RM_STATE_STORE_MAX_COMPLETED_APPLICATIONS, 2);
	...
	public void testRMAppRetireZeroSetting() throws Exception {
		...
		Configuration conf = new YarnConfiguration();
		conf.setInt(
				YarnConfiguration.RM_STATE_STORE_MAX_COMPLETED_APPLICATIONS, 0);
	...
	public void testStateStoreAppLimitLargerThanMemoryAppLimit() {
		...
		conf.setInt(YarnConfiguration.RM_MAX_COMPLETED_APPLICATIONS,
				maxAppsInMemory);
		// larger than maxCompletedAppsInMemory, reset to
		...
		Assert.assertEquals("Number of completed apps incorrect after check",
				maxAppsInMemory, appMonitor.getCompletedAppsListSize());
		verify(rmContext.getStateStore(), times(numRemoveApps))
				.removeApplication(isA(RMApp.class));
	...
	protected void setupDispatcher(RMContext rmContext, Configuration conf) {
		...
		rmContext.getDispatcher()
				.register(RMAppEventType.class, testDispatcher);
		...
		Assert.assertEquals("app event type is wrong before",
				RMAppEventType.KILL, appEventType);
	}
	...
	public void testRMAppSubmit() throws Exception {
		...
		Assert.assertEquals("app event type sent is wrong",
				RMAppEventType.START, getAppEventType());
	}
	...
	public void testRMAppSubmitMaxAppAttempts() throws Exception {
		...
		int[][] individualMaxAppAttempts = new int[][] {
				new int[] { 9, 10, 11, 0 }, new int[] { 1, 10, 0, -1 } };
		int[][] expectedNums = new int[][] { new int[] { 9, 10, 10, 10 },
				new int[] { 1, 1, 1, 1 } };
	...
	public void testRMAppSubmitDuplicateApplicationId() throws Exception {
		...
		// our testApp1 should be rejected and original app with same id should
		// be left in place
		try {
			appMonitor.submitApplication(asContext, "test");
			Assert.fail("Exception is expected when applicationId is duplicate.");
		} catch (YarnException e) {
			Assert.assertTrue("The thrown exception is not the expectd one.", e
					.getMessage().contains("Cannot add a duplicate!"));
		}
	...
	public void testRMAppSubmitInvalidResourceRequest() throws Exception {
		asContext
				.setResource(Resources
						.createResource(YarnConfiguration.DEFAULT_RM_SCHEDULER_MAXIMUM_ALLOCATION_MB + 1));
		...
		try {
			...
			// TODO Change this to assert the expected exception type - post
			// YARN-142
			// sub-task related to specialized exceptions.
			Assert.assertTrue("The thrown exception is not"
					+ " InvalidResourceRequestException", e.getMessage()
					.contains("Invalid resource request"));
		}
	}
	...
	public void testEscapeApplicationSummary() {
		...
		RMAppManager.ApplicationSummary.SummaryBuilder summary = new RMAppManager.ApplicationSummary()
