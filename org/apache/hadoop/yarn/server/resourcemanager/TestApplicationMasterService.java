...
public class TestApplicationMasterService {
	...
	public void testRMIdentifierOnContainerAllocation() throws Exception {
		...
		Container allocatedContainer = alloc1Response.getAllocatedContainers()
				.get(0);
		ContainerTokenIdentifier tokenId = BuilderUtils
				.newContainerTokenIdentifier(allocatedContainer
						.getContainerToken());
	...
	public void testInvalidContainerReleaseRequest() throws Exception {
		...
		try {
			...
			am1.addRequests(new String[] { "127.0.0.1" }, GB, 1, 1);
			AllocateResponse alloc1Response = am1.schedule(); // send the
																// request
			...
			// Now trying to release container allocated for app1 ->
			// appAttempt1.
			ContainerId cId = alloc1Response.getAllocatedContainers().get(0)
					.getId();
	...
	public void testFinishApplicationMasterBeforeRegistering() throws Exception {
		...
		try {
			...
			Assert.assertTrue(cause instanceof InvalidApplicationMasterRequestException);
