...
public class TestKillApplicationWithRMHA extends RMHATestBase {
	...
	public void testKillAppWhenFailoverHappensAtNewState() throws Exception {
		...
		try {
			...
			Assert.assertTrue(ex.getMessage().contains(
					"Trying to kill an absent application "
	...
	public void testKillAppWhenFailoverHappensAtRunningState() throws Exception {
		...
		// will load the ApplicationState. After that, the application will be
		// at
		// ACCEPTED State. Because the application is not at Final State,
		// KillApplicationResponse.getIsKillCompleted is expected to return
		// false.
	...
	private void failOverAndKillApp(ApplicationId appId,
		...
		Assert.assertEquals(initialRMAppState, rm1.getRMContext().getRMApps()
				.get(appId).getState());
		Assert.assertEquals(initialRMAppAttemptState, rm1.getRMContext()
				.getRMApps().get(appId).getAppAttempts().get(appAttemptId)
				.getState());
		explicitFailover();
		Assert.assertEquals(expectedAppStateBeforeKillApp, rm2.getRMContext()
				.getRMApps().get(appId).getState());
