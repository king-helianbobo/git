...
public class TestZKRMStateStoreZKClientConnections extends ClientBaseWithFixes {
	...
	private Log LOG = LogFactory
			.getLog(TestZKRMStateStoreZKClientConnections.class);
	...
	static {
		try {
			DIGEST_USER_HASH = DigestAuthenticationProvider
					.generateDigest(DIGEST_USER_PASS);
		} catch (NoSuchAlgorithmException e) {
			throw new RuntimeException(e);
		}
	}
	private static final String TEST_ACL = "digest:" + DIGEST_USER_HASH
			+ ":rwcda";
	...
	class TestZKClient {
		...
		public RMStateStore getRMStateStore(Configuration conf)
				throws Exception {
	...
	public void testZKClientRetry() throws Exception {
		...
		final ZKRMStateStore store = (ZKRMStateStore) zkClientTester
	...
	public void testSetZKAcl() {
		...
		try {
			zkClientTester.store.zkClient.delete(
					zkClientTester.store.znodeWorkingPath, -1);
