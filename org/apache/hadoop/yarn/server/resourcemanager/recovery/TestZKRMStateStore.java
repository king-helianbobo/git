...
public class TestZKRMStateStore extends RMStateStoreTestBase {
	...
	class TestZKRMStateStoreTester implements RMStateStoreHelper {
		...
		class TestZKRMStateStoreInternal extends ZKRMStateStore {
			...
			public TestZKRMStateStoreInternal(Configuration conf,
			...
			public String getVersionNode() {
				return znodeWorkingPath + "/" + ROOT_ZNODE_NAME + "/"
						+ VERSION_NODE;
			}
		...
		public RMStateStore getRMStateStore() throws Exception {
			...
			conf.set(YarnConfiguration.ZK_RM_STATE_STORE_PARENT_PATH,
					workingZnode);
		...
		public boolean isFinalStateValid() throws Exception {
			List<String> nodes = client.getChildren(store.znodeWorkingPath,
					false);
			return nodes.size() == 1;
		}
		...
		public void writeVersion(RMStateVersion version) throws Exception {
			client.setData(store.getVersionNode(),
		...
		public boolean appExists(RMApp app) throws Exception {
			Stat node = client.exists(
					store.getAppNode(app.getApplicationId().toString()), false);
			return node != null;
		}
	}
	...
	private Configuration createHARMConf(String rmIds, String rmId,
			int adminPort) {
		...
		for (String rpcAddress : YarnConfiguration
	...
	public void testFencing() throws Exception {
		...
		assertEquals("RM with ZKStore didn't start", Service.STATE.STARTED,
				rm1.getServiceState());
		assertEquals("RM should be Active",
				HAServiceProtocol.HAServiceState.ACTIVE, rm1.getRMContext()
		...
		assertEquals("RM with ZKStore didn't start", Service.STATE.STARTED,
				rm2.getServiceState());
		assertEquals("RM should be Active",
				HAServiceProtocol.HAServiceState.ACTIVE, rm2.getRMContext()
						.getRMAdminService().getServiceStatus().getState());

		for (int i = 0; i < ZK_TIMEOUT_MS / 50; i++) {
			if (HAServiceProtocol.HAServiceState.ACTIVE == rm1.getRMContext()
					.getRMAdminService().getServiceStatus().getState()) {
				Thread.sleep(100);
			}
		}
		assertEquals("RM should have been fenced",
				HAServiceProtocol.HAServiceState.STANDBY, rm1.getRMContext()
						.getRMAdminService().getServiceStatus().getState());
		assertEquals("RM should be Active",
				HAServiceProtocol.HAServiceState.ACTIVE, rm2.getRMContext()
						.getRMAdminService().getServiceStatus().getState());
