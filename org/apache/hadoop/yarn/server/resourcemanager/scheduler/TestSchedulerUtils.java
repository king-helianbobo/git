...
public class TestSchedulerUtils {
	...
	public void testNormalizeRequestWithDominantResourceCalculator() {
		...
		SchedulerUtils.normalizeRequest(ask, resourceCalculator,
				clusterResource, minResource, maxResource);
	...
	public void testValidateResourceRequest() {
		Resource maxResource = Resources
				.createResource(
	...
	public void testValidateResourceBlacklistRequest() throws Exception {
		...
		final MockRMWithAMS rm = new MockRMWithAMS(new YarnConfiguration(),
				containerManager);
		rm.start();
				...
				.doAs(new PrivilegedAction<ApplicationMasterProtocol>() {
					...
					public ApplicationMasterProtocol run() {
						return (ApplicationMasterProtocol) rpc.getProxy(
								ApplicationMasterProtocol.class, rmBindAddress,
								conf);
					}
				});
		...
		ResourceBlacklistRequest blacklistRequest = ResourceBlacklistRequest
				.newInstance(Collections.singletonList(ResourceRequest.ANY),
						null);

		AllocateRequest allocateRequest = AllocateRequest.newInstance(0, 0.0f,
				null, null, blacklistRequest);
		boolean error = false;
		try {
		...
		Assert.assertTrue(
				"Didn't not catch InvalidResourceBlacklistRequestException",
				error);
	}
	...
	public static <T> SchedulerApplication verifyAppAddedAndRemovedFromScheduler(
		...
		AppRemovedSchedulerEvent appRemoveEvent = new AppRemovedSchedulerEvent(
				appId, RMAppState.FINISHED);
