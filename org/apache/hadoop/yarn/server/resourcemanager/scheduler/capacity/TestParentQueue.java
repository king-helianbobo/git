...
public class TestParentQueue {
	...
	private FiCaSchedulerApp getMockApplication(int appId, String user) {
		...
		doReturn(Resources.createResource(0, 0)).when(application)
				.getHeadroom();
		return application;
	}
		...
		doAnswer(new Answer<CSAssignment>() {
			...
			public CSAssignment answer(InvocationOnMock invocation)
				...
				try {
					...
					LOG.info("FOOBAR q.assignContainers q="
							+ queue.getQueueName() + " alloc=" + allocation
							+ " node=" + node.getNodeName());
				}
				final Resource allocatedResource = Resources
						.createResource(allocation);
				...
				// Next call - nothing
				if (allocation > 0) {
					doReturn(new CSAssignment(Resources.none(), type)).when(
							queue).assignContainers(eq(clusterResource),
							eq(node));
					...
					doReturn(
							Resources
									.subtractFrom(available, allocatedResource))
	...
	private float computeQueueUsedCapacity(CSQueue queue, int expectedMemory,
		...
		return (expectedMemory / (clusterResource.getMemory() * queue
				.getAbsoluteCapacity()));
	}
	...
	private void verifyQueueMetrics(CSQueue queue, int expectedMemory,
		...
		assertEquals(
				computeQueueAbsoluteUsedCapacity(queue, expectedMemory,
						clusterResource), queue.getAbsoluteUsedCapacity(),
				DELTA);
		assertEquals(
	...
	public void testSingleLevelQueues() throws Exception {
		...
		FiCaSchedulerNode node_0 = TestUtils.getMockNode("host_0",
				DEFAULT_RACK, 0, memoryPerNode * GB);
		FiCaSchedulerNode node_1 = TestUtils.getMockNode("host_1",
				DEFAULT_RACK, 0, memoryPerNode * GB);

		final Resource clusterResource = Resources.createResource(numNodes
				* (memoryPerNode * GB), numNodes * coresPerNode);
	...
	public void testMultiLevelQueues() throws Exception {
		...
		FiCaSchedulerNode node_1 = TestUtils.getMockNode("host_1",
				DEFAULT_RACK, 0, memoryPerNode * GB);
		FiCaSchedulerNode node_2 = TestUtils.getMockNode("host_2",
				DEFAULT_RACK, 0, memoryPerNode * GB);
		...
		stubQueueAllocation(a1, clusterResource, node_2, 1 * GB); // shouldn't
																	// be
																	// allocated
																	// due
																	// to
	...
	public void testOffSwitchSchedulingMultiLevelQueues() throws Exception {
		...
		// Now, B2 should get the scheduling opportunity since B2=0G/2G,
		// B3=1G/7G
