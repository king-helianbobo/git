...
public class TestRMRestart {
	...
	public void testRMRestart() throws Exception {
		...
		Map<ApplicationId, ApplicationState> rmAppState = rmState
				.getApplicationState();
		...
		MockNM nm2 = new MockNM("127.0.0.2:5678", 15120,
				rm1.getResourceTrackerService());
		nm1.registerNode();
		...
		RMApp appUnmanaged = rm1.submitApp(200, "someApp", "someUser", null,
				true, null, conf.getInt(YarnConfiguration.RM_AM_MAX_ATTEMPTS,
						YarnConfiguration.DEFAULT_RM_AM_MAX_ATTEMPTS), null);
		ApplicationAttemptId unmanagedAttemptId = appUnmanaged
		...
		// Unmanaged app state is still loaded back but it cannot be restarted
		// by
		// the RM. this will change with work preserving RM restart in which
		// AMs/NMs
		// are not rebooted.
		Assert.assertEquals(4, rm2.getRMContext().getRMApps().size());
		// check that earlier finished app and attempt is also loaded back and
		// move
		// to finished state.
		rm2.waitForState(app0.getApplicationId(), RMAppState.FINISHED);
		rm2.waitForState(am0.getApplicationAttemptId(),
				RMAppAttemptState.FINISHED);

		// verify correct number of attempts and other data
		RMApp loadedApp1 = rm2.getRMContext().getRMApps()
		...
		Assert.assertEquals(app1.getApplicationSubmissionContext()
				.getApplicationId(), loadedApp1
				.getApplicationSubmissionContext().getApplicationId());

		RMApp loadedApp2 = rm2.getRMContext().getRMApps()
		...
		// container finish event to arrive. However for an application for
		// which
		...
		nm1 = new MockNM("127.0.0.1:1234", 15120,
				rm2.getResourceTrackerService());
		nm2 = new MockNM("127.0.0.2:5678", 15120,
				rm2.getResourceTrackerService());

		List<ContainerStatus> containerStatuses = new ArrayList<ContainerStatus>();
		ContainerStatus containerStatus = BuilderUtils.newContainerStatus(
				BuilderUtils.newContainerId(loadedApp1.getCurrentAppAttempt()
						.getAppAttemptId(), 1), ContainerState.COMPLETE,
				"Killed AM container", 143);
	...
	public void testRMRestartAppRunningAMFailed() throws Exception {
		...
		// new AM started but not registered, app still stays at ACCECPTED
		// state.
	...
	public void testRMRestartWaitForPreviousAMToFinish() throws Exception {
		...
		// node manager on which this AM container was running also went down)
		// in
		// that case AMLivenessMonitor should time out previous attempt and
		// start
		// new attempt.
		...
		rm1.start();
		MockNM nm1 = new MockNM("127.0.0.1:1234", 16382,
				rm1.getResourceTrackerService());
		nm1.registerNode();
		...
		RMApp rmApp = rm2.getRMContext().getRMApps()
				.get(app1.getApplicationId());
		...
		// am1 attempt should be in FAILED state where as am2 attempt should be
		// in
		// LAUNCHED state
		rm2.waitForState(am1.getApplicationAttemptId(),
				RMAppAttemptState.FAILED);
		rm2.waitForState(am2.getApplicationAttemptId(),
				RMAppAttemptState.LAUNCHED);
		Assert.assertEquals(RMAppAttemptState.FAILED, rmApp.getAppAttempts()
				.get(am1.getApplicationAttemptId()).getAppAttemptState());
		Assert.assertEquals(RMAppAttemptState.LAUNCHED, rmApp.getAppAttempts()
				.get(am2.getApplicationAttemptId()).getAppAttemptState());
		...
		// in LAUNCHED state
		rm3.waitForState(am1.getApplicationAttemptId(),
				RMAppAttemptState.FAILED);
		rm3.waitForState(am2.getApplicationAttemptId(),
				RMAppAttemptState.FAILED);
		ApplicationAttemptId latestAppAttemptId = rmApp.getCurrentAppAttempt()
				.getAppAttemptId();
		...
		Assert.assertEquals(
				...
				memStore.getState().getApplicationState()
						.get(app2.getApplicationId()).getAttemptCount());
	...
	// Test RM restarts after previous attempt succeeded and was saved into
	// state
	// store but before the RMAppAttempt notifies RMApp that it has succeeded.
	// On
	...
	public void testRMRestartWaitForPreviousSucceededAttempt() throws Exception {
		...
		// recovered via its own saved state, but only via the event
		// notification
	...
	public void testRMRestartFailedApp() throws Exception {
		...
		MockRM rm2 = new MockRM(conf, memStore);
		rm2.start();
		RMApp loadedApp0 = rm2.getRMContext().getRMApps()
	...
	public void testRMRestartKilledApp() throws Exception {
		...
		rm1.waitForState(am0.getApplicationAttemptId(),
	...
	public void testRMRestartSucceededApp() throws Exception {
		...
		ApplicationState appState = rmAppState.get(app0.getApplicationId());
		ApplicationAttemptState attemptState0 = appState.getAttempt(am0
	...
	public void testRMRestartGetApplicationList() throws Exception {
		...
		// a succeeded app.
		RMApp app0 = rm1.submitApp(200, "name", "user", null, false, "default",
				1, null, "myType");
		MockAM am0 = launchAM(app0, rm1, nm1);
		finishApplicationMaster(app0, rm1, nm1, am0);

		// a failed app.
		RMApp app1 = rm1.submitApp(200, "name", "user", null, false, "default",
				1, null, "myType");
		MockAM am1 = launchAM(app1, rm1, nm1);
		// fail the AM by sending CONTAINER_FINISHED event without registering.
		nm1.nodeHeartbeat(am1.getApplicationAttemptId(), 1,
		...
		// a killed app.
		RMApp app2 = rm1.submitApp(200, "name", "user", null, false, "default",
				1, null, "myType");
		MockAM am2 = launchAM(app2, rm1, nm1);
		rm1.killApp(app2.getApplicationId());
		rm1.waitForState(app2.getApplicationId(), RMAppState.KILLED);
		rm1.waitForState(am2.getApplicationAttemptId(),
				RMAppAttemptState.KILLED);
		...
		GetApplicationsRequest request1 = GetApplicationsRequest
				.newInstance(EnumSet.of(YarnApplicationState.FINISHED,
						YarnApplicationState.KILLED,
						YarnApplicationState.FAILED));
		GetApplicationsResponse response1 = rm2.getClientRMService()
		...
		appTypes.add("myType");
		GetApplicationsRequest request2 = GetApplicationsRequest
				.newInstance(appTypes);
		GetApplicationsResponse response2 = rm2.getClientRMService()
				.getApplications(request2);
		List<ApplicationReport> appList2 = response2.getApplicationList();
		Assert.assertTrue(3 == appList2.size());

		// check application summary is logged for the completed apps after RM
		// restart.
	...
	private ApplicationReport verifyAppReportAfterRMRestart(RMApp app, MockRM rm)
			throws Exception {
		GetApplicationReportRequest reportRequest = GetApplicationReportRequest
				.newInstance(app.getApplicationId());
		GetApplicationReportResponse response = rm.getClientRMService()
	...
	private void finishApplicationMaster(RMApp rmApp, MockRM rm, MockNM nm,
		...
		RMState rmState = ((MemoryRMStateStore) rm.getRMContext()
				.getStateStore()).getState();
		Map<ApplicationId, ApplicationState> rmAppState = rmState
				.getApplicationState();
		am.unregisterAppAttempt(req, true);
		am.waitForState(RMAppAttemptState.FINISHING);
		nm.nodeHeartbeat(am.getApplicationAttemptId(), 1,
				ContainerState.COMPLETE);
	...
	public void testRMRestartOnMaxAppAttempts() throws Exception {
		...
		// app1 failed state is saved in state store. app2 final saved state is
		// not
		// determined yet.
	...
	public void testDelegationTokenRestoredInDelegationTokenRenewer()
		...
		RMDelegationTokenIdentifier dtId1 = new RMDelegationTokenIdentifier(
				userText1, new Text("renewer1"), userText1);
		Token<RMDelegationTokenIdentifier> token1 = new Token<RMDelegationTokenIdentifier>(
		...
		RMDelegationTokenIdentifier dtId2 = new RMDelegationTokenIdentifier(
				userText2, new Text("renewer2"), userText2);
		Token<RMDelegationTokenIdentifier> token2 = new Token<RMDelegationTokenIdentifier>(
				dtId2, rm1.getRMContext().getRMDelegationTokenSecretManager());
		...
		// verify tokens are properly populated back to rm2
	...
	public void testAppAttemptTokensRestoredOnRMRestart() throws Exception {
		...
		MockRM rm1 = new TestSecurityMockRM(conf, memStore);
		rm1.start();
		MockNM nm1 = new MockNM("0.0.0.0:4321", 15120,
				rm1.getResourceTrackerService());
		nm1.registerNode();
		...
		byte[] clientTokenMasterKey = attempt1.getClientTokenMasterKey()
				.getEncoded();
		...
		Assert.assertArrayEquals(clientTokenMasterKey, rm2
				.getClientToAMTokenSecretManager().getMasterKey(attemptId1)
				.getEncoded());
	...
	public void testRMDelegationTokenRestoredOnRMRestart() throws Exception {
		...
		Map<ApplicationId, ApplicationState> rmAppState = rmState
				.getApplicationState();
		Map<RMDelegationTokenIdentifier, Long> rmDTState = rmState
				.getRMDTSecretManagerState().getTokenState();
		Set<DelegationKey> rmDTMasterKeyState = rmState
				.getRMDTSecretManagerState().getMasterKeyState();
		...
		GetDelegationTokenRequest request1 = GetDelegationTokenRequest
				.newInstance("renewer1");
		UserGroupInformation.getCurrentUser().setAuthenticationMethod(
				AuthMethod.KERBEROS);
		GetDelegationTokenResponse response1 = rm1.getClientRMService()
				.getDelegationToken(request1);
		org.apache.hadoop.yarn.api.records.Token delegationToken1 = response1
		...
		Map<RMDelegationTokenIdentifier, Long> allTokensRM1 = rm1
		...
		Assert.assertEquals(rm1.getRMContext()
				.getRMDelegationTokenSecretManager()
				.getLatestDTSequenceNumber(), rmState
				.getRMDTSecretManagerState().getDTSequenceNumber());

		// request one more token
		GetDelegationTokenRequest request2 = GetDelegationTokenRequest
				.newInstance("renewer2");
		GetDelegationTokenResponse response2 = rm1.getClientRMService()
				.getDelegationToken(request2);
		org.apache.hadoop.yarn.api.records.Token delegationToken2 = response2
				.getRMDelegationToken();
		Token<RMDelegationTokenIdentifier> token2 = ConverterUtils
		...
		// Assert the token which has the latest delegationTokenSequenceNumber
		// is removed
		Assert.assertEquals(rm1.getRMContext()
				.getRMDelegationTokenSecretManager()
		...
		Map<RMDelegationTokenIdentifier, Long> allTokensRM2 = rm2
				.getRMContext().getRMDelegationTokenSecretManager()
				.getAllTokens();
		...
		try {
			// Sleep for one millisecond to make sure renewDataAfterRenew is
			// greater
			Thread.sleep(1);
	...
	// This is to test submit an application to the new RM with the old
	// delegation
	...
	public void testRMStateStoreDispatcherDrainedOnRMStop() throws Exception {
		...
		// all apps's saving request are now enqueued to RMStateStore's
		// dispatcher
		// queue, and will be processed once rm.stop() is called.

		// Nothing exist in state store before stop is called.
		Map<ApplicationId, ApplicationState> rmAppState = memStore.getState()
				.getApplicationState();
		...
		// Assert app info is still saved even if stop is called with pending
		// saving
	...
	// reaches killed state and also check that attempt state is saved before
	// app
	// state is saved.
	@Test(timeout = 60000)
	public void testClientRetryOnKillingApplication() throws Exception {
		...
		// we expect at least 2 calls for killApp as the first killApp always
		// return
		// false.
	...
	private void assertQueueMetrics(QueueMetrics qm, int appsSubmitted,
		...
		Assert.assertEquals(qm.getAppsSubmitted(), appsSubmitted
				+ appsSubmittedCarryOn);
		Assert.assertEquals(qm.getAppsPending(), appsPending
				+ appsPendingCarryOn);
		Assert.assertEquals(qm.getAppsRunning(), appsRunning
				+ appsRunningCarryOn);
		Assert.assertEquals(qm.getAppsCompleted(), appsCompleted
				+ appsCompletedCarryOn);
	}
	...
	public void testSynchronouslyRenewDTOnRecovery() throws Exception {
		...
		MockRM rm1 = new MockRM(conf, memStore);
		rm1.start();
		final MockNM nm1 = new MockNM("127.0.0.1:1234", 15120,
				rm1.getResourceTrackerService());
		nm1.registerNode();
		RMApp app0 = rm1.submitApp(200);
			...
			protected ResourceTrackerService createResourceTrackerService() {
				return new ResourceTrackerService(this.rmContext,
					...
					protected void serviceStart() throws Exception {
						...
						ContainerId amContainer = ContainerId.newInstance(
								am0.getApplicationAttemptId(), 1);
						status.add(ContainerStatus.newInstance(amContainer,
								ContainerState.COMPLETE, "AM container exit",
								143));
	...
	public static class TestSecurityMockRM extends MockRM {
		...
		protected ClientRMService createClientRMService() {
			return new ClientRMService(getRMContext(), getResourceScheduler(),
					rmAppManager, applicationACLsManager, null, getRMContext()
							.getRMDelegationTokenSecretManager()) {
				@Override
				protected void serviceStart() throws Exception {
					// do nothing
