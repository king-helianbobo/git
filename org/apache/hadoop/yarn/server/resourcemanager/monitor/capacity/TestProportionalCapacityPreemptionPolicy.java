...
public class TestProportionalCapacityPreemptionPolicy {
	...
	final ArgumentCaptor<ContainerPreemptEvent> evtCaptor = ArgumentCaptor
			.forClass(ContainerPreemptEvent.class);
	...
	public void testProportionalPreemption() {
		...
		ProportionalCapacityPreemptionPolicy policy = buildPolicy(qData);
		policy.editSchedule();
		verify(mDisp, times(16)).handle(
				argThat(new IsPreemptionRequestFor(appA)));
	}
	...
	public void testPreemptCycle() {
		...
		verify(mDisp, times(10)).handle(
				argThat(new IsPreemptionRequestFor(appC)));
	}
	...
	public void testHierarchicalLarge() {
		...
		verify(mDisp, times(9)).handle(
				argThat(new IsPreemptionRequestFor(appA)));
		verify(mDisp, times(4)).handle(
				argThat(new IsPreemptionRequestFor(appE)));
	}
	...
	static class IsPreemptionRequestFor extends
			ArgumentMatcher<ContainerPreemptEvent> {
	...
	ProportionalCapacityPreemptionPolicy buildPolicy(int[][] qData) {
		...
		Resource clusterResources = Resource.newInstance(
				leafAbsCapacities(qData[0], qData[6]), 0);
	...
	ParentQueue mockNested(int[] abs, int[] used, int[] pending,
			int[] reserved, int[] apps, int[] gran, int[] queues) {
		...
		for (int i = 1; i < queues.length; ++i) {
			...
			if (queues[i] > 0) {
				...
				q = mockLeafQueue(p, tot, i, abs, used, pending, reserved,
						apps, gran);
			}
	...
	LeafQueue mockLeafQueue(ParentQueue p, float tot, int i, int[] abs,
		...
		NavigableSet<FiCaSchedulerApp> qApps = new TreeSet<FiCaSchedulerApp>(
				new Comparator<FiCaSchedulerApp>() {
					...
					public int compare(FiCaSchedulerApp a1, FiCaSchedulerApp a2) {
						return a1.getApplicationAttemptId().compareTo(
								a2.getApplicationAttemptId());
					}
				});
		...
		if (apps[i] != 0) {
			...
			for (int a = 0; a < apps[i]; ++a) {
				qApps.add(mockApp(i, appAlloc, aUsed, aPending, aReserve,
						gran[i]));
				++appAlloc;
			}
		}
	...
	FiCaSchedulerApp mockApp(int qid, int id, int used, int pending,
			int reserved, int gran) {
		FiCaSchedulerApp app = mock(FiCaSchedulerApp.class);

		ApplicationId appId = ApplicationId.newInstance(TS, id);
		ApplicationAttemptId appAttId = ApplicationAttemptId.newInstance(appId,
				0);
	...
	void printString(CSQueue nq, String indent) {
		if (nq instanceof ParentQueue) {
			System.out.println(indent + nq.getQueueName() + " cur:"
