...
public class TestRMAppTransitions {
	...
	private static final class TestApplicationAttemptEventDispatcher implements
		...
		public void handle(RMAppAttemptEvent event) {
			ApplicationId appId = event.getApplicationAttemptId()
					.getApplicationId();
			RMApp rmApp = this.rmContext.getRMApps().get(appId);
			if (rmApp != null) {
				try {
					rmApp.getRMAppAttempt(event.getApplicationAttemptId())
							.handle(event);
				} catch (Throwable t) {
	...
	public void setUp() throws Exception {
		...
		this.rmContext = new RMContextImpl(rmDispatcher,
				containerAllocationExpirer, amLivelinessMonitor,
				amFinishingMonitor, null, new AMRMTokenSecretManager(conf),
	...
	private static void testAppStartState(ApplicationId applicationId,
		...
		Assert.assertEquals("application user is not correct", user,
				application.getUser());
		Assert.assertEquals("application id is not correct", applicationId,
				application.getApplicationId());
		Assert.assertEquals("application progress is not correct", (float) 0.0,
				application.getProgress());
		Assert.assertEquals("application queue is not correct", queue,
				application.getQueue());
		Assert.assertEquals("application name is not correct", name,
				application.getName());
		Assert.assertEquals("application finish time is not 0 and should be",
				0, application.getFinishTime());
		Assert.assertEquals("application tracking url is not correct", null,
				application.getTrackingUrl());
		StringBuilder diag = application.getDiagnostics();
		Assert.assertEquals("application diagnostics is not correct", 0,
				diag.length());
	}
	...
	private static void assertFinalAppStatus(FinalApplicationStatus status,
			RMApp application) {
		Assert.assertEquals("Final application status should have been "
				+ status, status, application.getFinalApplicationStatus());
	}
	...
	private void assertFailed(RMApp application, String regex) {
		...
		StringBuilder diag = application.getDiagnostics();
		Assert.assertTrue("application diagnostics is not correct", diag
				.toString().matches(regex));
	}

	private void sendAppUpdateSavedEvent(RMApp application) {
		RMAppEvent event = new RMAppUpdateSavedEvent(
	...
	private void sendAttemptUpdateSavedEvent(RMApp application) {
		application.getCurrentAppAttempt().handle(
				new RMAppAttemptUpdateSavedEvent(application
						.getCurrentAppAttempt().getAppAttemptId(), null));
	}
	...
	protected RMApp testCreateAppSubmittedNoRecovery(
		...
		RMAppEvent event = new RMAppNewSavedEvent(
				application.getApplicationId(), null);
		application.handle(event);
		assertStartTimeSet(application);
	...
	protected RMApp testCreateAppFinalSaving(
		...
		RMAppEvent finishingEvent = new RMAppEvent(
				application.getApplicationId(),
	...
	protected RMApp testCreateAppFinishing(
		...
		RMAppEvent appUpdated = new RMAppUpdateSavedEvent(
				application.getApplicationId(), null);
	...
	protected RMApp testCreateAppFinished(
			ApplicationSubmissionContext submissionContext, String diagnostics)
			throws IOException {
		// unmanaged AMs don't use the FINISHING state
		...
		Assert.assertTrue("Finished app missing diagnostics", application
				.getDiagnostics().indexOf(diagnostics) != -1);
		return application;
	}
	...
	public void testAppNewReject() throws IOException {
		...
		RMAppEvent event = new RMAppRejectedEvent(
				application.getApplicationId(), rejectedText);
		application.handle(event);
		rmDispatcher.await();
		sendAppUpdateSavedEvent(application);
	...
	public void testAppAcceptedFailed() throws IOException {
		...
		// RMAppEventType.RMAppEventType.ATTEMPT_FAILED
		...
		rmDispatcher.await();
		sendAppUpdateSavedEvent(application);
		assertFailed(application, ".*" + message
	...
	public void testAppAcceptedKill() throws IOException, InterruptedException {
		...
		RMAppEvent appAttemptKilled = new RMAppEvent(
				application.getApplicationId(), RMAppEventType.ATTEMPT_KILLED);
	...
	public void testAppRunningKill() throws IOException {
		...
		assertAppState(RMAppState.KILLING, application);
		RMAppEvent finishEvent = new RMAppFinishedAttemptEvent(
				application.getApplicationId(), null);
	...
	public void testAppRunningFailed() throws IOException {
		...
		Assert.assertEquals(expectedAttemptId, appAttempt.getAppAttemptId()
				.getAttemptId());
		...
		for (int i = 1; i < maxAppAttempts; i++) {
			...
			Assert.assertEquals(++expectedAttemptId, appAttempt
	...
	public void testAppFinalSavingToFinished() throws IOException {
		...
		RMAppEvent event = new RMAppFinishedAttemptEvent(
				application.getApplicationId(), diagMsg);
		application.handle(event);
		assertAppState(RMAppState.FINAL_SAVING, application);
		RMAppEvent appUpdated = new RMAppUpdateSavedEvent(
				application.getApplicationId(), null);
	...
	public void testAppsRecoveringStates() throws Exception {
		...
		Map<ApplicationId, ApplicationState> applicationState = state
				.getApplicationState();
	...
	public void testRecoverApplication(ApplicationState appState,
			RMState rmState) throws Exception {
		ApplicationSubmissionContext submissionContext = appState
				.getApplicationSubmissionContext();
		RMAppImpl application = new RMAppImpl(appState.getAppId(), rmContext,
				conf, submissionContext.getApplicationName(), null,
	...
	public void createRMStateForApplications(
		...
		ApplicationState appState = new ApplicationState(app.getSubmitTime(),
				...
				app.getUser(), rmAppState, null, app.getFinishTime());
	...
	public void testGetAppReport() {
		...
		ApplicationReport report = app
				.createAndGetApplicationReport(null, true);
	...
	private void verifyApplicationFinished(RMAppState state) {
		ArgumentCaptor<RMAppState> finalState = ArgumentCaptor
				.forClass(RMAppState.class);
