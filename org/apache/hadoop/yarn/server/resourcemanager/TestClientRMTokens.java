...
public class TestClientRMTokens {
	...
	public void testDelegationToken() throws IOException, InterruptedException {
		...
		conf.set(YarnConfiguration.RM_PRINCIPAL,
		...
		final ClientRMService clientRMService = new ClientRMServiceForTest(
		...
		try {
			...
			org.apache.hadoop.yarn.api.records.Token token = getDelegationToken(
					loggedInUser, clientRMService,
			...
			while (System.currentTimeMillis() < tokenFetchTime
					+ initialInterval / 2) {
				Thread.sleep(500l);
			}
			long nextExpTime = renewDelegationToken(loggedInUser,
					clientRMService, token);
			...
			try {
				...
				assertEquals(InvalidToken.class.getName(), e.getClass()
						.getName());
			...
			clientRMWithDT = getClientRMProtocolWithDT(token,
					clientRMService.getBindAddress(), "loginuser2", conf);
			LOG.info("Cancelled delegation token at: "
	...
	@Test
	public void testShortCircuitRenewCancel() throws IOException,
			InterruptedException {
		InetSocketAddress addr = new InetSocketAddress(
				InetAddress.getLocalHost(), 123);
	...
	public void testShortCircuitRenewCancelWildcardAddress()
		...
		checkShortCircuitRenewCancel(
				rmAddr,
				new InetSocketAddress(InetAddress.getLocalHost(), rmAddr
						.getPort()), true);
	}
	...
	public void testShortCircuitRenewCancelSameHostDifferentPort()
			throws IOException, InterruptedException {
		InetSocketAddress rmAddr = new InetSocketAddress(
				InetAddress.getLocalHost(), 123);
		checkShortCircuitRenewCancel(
				rmAddr,
	...
	public void testShortCircuitRenewCancelDifferentHostSamePort()
		...
		InetSocketAddress rmAddr = new InetSocketAddress(
				InetAddress.getLocalHost(), 123);
		checkShortCircuitRenewCancel(rmAddr, new InetSocketAddress("1.1.1.1",
				rmAddr.getPort()), false);
	}
	...
	private void checkShortCircuitRenewCancel(InetSocketAddress rmAddr,
		...
		RMDelegationTokenIdentifier.Renewer.setSecretManager(secretManager,
				rmAddr);
		...
		if (shouldShortCircuit) {
			...
			verify(secretManager, never()).renewToken(any(Token.class),
					anyString());
			try {
				token.cancel(conf);
				fail();
			...
			verify(secretManager, never()).cancelToken(any(Token.class),
					anyString());
		}
	}
	...
	private org.apache.hadoop.yarn.api.records.Token getDelegationToken(
			...
			final String renewerString) throws IOException,
			InterruptedException {
	...
	class ClientRMServiceForTest extends ClientRMService {
		...
		public ClientRMServiceForTest(Configuration conf,
			...
			super(mock(RMContext.class), scheduler, mock(RMAppManager.class),
					new ApplicationACLsManager(conf), new QueueACLsManager(
							scheduler, conf), rmDTSecretManager);
		}
	...
	private static RMDelegationTokenSecretManager createRMDelegationTokenSecretManager(
			long secretKeyInterval, long tokenMaxLifetime,
