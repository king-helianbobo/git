...
public class TestYarnConfiguration {
	...
	public void testDefaultRMWebUrl() throws Exception {
		...
		// shouldn't have a "/" on the end of the url as all the other uri
		// routinnes
		// specifically add slashes and Jetty doesn't handle double slashes.
		Assert.assertNotSame("RM Web Url is not correct",
				"http://0.0.0.0:8088", rmWebUrl);
	}
	...
	public void testRMWebUrlSpecified() throws Exception {
		...
		Assert.assertEquals("RM Web URL Port is incrrect", 24543, Integer
				.valueOf(parts[parts.length - 1]).intValue());
	...
	public void testGetSocketAddressForNMWithHA() {
		...
		InetSocketAddress addr = conf.getSocketAddr(
