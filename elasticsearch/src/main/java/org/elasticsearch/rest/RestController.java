...
public class RestController extends AbstractLifecycleComponent<RestController> {
    ...
    // non volatile since the assumption is that pre processors are registered
    // on startup
    ...
    /**
     * Registers a pre processor to be executed before the rest request is
     * actually handled.
     */
    ...
    /**
     * Registers a rest handler to be execute when the provided method and path
     * match the request.
     */
    ...
    /**
     * Returns a filter chain (if needed) to execute. If this method returns
     * null, simply execute as usual.
     */
    @Nullable
    ...
    void executeHandler(RestRequest request, RestChannel channel) {
        ...
        if (handler != null) {
            ...
            if (request.method() == RestRequest.Method.OPTIONS) {
                // when we have OPTIONS request, simply send OK by default (with
                // the Access Control Origin header which gets automatically
                // added)
                StringRestResponse response = new StringRestResponse(OK);
                channel.sendResponse(response);
            } else {
                channel.sendResponse(new StringRestResponse(BAD_REQUEST, "No handler found for uri [" + request.uri() + "] and method ["
                        + request.method() + "]"));
            }
        }
    }
    ...
    private String getPath(RestRequest request) {
        // we use rawPath since we don't want to decode it while processing the
        // path resolution
