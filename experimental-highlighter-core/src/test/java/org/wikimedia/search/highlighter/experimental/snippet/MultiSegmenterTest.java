...
public class MultiSegmenterTest extends RandomizedTest {
	...
	public void startWithSomeEmptyThenSingleChar() {
		...
		assertThat(
				segmenter.memo(offsetGap * 3, offsetGap * 3 + 1).pickBounds(0,
						Integer.MAX_VALUE), extracted(extracter, equalTo("a")));
		if (offsetGap > 0) {
			assertFalse(segmenter.acceptable(0, 1));
		}
	...
	public void startWithSingleCharThenSomeOthers() {
		...
		assertThat(
				segmenter.memo(offsetGap * 4 + 5, offsetGap * 4 + 36)
						.pickBounds(0, Integer.MAX_VALUE),
				extracted(
						extracter,
						equalTo("The quick brown fox jumped over the lazy dog.")));
	...
	public void twoSentences() {
		...
		assertThat(
				segmenter.memo(offsetGap + 34, offsetGap + 38).pickBounds(0,
						Integer.MAX_VALUE),
	...
	private void setup(String... sources) {
		...
		for (String source : sources) {
			builder.add(new CharScanningSegmenter(source, 100, 20),
					source.length());
			extracterBuilder.add(new StringSourceExtracter(source),
					source.length());
		}
