...
public abstract class AbstractMultiSourceExtracterTestBase extends
		RandomizedTest {
	protected abstract Builder<String, ? extends Builder<String, ?>> builder(
			int offsetGap);
	...
	public void entirelyWithinOneString() {
		...
		assertEquals("baz",
				extracter.extract(6 + offsetGap * 2, 9 + offsetGap * 2));
		assertEquals("cupcakes",
				extracter.extract(9 + offsetGap * 3, 17 + offsetGap * 3));
		assertEquals("up",
				extracter.extract(10 + offsetGap * 3, 12 + offsetGap * 3));
	}
