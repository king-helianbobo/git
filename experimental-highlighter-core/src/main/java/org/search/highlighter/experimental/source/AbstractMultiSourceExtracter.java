package org.search.highlighter.experimental.source;
 ...
 */
public abstract class AbstractMultiSourceExtracter<T> implements
		SourceExtracter<T> {
	public interface Builder<T, S extends Builder<T, S>> {
		 ...
		 * 
		 * @param segmenter
		 *            the segmenter to delegate to
		 * @param length
		 *            the length of the source underlying the segmenter
	...
	public AbstractMultiSourceExtracter(
			List<ConstituentExtracter<T>> extracters, int offsetGap) {
	 ...
	 * 
	 * @param extracts
	 *            extracts from all constituent extracters
	...
	public T extract(int startOffset, int endOffset) {
		...
		if (extracter == null) {
			throw new IllegalArgumentException(
					"startOffset after length of last extracter");
		}
