...
import org.highlighter.experimental.es.BytesRefHashTermInfos;
import org.highlighter.experimental.es.CharScanningSegmenterFactory;
import org.highlighter.experimental.es.DelayedSegmenter;
import org.highlighter.experimental.es.ElasticsearchQueryFlattener;
import org.highlighter.experimental.es.FetchedFieldIndexPicker;
import org.highlighter.experimental.es.SegmenterFactory;
import org.highlighter.experimental.es.SentenceIteratorSegmenterFactory;
import org.highlighter.experimental.es.WholeSourceSegmenterFactory;
import org.highlighter.experimental.lucene.AutomatonHitEnum;
import org.highlighter.experimental.lucene.BasicQueryWeigher;
import org.search.highlighter.experimental.HitEnum;
import org.search.highlighter.experimental.Snippet;
import org.search.highlighter.experimental.SnippetChooser;
import org.search.highlighter.experimental.SnippetFormatter;
import org.search.highlighter.experimental.SnippetWeigher;
import org.search.highlighter.experimental.hit.ConcatHitEnum;
import org.search.highlighter.experimental.hit.EmptyHitEnum;
import org.search.highlighter.experimental.hit.MergingHitEnum;
import org.search.highlighter.experimental.hit.OverlapMergingHitEnumWrapper;
import org.search.highlighter.experimental.hit.RegexHitEnum;
import org.search.highlighter.experimental.hit.ReplayingHitEnum.HitEnumAndLength;
import org.search.highlighter.experimental.snippet.BasicScoreBasedSnippetChooser;
import org.search.highlighter.experimental.snippet.BasicSourceOrderSnippetChooser;
import org.search.highlighter.experimental.snippet.ExponentialSnippetWeigher;
import org.search.highlighter.experimental.snippet.SumSnippetWeigher;
...
public class ExperimentalHighlighter implements Highlighter {
	...
	public HighlightField highlight(HighlighterContext context) {
		try {
			CacheEntry entry = (CacheEntry) context.hitContext.cache().get(
					CACHE_KEY);
			if (entry == null) {
				entry = new CacheEntry();
				context.hitContext.cache().put(CACHE_KEY, entry);
			}
			HighlightExecutionContext executionContext = new HighlightExecutionContext(
					context, entry);
			try {
			...
			throw new FetchPhaseExecutionException(context.context,
					"Failed to highlight field [" + context.fieldName + "]", e);
		}
	}
	...
	static class HighlightExecutionContext {
		...
		HighlightField highlight() throws IOException {
			...
			int numberOfSnippets = context.field.fieldOptions()
					.numberOfFragments();
		...
		private void ensureWeigher() {
			...
			QueryCacheKey key = new QueryCacheKey(
					context.query.originalQuery(), phraseAsTerms);
			...
			weigher = new BasicQueryWeigher(new ElasticsearchQueryFlattener(
					100, phraseAsTerms), infos,
		...
		private List<HitEnum> buildRegexHitEnums() throws IOException {
			...
			for (String regex : getRegexes()) {
				if (luceneRegex) {
					...
					AutomatonHitEnum.Factory factory = cache.automatonHitEnumFactories
							.get(regex);
					if (factory == null) {
					...
					hitEnums.add(buildJavaRegexHitEnumForRegex(
							Pattern.compile(regex, options), fieldValues));
				}
			}
			return hitEnums;
		}
		...
		private HitEnum buildLuceneRegexHitEnumForRegex(
				final AutomatonHitEnum.Factory factory,
			...
			if (fieldValues.size() == 1) {
				...
				Iterator<HitEnumAndLength> hitEnumsFromStreams = Iterators
						.transform(fieldValues.iterator(),
								new Function<String, HitEnumAndLength>() {
									@Override
									public HitEnumAndLength apply(
											String fieldValue) {
										if (caseInsensitive) {
											fieldValue = fieldValue
													.toLowerCase(getLocale());
										}
										return new HitEnumAndLength(factory
												.build(fieldValue), fieldValue
												.length());
									}
								});
		...
		private HitEnum buildJavaRegexHitEnumForRegex(final Pattern pattern,
				...
				Iterator<HitEnumAndLength> hitEnumsFromStreams = Iterators
						.transform(fieldValues.iterator(),
								new Function<String, HitEnumAndLength>() {
									...
									public HitEnumAndLength apply(
											String fieldValue) {
										return new HitEnumAndLength(
												new RegexHitEnum(pattern
														.matcher(fieldValue)),
												fieldValue.length());
									}
								});
		...
		private boolean isLuceneRegexFlavor() {
			...
			throw new IllegalArgumentException("Unknown regex flavor:  "
					+ regexFlavor);
		}
		 ...
		 */
		private List<HitEnum> buildLuceneHitFindingHitEnums()
				throws IOException {
			Set<String> matchedFields = context.field.fieldOptions()
					.matchedFields();
			...
			List<HitEnum> hitEnums = new ArrayList<HitEnum>(
					matchedFields.size());
		...
		private SnippetChooser buildScoreBasedSnippetChooser(
			...
			return new BasicScoreBasedSnippetChooser(scoreOrdered,
					buildSnippetWeigher(), maxFragmentsScored);
		}
		...
		private SnippetWeigher buildSnippetWeigher() {
			...
			try {
				...
				throw new IllegalArgumentException(
						"Invalid snippet weigher config:  " + config, e);
			}
			throw new IllegalArgumentException(
					"Invalid snippet weigher config:  " + config);
		}

		private Text[] formatSnippets(List<Snippet> snippets)
				throws IOException {
			SnippetFormatter formatter = new SnippetFormatter(
					defaultField.buildSourceExtracter(), context.field
							.fieldOptions().preTags()[0], context.field
							.fieldOptions().postTags()[0]);
			...
			FetchedFieldIndexPicker picker = segmenter
					.buildFetchedFieldIndexPicker();
			int i = 0;
			for (Snippet snippet : snippets) {
				result[i++] = new StringText(formatter.format(snippet));
		...
		private List<FieldWrapper> buildFetchFields() {
			...
			List<FieldWrapper> fetchFieldWrappers = new ArrayList<FieldWrapper>(
					fetchFields.size());
		...
		private Locale getLocale() {
			...
			locale = localeString == null ? Locale.US : Strings
