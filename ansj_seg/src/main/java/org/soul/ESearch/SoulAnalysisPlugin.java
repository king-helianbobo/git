...
import org.ansj.treeSplit.GetTrieWords;
...
import org.soul.lucene4.SoulTokenizer;
...
public class SoulAnalysisPlugin extends AbstractPlugin {
	...
	public static void main(String[] args) {
		...
		Forest forest = null;
		try {
			forest = Library.makeForest(new BufferedReader(
					new StringReader(dic)));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		GetTrieWords getWords = forest.getWord(content);

		String temp = null;
		while ((temp = getWords.getFrontWords()) != null)
			System.out.println(temp + "\t\t" + getWords.getParam(1) + "\t\t"
					+ getWords.getParam(2));
		...
		try {
			while (tokenizer.incrementToken()) {
				CharTermAttribute attribute = tokenizer
